...
//! with `cargo nextest run -p dory_transfer --run-ignored all` against a
//! Docker daemon.
...
use dory_transfer::export::{ExportOptions, ExportTable, run_export};
use dory_transfer::import::{ImportOptions, ImportTablePlan, run_import};
use dory_transfer::migration::{
...
fn postgres_export_streams_table_to_csv_and_manifest() -> Result<(), DbError> {
    containers::with_postgres_url(|uri| {
        ...
        let dir =
            std::env::temp_dir().join(format!("dory_transfer_live_export_{}", std::process::id()));
...
fn postgres_export_then_import_round_trips_row_values() -> Result<(), DbError> {
    containers::with_postgres_url(|uri| {
        ...
        let plans = vec![ImportTablePlan {
            ...
            mapping_mode: dory_transfer::TableMappingMode::Existing,
        ...
        assert_eq!(
            ...
            dory_transfer::TableTransferStatus::Completed { rows: 5 }
        );
...
fn postgres_migration_orders_parent_before_child_via_real_fk_constraint() -> Result<(), DbError> {
    containers::with_postgres_url(|uri| {
        ...
        let plans = vec![
            MigrationTablePlan {
                source_table: dory_core::TableRef::with_schema("public", "child_orders"),
            ...
            MigrationTablePlan {
                source_table: dory_core::TableRef::with_schema("public", "parent_accounts"),
