...
//! per table via `dory_export`'s streaming writers. CSV reuses the exact
...
//! `dory_export::JsonStreamWriter`) so `file_source::FileSource` can import
...
use dory_core::{ColumnKind, ColumnMeta, TransferColumn};
use dory_export::{CsvStreamWriter, JsonStreamWriter};
...
mod tests {
    ...
    fn temp_dir(label: &str) -> PathBuf {
        let dir = std::env::temp_dir().join(format!(
            "dory_transfer_file_sink_test_{label}_{}",
            std::process::id()
        ));
