...
impl CombinedPemFile {
    ...
    pub(crate) fn create(cert_path: &Path, key_path: &Path) -> io::Result<Self> {
        ...
        let filename = format!("dory-mongo-pem-{}.pem", Uuid::new_v4());
...
fn mongo_collection_shell_prefix(collection: &dory_core::CollectionRef) -> String {
    format!(
...
fn plan_mongo_collection_browse(
    ...
    Ok(SemanticPlan::single_query(
        ...
        dory_core::PlannedQuery::new(QueryLanguage::MongoQuery, query)
...
fn mongo_field_path_from_column(column: &dory_core::ColumnRef) -> String {
...
fn plan_mongo_aggregate(request: &dory_core::AggregateRequest) -> Result<SemanticPlan, DbError> {
        ...
        for order in &request.order_by {
            ...
            sort.insert(
                ...
                serde_json::json!(match order.direction {
                    dory_core::SortDirection::Ascending => 1,
                    dory_core::SortDirection::Descending => -1,
                }),
            );
        }
    ...
    Ok(SemanticPlan::single_query(
        SemanticPlanKind::Query,
        dory_core::PlannedQuery::new(QueryLanguage::MongoQuery, query_text)
            .with_database(target_database),
    ))
}

fn plan_mongo_mutation(mutation: &dory_core::MutationRequest) -> Result<SemanticPlan, DbError> {
...
mod tests {
    ...
    fn semantic_planner_builds_aggregate_preview() {
        let request = dory_core::AggregateRequest::new(dory_core::TableRef::new("orders"))
            ...
            .with_aggregations(vec![
                dory_core::AggregateSpec::count_all("total_orders"),
                dory_core::AggregateSpec::new(
            ...
            .with_order_by(vec![dory_core::OrderByColumn::desc("total_amount")])
            .with_limit(Some(10))
    ...
    mod combined_pem_file_tests {
        ...
        impl TempDir {
            fn new(label: &str) -> Self {
                ...
                path.push(format!("dory-test-{}-{}", label, uuid::Uuid::new_v4()));
    ...
    fn mongo_ssl_mode_verify_with_separate_cert_and_key_uses_combined_pem_path() {
        ...
        let combined_path = "/tmp/dory-mongo-pem-xyz.pem";
        let params = mongo_ssl_mode_to_uri_params(
            Some("verify"),
            Some("/etc/ssl/ca.pem"),
            Some(combined_path),
        ...
        assert!(params.contains("tls=true"));
        assert!(params.contains("tlsCAFile=%2Fetc%2Fssl%2Fca.pem"));
        assert!(params.contains("tlsCertificateKeyFile=%2Ftmp%2Fdory-mongo-pem-xyz.pem"));
    }
