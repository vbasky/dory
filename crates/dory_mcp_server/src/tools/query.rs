...
impl DoryServer {
    ...
    fn build_explain_request(
        ...
        let table_ref = table
            ...
            .unwrap_or_else(|| TableRef::new("__dory_explain__"));
    ...
    ) -> Result<dory_core::QueryResult, String> {
        tokio::task::spawn_blocking(move || {
            match connection.plan_semantic_request(&SemanticRequest::Explain(request.clone())) {
                Ok(plan) => {
                    ...
                    if !matches!(
                        classification,
                        dory_policy::ExecutionClassification::Metadata
                            | dory_policy::ExecutionClassification::Read
                    ) {
                        return Err(format!(
                ...
                Err(dory_core::DbError::NotSupported(_)) => connection
                    .explain(&request)
                    .map_err(|e| format!("{} error: {}", error_prefix, e)),
...
mod tests {
    ...
    impl Connection for TestConnection {
        ...
        fn execute(&self, req: &dory_core::QueryRequest) -> Result<QueryResult, DbError> {
    ...
    fn build_explain_request_uses_default_schema_for_tables() {
        ...
        let request = DoryServer::build_explain_request(&connection, None, Some("users")).unwrap();
    ...
    async fn run_explain_request_executes_driver_plan_when_available() {
        let connection = Arc::new(TestConnection::new(
            Some(SemanticPlan::single_query(
                ...
                dory_core::PlannedQuery::new(QueryLanguage::Sql, "EXPLAIN SELECT 1"),
            )),
            true,
        ));

        DoryServer::run_explain_request(
            connection.clone(),
    ...
    async fn run_explain_request_falls_back_to_legacy_explain_when_planning_is_unsupported() {
        ...
        DoryServer::run_explain_request(connection.clone(), request, "Preview")
            .await
            .unwrap();
    ...
    async fn run_explain_request_rejects_non_read_only_plans() {
        let connection = Arc::new(TestConnection::new(
            Some(SemanticPlan::single_query(
                SemanticPlanKind::Query,
                dory_core::PlannedQuery::new(QueryLanguage::Sql, "UPDATE users SET active = true"),
            )),
            true,
        ));

        let error = DoryServer::run_explain_request(
            connection.clone(),
            ExplainRequest::new(TableRef::new("users")).with_query("SELECT 1"),
            "Preview",
        )
