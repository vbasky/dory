...
/// `SchemaAlterKind` ladder shared with `dory_core`'s schema diff. Unknown
...
fn schema_alter_kind_for_op(op: &AlterOperation) -> Option<dory_policy::SchemaAlterKind> {
    use dory_policy::SchemaAlterKind;
...
) -> dory_policy::ExecutionClassification {
    use dory_policy::{ExecutionClassification, classify_schema_alter};
...
fn generate_add_foreign_key_sql(
    code_generator: &dyn dory_core::CodeGenerator,
    table: &TableRef,
    constraint_name: &str,
    definition: &serde_json::Value,
...
async fn lookup_postgres_custom_type_kind(
    ...
    DoryServer::execute_connection_blocking(connection, move |connection| {
        connection
            .execute(&request)
...
fn build_alter_op_sql(
    op: &AlterOperation,
    table_quoted: &str,
    dialect: &dyn dory_core::SqlDialect,
    code_generator: &dyn dory_core::CodeGenerator,
    table_ref: &TableRef,
) -> Result<Vec<String>, String> {
    let action_upper = op.action.to_uppercase();
    match action_upper.as_str() {
        "ADD_COLUMN" | "ADD COLUMN" => {
            ...
            let request = dory_core::AddColumnRequest {
        ...
        "DROP_COLUMN" | "DROP COLUMN" => {
            ...
            let request = dory_core::DropColumnRequest {
                table_name: &table_ref.name,
                schema_name: table_ref.schema.as_deref(),
                column_name: col_name,
            };
        ...
        "ALTER_COLUMN" | "ALTER COLUMN" => {
            ...
            let default = match &default_literal {
                None => None,
                Some(None) => Some(dory_core::DefaultSpec::Drop),
                Some(Some(literal)) => Some(dory_core::DefaultSpec::Set(literal.as_str())),
            };

            let request = dory_core::AlterColumnRequest {
                table_name: &table_ref.name,
                schema_name: table_ref.schema.as_deref(),
                column_name: col_name,
                new_type,
...
impl DoryServer {
    ...
    async fn create_table_impl(
        ...
        for col in columns {
            dory_core::validate_ddl_fragment(&col.r#type, "column type")
                .map_err(|rejection| rejection.reason)?;
        }
    ...
    async fn run_alter_transactional(
        connection: std::sync::Arc<dyn dory_core::Connection>,
        ops: &[AlterOperation],
        table: &str,
...
mod tests {
    ...
    fn test_parse_custom_type_kind_result_reads_kind_values() {
        let result = dory_core::QueryResult::table(
            Vec::new(),
...
// tested exhaustively in `dory_policy::schema_alter` against
...
/// `dory_core`'s schema-diff classification path: both map onto
/// `dory_policy::classify_schema_alter`, so equivalent operations must
/// agree on risk.
#[cfg(test)]
mod diff_parity_tests {
    use super::*;
    use dory_core::{ColumnInfo, TableChange, TableInfo, diff_schema};
    use dory_policy::ExecutionClassification;
...
mod alter_table_integration_tests {
    ...
    async fn alter_table_rolls_back_on_mid_batch_failure() {
        ...
        let result =
            DoryServer::alter_table_impl(state, &connection_id, "test_rollback", &ops).await;
    ...
    async fn alter_table_commits_all_ops_atomically() {
        ...
        let result = DoryServer::alter_table_impl(state, &connection_id, "test_atomic", &ops).await;
    ...
    async fn alter_table_marks_non_atomic_flag() {
        ...
        let result = DoryServer::run_alter_non_atomic(connection, &ops, "test_tbl").await;
