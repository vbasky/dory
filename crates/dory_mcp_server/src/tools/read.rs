...
impl DoryServer {
    ...
    async fn select_data_document(
        ...
        database: Option<&str>,
        filter: Option<&serde_json::Value>,
        semantic_filter: Option<&dory_core::SemanticFilter>,
        limit: u32,
        offset: u32,
    ) -> Result<serde_json::Value, String> {
...
mod tests {
    ...
    impl Connection for UnsupportedAggregateConnection {
        ...
        fn execute(&self, _req: &dory_core::QueryRequest) -> Result<QueryResult, DbError> {
        ...
        ) -> Result<dory_core::SemanticPlan, DbError> {
    ...
    fn test_aggregate_function_accepts_case_insensitive_names() {
        assert!(matches!(
            DoryServer::aggregate_function("SuM"),
    ...
    fn build_sqlite_state_read(
        ...
        let audit_path =
            dory_audit::temp_sqlite_path(&format!("read_test_{}.sqlite", uuid::Uuid::new_v4()));
        let audit_service =
    ...
    async fn aggregate_data_impl_sql_text_is_some_and_contains_select() {
        ...
        let (_, sql_text) = DoryServer::aggregate_data_impl(
            state,
            &connection_id,
            "orders",
            None,
