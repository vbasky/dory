...
impl GovernanceMiddleware {
        ...
        Fut: Future<Output = Result<(CallToolResult, AuditDetails), McpError>>,
    {
        ...
        let clients: Vec<dory_policy::TrustedClient> = trusted_clients_dto
            .into_iter()
            .map(|dto| dory_policy::TrustedClient {
                id: dto.id,
                name: dto.name,
                issuer: dto.issuer,
                active: dto.active,
            ...
            .collect();
        let trusted_clients = dory_policy::TrustedClientRegistry::new(clients);
        ...
        let policy_engine = dory_policy::PolicyEngine::new(assignments, roles, policies);
...
fn execution_action(tool_id: &str, outcome: EventOutcome) -> dory_core::observability::AuditAction {
...
mod tests {
    use super::*;
    use dory_mcp::{McpRuntime, builtin_policies, builtin_roles};
    use std::sync::Arc;
    ...
    fn create_test_state() -> ServerState {
        ...
        let temp_path = dory_audit::temp_sqlite_path(&format!(
            "test_audit_{}.sqlite",
            std::time::SystemTime::now()
                .duration_since(std::time::UNIX_EPOCH)
                .unwrap()
        ...
        let audit_service = dory_audit::AuditService::new_sqlite(&temp_path)
        ...
        let _ = runtime.upsert_trusted_client_mut(dory_mcp::TrustedClientDto {
        ...
        let _ = runtime.save_connection_policy_assignment_mut(
            dory_mcp::ConnectionPolicyAssignmentDto {
                ...
                assignments: vec![dory_policy::ConnectionPolicyAssignment {
        ...
        ServerState {
            ...
            profile_manager: Arc::new(RwLock::new(dory_core::ProfileManager::new_in_memory())),
            ...
            secret_manager: Arc::new(dory_core::SecretManager::new(Box::new(
                dory_core::NoopSecretStore,
            ))),
