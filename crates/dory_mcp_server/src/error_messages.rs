...
pub fn connection_error(
    ...
    format!(
        "Failed to establish database connection: {}\n\
         ...
         \n\
         Troubleshooting:\n\
         • Verify credentials in Dory GUI → Edit Connection\n\
         ...
         • Test with Dory GUI first to isolate MCP-specific issues",
...
pub fn connection_not_found(connection_id: &str) -> String {
    format!(
        "Connection not found: {}\n\
         ...
         • Verify connection was not deleted in Dory GUI\n\
         • Check if connection was created in a different Dory profile\n\
         • Connection profiles are stored in the Dory database — open the Connection Manager in the Dory GUI to manage them",
        connection_id
    )
}
...
pub fn driver_not_available(driver_id: &str, available_drivers: &[String]) -> String {
    ...
    format!(
        "Database driver not available: '{}'\n\
         ...
         • Rebuild server with driver: cargo build -p dory --features {}\n\
         • Check the connection profile driver_id via the Connection Manager in the Dory GUI",
...
pub fn query_execution_error(
    ...
    format!(
        "Query execution failed: {}\n\
         ...
         • Test query in Dory GUI query editor\n\
...
pub fn authorization_denied(
    ...
    format!(
        "Authorization denied: {}\n\
         ...
         \n\
         Resolution:\n\
         • Open Dory GUI → Connection Manager\n\
...
pub fn script_error(operation: &str, script_id: &str, error: impl std::fmt::Display) -> String {
    format!(
        "Failed to {} script: {}\n\
         ...
         • Verify file exists: ls -la ~/.local/share/dory/scripts/{}\n\
...
pub fn config_error(
    ...
    let path_info = config_path
        ...
        .unwrap_or_else(|| "Config path: ~/.local/share/dory".to_string());
    ...
    format!(
        "Failed to {}: {}\n\
         ...
         \n\
         Common causes:\n\
         • dory.db is missing, corrupted, or inaccessible\n\
         • Insufficient permissions\n\
         • Directory does not exist\n\
         • Disk full\n\
         \n\
         Resolution:\n\
         • Check permissions: ls -la ~/.local/share/dory/\n\
         • Ensure directory exists: mkdir -p ~/.local/share/dory\n\
         • Verify disk space: df -h ~/.local\n\
         • Test integrity: sqlite3 ~/.local/share/dory/dory.db 'PRAGMA integrity_check;'",
...
pub fn audit_error(operation: &str, error: impl std::fmt::Display) -> String {
    format!(
        "Audit operation failed: {}\n\
         ...
         \n\
         Resolution:\n\
         • Check audit database: ~/.local/share/dory/dory.db (aud_audit_events table)\n\
         • Verify permissions: chmod 644 ~/.local/share/dory/dory.db\n\
         • Check disk space: df -h ~/.local\n\
         • Test integrity: sqlite3 ~/.local/share/dory/dory.db 'PRAGMA integrity_check;'\n\
         \n\
         If database is corrupted (WARNING: loses audit history):\n\
         • rm ~/.local/share/dory/dory.db\n\
