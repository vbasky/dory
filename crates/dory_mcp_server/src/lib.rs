...
pub async fn run_mcp_server(args: McpServerArgs) -> anyhow::Result<()> {
    ...
    let bridge_handle = match init_tracing(bridge_config) {
        Ok(h) => Some(h),
        Err(err) => {
            eprintln!("dory-mcp-server: failed to initialize tracing: {err}");
            None
        }
    };

    log::info!("dory-mcp-server starting, client_id={}", args.client_id);
    ...
    log::info!("dory-mcp-server initialized");

    let server = DoryServer::new(state);

    let service = server.serve(stdio()).await?;

    log::info!("dory-mcp-server ready");

    service.waiting().await?;

    log::info!("dory-mcp-server shutting down");

    if let Some(handle) = bridge_handle {
        use dory_core::observability::tracing_bridge::ShutdownError;
        match handle.shutdown() {
            ...
            Err(ShutdownError::DrainTimeout {
                ...
                eprintln!(
                    "dory-mcp-server: audit bridge shutdown timed out, dropped {} in-flight events",
                    remaining_in_flight
                );
            }
            Err(ShutdownError::JoinPanic) => {
                eprintln!("dory-mcp-server: audit bridge drain thread panicked during shutdown");
            }
        }
    }
...
fn resolve_mcp_log_path() -> PathBuf {
    if let Ok(data_dir) = dory_storage::paths::data_dir() {
        let logs_dir = data_dir.join("logs");
        return logs_dir.join("dory_mcp.log");
    }
    std::env::temp_dir().join("dory_mcp.log")
}
