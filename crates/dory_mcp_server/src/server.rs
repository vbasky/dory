...
use dory_core::access::{AccessHandle, AccessKind, AccessManager};
use dory_core::auth::SharedDynAuthProvider;
use dory_core::secrecy::SecretString;
use dory_core::values::{CompositeValueResolver, ValueCache, ValueRef};
use dory_core::{CancelToken, Connection, ConnectionOverrides, PipelineInput};
...
struct McpAccessManager {
    #[cfg(feature = "aws")]
    ssm_factory: Option<Arc<dory_ssm::SsmTunnelFactory>>,
}

impl McpAccessManager {
    #[cfg(feature = "aws")]
    fn new(ssm_factory: Option<Arc<dory_ssm::SsmTunnelFactory>>) -> Self {
        Self { ssm_factory }
    }
    ...
    async fn open_managed(
        ...
        remote_host: &str,
    ) -> Result<AccessHandle, dory_core::DbError> {
        match provider {
            #[cfg(feature = "aws")]
            "aws-ssm" => {
                ...
                let factory = self.ssm_factory.as_ref().ok_or_else(|| {
                    dory_core::DbError::connection_failed("SSM tunnel factory not available")
                })?;
            ...
            _ => {
                let _ = params;
                let _ = remote_host;
                Err(dory_core::DbError::connection_failed(
                    "Managed access providers are not available in this build",
                ))
            }
            #[cfg(feature = "aws")]
            other => Err(dory_core::DbError::connection_failed(format!(
...
impl AccessManager for McpAccessManager {
    async fn open(
        ...
        match access_kind {
            ...
            AccessKind::Ssh { .. } => Err(dory_core::DbError::connection_failed(
                "SSH tunnels are not supported by the MCP server access manager",
            )),
            AccessKind::Proxy { .. } => Err(dory_core::DbError::connection_failed(
...
impl McpConnectionFactory {
    ...
    async fn connect_with_pipeline(
        ...
        driver_id: String,
        driver: Arc<dyn dory_core::DbDriver>,
        ...
        let pipeline_output = dory_core::run_pipeline(pipeline_input, &state_tx)
            .await
    ...
    async fn connect_direct(
        ...
        let resolved_secrets = DoryServer::resolve_profile_secrets(&self.state, &profile)?;
    ...
    async fn connect_and_cache(&self, connection_id: &str) -> Result<(), String> {
        DoryServer::get_or_connect(self.state.clone(), connection_id).await?;
        Ok(())
    }
}

/// Main Dory MCP Server
#[derive(Clone)]
pub(crate) struct DoryServer {
    ...
    pub(crate) tool_router: ToolRouter<DoryServer>,
}
...
impl DoryServer {
    ...
    fn classify_query(&self, query: &str) -> dory_policy::ExecutionClassification {
...
impl ServerHandler for DoryServer {
    fn get_info(&self) -> ServerInfo {
        ServerInfo::new(ServerCapabilities::builder().enable_tools().build()).with_instructions(
            "Dory MCP Server - AI-powered database client with governance controls.\n\
             \n\
...
mod tests {
    ...
    use dory_mcp::McpRuntime;
    use dory_test_support::FakeDriver;
    ...
    impl DbDriver for RecordingDriver {
        ...
        fn form_definition(&self) -> &dory_core::DriverFormDef {
            self.inner.form_definition()
        }

        fn build_config(&self, values: &FormValues) -> Result<DbConfig, dory_core::DbError> {
        ...
        fn connect_with_secrets(
            ...
            password: Option<&SecretString>,
            ssh_secret: Option<&SecretString>,
        ) -> Result<Box<dyn Connection>, dory_core::DbError> {
            self.inner
        ...
        fn test_connection(&self, profile: &ConnectionProfile) -> Result<(), dory_core::DbError> {
    ...
    fn test_state_with_driver(
        ...
        ServerState {
            ...
            secret_manager: Arc::new(dory_core::SecretManager::new(Box::new(NoopSecretStore))),
    ...
    async fn server_handler_exposes_tools_from_all_sub_routers() {
        ...
        let server = DoryServer::new(state);
