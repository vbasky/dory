...
Complete reference for WHERE clause syntax in Dory MCP tools.
...
Dory automatically coerces JSON types to match column types:
...
If a WHERE clause has invalid syntax, Dory returns a structured error:
...
If coercion fails, Dory returns:
...
The WHERE clause syntax is a powerful, driver-agnostic filtering language for Dory MCP tools. By using JSON operators and logical composition, AI agents can query any database without knowing SQL, MongoDB query syntax, or DynamoDB filter expressions.
