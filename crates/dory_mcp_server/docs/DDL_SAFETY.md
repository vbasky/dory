...
Complete reference for DDL (Data Definition Language) operations in Dory MCP.
...
DDL operations modify database schema and structure, making them inherently higher risk than data queries. Dory implements a multi-layered safety system:
...
Dory uses `ExecutionClassification` to categorize operations by risk:
...
`classify_alter_table_operation()` in `dory_core/src/query/classify.rs`:
...
2. **Dory generates SQL** using `QueryGenerator` trait
...
Dory ships with a default policy:
...
Dory requires exact table name in `confirm` parameter:
...
```bash
./target/debug/dory mcp --client-id test
```
...
Dory includes integration tests for DDL operations:
...
```bash
cargo test -p dory_policy test_alter_table
```
...
DDL operations are powerful but risky. Dory provides multiple safety layers:
...
- [Dory Policy Documentation](../../dory_policy/README.md)
- [Dory Core Documentation](../../dory_core/README.md)
