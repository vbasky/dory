...
async fn create_test_server() -> (Box<dyn std::any::Any>, DuplexStream) {
    ...
    todo!("Requires ServerState::new() to be accessible and DoryServer to be constructible")
}
...
#[allow(dead_code)]
fn create_test_server_state() -> dory_mcp_server::state::ServerState {
    ...
    let audit_service = dory_audit::AuditService::new_sqlite(&audit_path).unwrap();
    ...
    dory_mcp_server::state::ServerState {
        client_id: "test-client".to_string(),
        ...
        connection_cache: Arc::new(RwLock::new(
            dory_mcp_server::connection_cache::ConnectionCache::new(),
        )),
...
// 2. Exposing DoryServer::new() as public API
