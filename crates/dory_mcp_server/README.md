...
`dory_mcp_server` provides a JSON-RPC interface over stdin/stdout for AI clients (Claude Desktop, OpenAI GPT, etc.) to interact with databases through Dory's governance layer.
...
# Build and run
cargo build -p dory
./target/debug/dory mcp --client-id my-ai-client

# Or run directly
cargo run -p dory -- mcp --client-id my-ai-client
```
...
Dory uses a unified JSON WHERE clause syntax that works across all database drivers.
...
Dory evaluates every operation against a policy:
...
All operations are logged to SQLite (`~/.local/share/dory/dory.db`, `aud_audit_events` table):
...
Dory stores connection profiles in `~/.local/share/dory/dory.db` (`cfg_connection_profiles` table). Use the Connection Manager in the Dory GUI to manage profiles. For reference, a profile contains fields like:
...
MCP settings are stored in `~/.local/share/dory/dory.db`. Example governance configuration:
...
See driver READMEs:
- [PostgreSQL](../dory_driver_postgres/README.md)
- [MySQL](../dory_driver_mysql/README.md)
- [SQLite](../dory_driver_sqlite/README.md)
...
Dory uses the `ColumnRef` type to represent column references in WHERE clauses:
...
See `crates/dory_core/src/query/column_ref.rs` for implementation.
...
```bash
# Unit tests
cargo test -p dory_mcp
cargo test -p dory_policy
cargo test -p dory_approval
cargo test -p dory_audit

# Integration tests (requires Docker)
cargo test -p dory_mcp_server --test integration
```
...
```bash
RUST_LOG=dory_mcp=debug ./target/debug/dory mcp --client-id test
```
...
```bash
RUST_LOG=dory_mcp=trace ./target/debug/dory mcp --client-id test
```
...
1. Define tool schema in `crates/dory_mcp/src/tool_catalog.rs`
2. Implement handler in `crates/dory_mcp/src/handlers/`
3. Register in `McpRuntime::handle_tool_call()`
4. Add tests in `crates/dory_mcp/tests/`
5. Update this README

### Adding New Drivers

1. Implement `DbDriver` trait in new crate
2. Register in `crates/dory/src/app.rs`
...
- [Policy Documentation](../dory_policy/README.md) — Policy engine reference
- [Audit Documentation](../dory_audit/README.md) — Audit logging reference
