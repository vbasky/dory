...
use dory_ui_base::hook_phase_runner::{DetachedHookScope, HookPhaseState, run_hook_phase};
...
impl ConnectionManagerWindow {
    ...
    pub(super) fn validate_form(&mut self, require_name: bool, cx: &mut Context<Self>) -> bool {
        ...
        if require_name {
            ...
            if name.trim().is_empty() {
                self.validation_errors
                    .push(dory_i18n::t!("form.validation.connection_name_required"));
            }
        }

        let Some(driver) = &self.form.selected_driver else {
            self.validation_errors
                .push(dory_i18n::t!("form.validation.no_driver_selected"));
            return false;
        };
        ...
        for tab in form.tabs.iter().filter(|t| t.id != "ssh") {
            for section in &tab.sections {
                for field in &section.fields {
                    ...
                    {
                        self.validation_errors.push(dory_i18n::t!(
        ...
        if self.access.ssh_enabled && form.supports_ssh() {
            ...
            if ssh_host.trim().is_empty() {
                self.validation_errors
                    .push(dory_i18n::t!("form.validation.ssh_host_required"));
            }

            let ssh_user = self.access.input_ssh_user.read(cx).value().to_string();
            if ssh_user.trim().is_empty() {
                self.validation_errors
                    .push(dory_i18n::t!("form.validation.ssh_user_required"));
            }
            ...
            if !ssh_port_str.trim().is_empty() && ssh_port_str.parse::<u16>().is_err() {
                self.validation_errors
                    .push(dory_i18n::t!("form.validation.ssh_port_invalid"));
            }
        }
        ...
        if self.is_ssm_selected() {
            ...
            if !self.has_dynamic_value_ref_for_field("ssm_remote_port", cx) {
                match port_str.parse::<u16>() {
                    Ok(0) => {
                        self.validation_errors
                            .push(dory_i18n::t!("form.validation.ssm_remote_port_positive"));
                    }
                    Err(_) => {
                        self.validation_errors
                            .push(dory_i18n::t!("form.validation.ssm_remote_port_invalid"));
                    }
                    _ => {}
                }
            }
    ...
    pub(super) fn save_profile(&mut self, window: &mut Window, cx: &mut Context<Self>) {
        ...
        let save_action = if is_edit {
            dory_i18n::t!("form.action.updating")
        } else {
            dory_i18n::t!("form.action.saving")
        };
        info!(
        ...
        {
            Toast::warning(dory_i18n::t!("form.warning.refresh_interval_invalid"))
                .meta_right(now_hms())
                .push(cx);
        }
        ...
        self.app_state.update(cx, |state, cx| {
            ...
            #[cfg(feature = "mcp")]
            {
                ...
                cx.emit(dory_ui_base::McpRuntimeEventRaised {
                    event: dory_mcp::McpRuntimeEvent::ConnectionPolicyUpdated {
                        connection_id: saved_profile_id.to_string(),
                    },
                });
            }
    ...
    pub(super) fn test_connection(&mut self, window: &mut Window, cx: &mut Context<Self>) {
        ...
        let Some(profile) = self.build_profile(cx) else {
            ...
            self.test_error = Some(dory_i18n::t!("form.error.build_profile_failed"));
            cx.notify();
            return;
        };
        ...
        let Some(driver) = self.form.selected_driver.clone() else {
            self.test_status = TestStatus::Failed;
            self.test_error = Some(dory_i18n::t!("form.validation.no_driver_selected"));
            cx.notify();
            return;
        };
                ...
                move |drop_guards| {
                    ...
                    Box::pin(async move {
                        if let Some(pipeline_input) = pipeline_input {
                            background_executor
                                .spawn(async move {
                                    let (state_tx, _state_rx) = dory_core::pipeline_state_channel();
                                    let pipeline_output =
                                        dory_core::run_pipeline(pipeline_input, &state_tx)
                                            .await
                                            .map_err(|error| {
                                                dory_i18n::t!(
                                    ...
                                    Ok(dory_core::TestConnectionResult::default())
                                })
                                .await
                        } else {
                            background_executor
...
struct TestConnectionOrchestrationResult {
    test_result: dory_core::TestConnectionResult,
    warnings: Vec<String>,
}

async fn run_test_connection_orchestration<'a, RunPhase, RunProbe, RunCleanup>(
    hooks: dory_core::ConnectionHooks,
    run_phase: RunPhase,
    run_probe: RunProbe,
    run_cleanup: RunCleanup,
    hook_context: dory_core::HookContext,
) -> Result<TestConnectionOrchestrationResult, String>
where
    RunPhase: FnMut(
        HookPhase,
        Vec<dory_core::ConnectionHook>,
        dory_core::HookContext,
    ) -> TestConnectionFuture<'a, HookPhaseState>,
    RunProbe: FnOnce(
        TestConnectionProbeDropGuards,
    )
        -> TestConnectionFuture<'a, Result<dory_core::TestConnectionResult, String>>,
    ...
    RunCleanup: FnOnce() -> TestConnectionFuture<'a, Result<(), String>>,
{
    ...
    match (outcome, cleanup) {
        ...
        (Ok(_), Err(cleanup_error)) => Err(dory_i18n::t!(
            "form.error.test_cleanup_failed",
            error = cleanup_error
        )),
        (Err(primary_error), Ok(())) => Err(primary_error),
        (Err(primary_error), Err(cleanup_error)) => Err(dory_i18n::t!(
        ...
        -> TestConnectionFuture<'a, Result<dory_core::TestConnectionResult, String>>,
{
    ...
    let mut warnings = match pre_connect {
        ...
        HookPhaseState::Cancelled => {
            return Err(dory_i18n::t!("form.error.test_cancelled_pre"));
        }
    };
    ...
    match post_connect {
        ...
        HookPhaseState::Cancelled => Err(dory_i18n::t!("form.error.test_cancelled_post")),
    }
}
...
fn format_detached_hook_cleanup_failure(
    ...
    profile_name: &str,
    error: &dory_ui_base::hook_phase_runner::DetachedHookCleanupError,
) -> String {
    let task_ids = error
        .task_ids()
        .iter()
...
/// secret access key directly into Dory.
fn normalize_aws_credentials_error(profile_name: &str, error: &str) -> String {
    ...
    if is_missing_credentials {
        return dory_i18n::t!("form.error.aws_credentials_missing", name = profile_name);
    }
...
mod tests {
    ...
    fn form_validation_connection_name_required_differs_between_locales() {
        let english = dory_i18n::t!("form.validation.connection_name_required", locale = "en");
        let spanish = dory_i18n::t!("form.validation.connection_name_required", locale = "es");
    ...
    fn current_unsaved_hook_context() -> dory_core::HookContext {
        dory_core::HookContext {
        ...
        let result = block_on(run_test_connection_orchestration(
            ...
            |_| {
                ...
                Box::pin(async move {
                    ...
                    Ok(dory_core::TestConnectionResult {
    ...
    fn test_connection_orchestration_cleans_up_after_cancelled_hook() {
        ...
        let expected = dory_i18n::t!("form.error.test_cancelled_pre");
        assert!(matches!(result, Err(error) if error == expected));
        assert_eq!(*cleanup_calls.lock().expect("cleanup log poisoned"), 1);
    }
    ...
    fn test_connection_orchestration_fails_successful_probe_when_cleanup_fails() {
        let result = block_on(run_test_connection_orchestration(
            ...
            |_| Box::pin(async { Ok(dory_core::TestConnectionResult::default()) }),
    ...
    fn detached_hook_cleanup_failure_keeps_scope_and_source_context() {
        ...
        let error = dory_ui_base::hook_phase_runner::DetachedHookCleanupError::new(
            vec![task_id],
