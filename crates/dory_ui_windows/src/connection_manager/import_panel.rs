...
use dory_components::controls::{Button, Checkbox, Input, InputEvent, InputState};
...
use dory_core::{AuthProfile, ConnectionProfile, ProxyProfile, SshTunnelProfile};
use dory_portability::{
...
impl ImportPersistence for AppStatePersistence<'_> {
    ...
    fn add_connection(
    ...
    ) -> dory_app::portability::ConnectionInsertResult {
        use dory_app::portability::ConnectionInsertResult;
        ...
        if let dory_core::DbConfig::External { values, .. } = &profile.config {
...
impl ImportConnectionsPanel {
    ...
    fn browse_input_path(&mut self, cx: &mut Context<Self>) {
        if dory_ui_base::file_dialog::is_native_file_dialog_available() {
            ...
            let task = cx.background_executor().spawn(async move {
                rfd::FileDialog::new()
                    .set_title(dory_i18n::t!("connection_manager.import.dialog.open_title"))
                    .add_filter(
                        dory_i18n::t!("connection_manager.import.filter.toml"),
                        &["toml"],
                    )
                    .add_filter(
                        dory_i18n::t!("connection_manager.import.filter.all"),
                        &["*"],
                    )
                    .pick_file()
            });
    ...
    fn do_parse_and_plan(&mut self, window: &mut Window, cx: &mut Context<Self>) {
        ...
        if path.is_empty() {
            self.parse_error = Some(dory_i18n::t!("connection_manager.import.error.choose_file"));
            cx.notify();
            return;
        }
        ...
        cx.spawn(async move |_this, cx| {
            let result: (bool, Result<(ParsedBundle, ImportPlan), String>) = cx
                ...
                .spawn(async move {
                    ...
                    let mut parsed = match dory_portability::import::parse(&bytes) {
                        Ok(p) => p,
                        Err(e) => {
                            return (
                                false,
                    ...
                    use dory_portability::bundle::EncryptionMode;
                    let is_encrypted =
                    ...
                    if let Err(e) = dory_portability::import::decrypt(&mut parsed, &passphrase) {
                        use dory_portability::PortabilityError;
                        let msg = if e.is_encryption_unavailable() {
                            dory_i18n::t!("connection_manager.import.error.encryption_unavailable")
                        } else if matches!(&e, PortabilityError::Decryption(_)) {
                            dory_i18n::t!("connection_manager.import.error.wrong_passphrase")
                        } else {
                    ...
                    let plan = dory_portability::import::plan(&parsed, &dest.as_ref_snapshot());
    ...
    fn do_apply(&mut self, window: &mut Window, cx: &mut Context<Self>) {
        ...
        cx.spawn(async move |_this, cx| {
            let apply_result = cx
                .background_executor()
                .spawn(async move { dory_portability::import::apply(&parsed, &plan, &choices) })
                .await;
                ...
                Ok(actions) => {
                    let outcome = app_state_entity.update(cx, |state, cx| {
                        let result = {
                            ...
                            dory_app::portability::persist_import_actions(actions, &mut deps)
                        };
...
impl Render for ImportConnectionsPanel {
    fn render(&mut self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        ModalShell::new(
            dory_i18n::t!("connection_manager.import.modal_title"),
            body,
...
impl ImportConnectionsPanel {
    fn render_select_file(&self, cx: &Context<Self>) -> AnyElement {
        ...
        let mut file_block = div()
            ...
            .gap(Spacing::XS)
            .child(
                Text::body(dory_i18n::t!("connection_manager.import.field.bundle_file"))
    ...
    fn render_preview(&self, cx: &Context<Self>) -> AnyElement {
        ...
        let mut col = div()
            ...
            .gap(Spacing::SM)
            .child(
                Text::body(dory_i18n::t!("connection_manager.import.preview.intro"))
                    .color(theme.muted_foreground),
            )
            .child(counts);
        ...
        if summary.has_driver_not_installed {
            col = col.child(BannerBlock::new(
                BannerVariant::Warning,
                dory_i18n::t!("connection_manager.import.preview.driver_not_installed_banner"),
            ));
        }
        ...
        col = col.child(
            BannerBlock::new(
                BannerVariant::Info,
                dory_i18n::t!("connection_manager.import.hint.external_refs"),
            )
            .with_body(dory_i18n::t!(
    ...
    fn render_conflicts(&self, cx: &Context<Self>) -> AnyElement {
        ...
        let mut col = div().flex().flex_col().gap(Spacing::SM).child(
            Text::body(dory_i18n::t!("connection_manager.import.conflicts.intro"))
                .color(theme.muted_foreground),
        );
    ...
    fn render_conflict_row(
        &self,
        conflict: &dory_portability::ProfileConflict,
        ...
        let mut items = vec![
            SegmentedItem::new(
                CHOICE_REUSE,
                dory_i18n::t!("connection_manager.import.action.reuse_existing"),
            ),
            SegmentedItem::new(
                CHOICE_CREATE,
                dory_i18n::t!("connection_manager.import.action.create_new"),
            ),
        ];
    ...
    fn render_required_references(&self, cx: &Context<Self>) -> AnyElement {
        ...
        let mut col = div().flex().flex_col().gap(Spacing::SM).child(
            Text::body(dory_i18n::t!("connection_manager.import.required.intro"))
                .color(theme.muted_foreground),
        );
    ...
    fn render_required_row(
        &self,
        resolution: &dory_portability::RequiredResolution,
        cx: &Context<Self>,
    ) -> AnyElement {
        let theme = cx.theme().clone();
    ...
    fn auth_choice_block(
        ...
        let mut items = vec![SegmentedItem::new(
            AUTH_SKIP,
            dory_i18n::t!("connection_manager.import.action.skip"),
        )];
    ...
    fn render_outcome(&self, cx: &Context<Self>) -> AnyElement {
        ...
        match self.run_result.as_ref() {
            None => {
                col = col.child(
                    Text::body(dory_i18n::t!("connection_manager.import.status.complete"))
            ...
            Some(ImportRunResult::Failed(msg)) => {
                col = col.child(
                    BannerBlock::new(
                        ...
                        dory_i18n::t!("connection_manager.import.banner.failed"),
                    )
                    .with_body(msg.clone()),
                );
            }
    ...
    fn render_footer(&self, cx: &Context<Self>) -> AnyElement {
        let left = match self.step {
            ...
            Step::Preview => Button::new(
                "import-back",
                dory_i18n::t!("connection_manager.import.action.back"),
            )
            .ghost()
            ...
            Step::Outcome => Button::new(
                "import-done",
                dory_i18n::t!("connection_manager.import.action.done"),
            )
            .ghost()
            .on_click(cx.listener(|_this, _: &gpui::ClickEvent, _, cx| {
        ...
        match self.step {
            Step::SelectFile => {
                ...
                let label = if self.is_parsing {
                    dory_i18n::t!("connection_manager.import.status.loading")
                } else {
                    dory_i18n::t!("connection_manager.import.action.load")
                };
                Some(
            ...
            Step::Preview => Some(
                Button::new(
                    "import-preview-next",
                    dory_i18n::t!("connection_manager.import.action.continue_"),
                )
                .primary()
                .on_click(cx.listener(|this, _: &gpui::ClickEvent, window, cx| {
            ...
            Step::RequiredReferences => {
                let label = if self.is_applying {
                    dory_i18n::t!("connection_manager.import.status.importing")
                } else {
                    dory_i18n::t!("connection_manager.import.action.import")
                };
                Some(
...
mod tests {
    ...
    /// `dory_i18n::t!`, including the plural `.one`/`.many` variants. Keys backed
