...
use dory_app::keymap::KeymapStack;
use dory_components::components::form_renderer::{self, FormRendererState};
use dory_components::components::multi_select::MultiSelect;
use dory_components::components::value_source_selector::ValueSourceSelector;
use dory_components::controls::{Dropdown, DropdownSelectionChanged};
...
use dory_ui_base::sso_wizard::SsoWizard;
use dory_ui_base::user_error::{ErrorKind, UserFacingError, report_error};
use dory_ui_base::{AppStateEntity, AuthProfileCreated};
use gpui::*;
...
use dory_components::components::form_navigation::FormEditState;
...
struct AccessState {
    ...
    ssh_tunnel_dropdown: Entity<dory_components::controls::Dropdown>,
...
pub struct ConnectionManagerWindow {
    ...
    test_result: Option<dory_core::TestConnectionResult>,
...
impl ConnectionManagerWindow {
    pub fn new(
        ...
        let input_password = cx.new(|cx| {
            InputState::new(window, cx)
                .placeholder(dory_i18n::t!("connection_manager.placeholder.password"))
                .masked(true)
        });
        ...
        let input_ssh_password = cx.new(|cx| {
            InputState::new(window, cx)
                .placeholder(dory_i18n::t!("connection_manager.placeholder.ssh_password"))
                .masked(true)
        });

        let ssl_ca_cert_input = cx.new(|cx| {
            InputState::new(window, cx)
                .placeholder(dory_i18n::t!("connection_manager.placeholder.ca_cert_path"))
        });
        ...
        let ssh_tunnel_dropdown = cx.new(|_cx| {
            Dropdown::new("ssh-tunnel-dropdown").placeholder(dory_i18n::t!(
                "connection_manager.placeholder.select_ssh_tunnel"
            ))
        });
        let proxy_dropdown = cx.new(|_cx| {
            Dropdown::new("proxy-dropdown")
                .placeholder(dory_i18n::t!("connection_manager.placeholder.select_proxy"))
        });

        let auth_profile_dropdown = cx.new(|_cx| {
            Dropdown::new("auth-profile-dropdown")
                .placeholder(dory_i18n::t!("connection_manager.placeholder.none"))
        });
        let access_method_dropdown = cx.new(|_cx| {
            Dropdown::new("access-method-dropdown")
                .placeholder(dory_i18n::t!("connection_manager.access_method.direct"))
        });
        ...
        let ssm_auth_profile_dropdown = cx.new(|_cx| {
            Dropdown::new("ssm-auth-profile-dropdown").placeholder(dory_i18n::t!(
                "connection_manager.placeholder.use_connection_auth_profile"
            ))
        });

        let conn_refresh_policy_dropdown = cx.new(|_cx| {
            Dropdown::new("conn-refresh-policy").placeholder(dory_i18n::t!(
                "connection_manager.placeholder.use_driver_default"
            ))
        });
        let conn_refresh_interval_input = cx.new(|cx| {
            InputState::new(window, cx)
                .placeholder(dory_i18n::t!("connection_manager.placeholder.seconds"))
                .default_value("5")
        });
        let conn_confirm_dangerous_dropdown = cx.new(|_cx| {
            Dropdown::new("conn-confirm-dangerous").placeholder(dory_i18n::t!(
                "connection_manager.placeholder.use_driver_default"
            ))
        });
        let conn_requires_where_dropdown = cx.new(|_cx| {
            Dropdown::new("conn-requires-where").placeholder(dory_i18n::t!(
                "connection_manager.placeholder.use_driver_default"
            ))
        });
        let conn_requires_preview_dropdown = cx.new(|_cx| {
            Dropdown::new("conn-requires-preview").placeholder(dory_i18n::t!(
                "connection_manager.placeholder.use_driver_default"
            ))
        });
        let conn_pre_hook_dropdown = cx.new(|_cx| {
            Dropdown::new("conn-pre-hook")
                .placeholder(dory_i18n::t!("connection_manager.placeholder.no_hook"))
        });
        ...
        let conn_mcp_actor_dropdown = cx.new(|_cx| {
            Dropdown::new("conn-mcp-actor").placeholder(dory_i18n::t!(
                "connection_manager.placeholder.select_trusted_client"
            ))
        });
        let conn_mcp_role_dropdown = cx.new(|_cx| {
            Dropdown::new("conn-mcp-role")
                .placeholder(dory_i18n::t!("connection_manager.placeholder.no_role"))
        });
        let conn_mcp_role_multi_select = cx.new(|_cx| {
            MultiSelect::new("conn-mcp-extra-roles").placeholder(dory_i18n::t!(
        ...
        let conn_mcp_policy_dropdown = cx.new(|_cx| {
            Dropdown::new("conn-mcp-policy")
                .placeholder(dory_i18n::t!("connection_manager.placeholder.no_policy"))
        });
        let conn_mcp_policy_multi_select = cx.new(|_cx| {
            MultiSelect::new("conn-mcp-extra-policies").placeholder(dory_i18n::t!(
        ...
        Self {
            ...
            ssh_test_error: None,
            focus_handle,
            keymap: dory_ui_base::keymap::default_keymap(),
    ...
    pub fn new_for_edit(
        ...
        if let Some(ssh) = profile.config.ssh_tunnel() {
            ...
            match &ssh.auth_method {
                ...
                dory_core::SshAuthMethod::Password => {
    ...
    fn load_mcp_dropdowns(
        &mut self,
        binding: Option<&dory_core::ConnectionMcpPolicyBinding>,
        _window: &mut Window,
        cx: &mut Context<Self>,
    ) {
        let clients = self
        ...
        let actor_items: Vec<dory_components::controls::DropdownItem> = clients
            .iter()
            .map(|c| {
                dory_components::controls::DropdownItem::with_value(
                    format!("{} ({})", c.name, c.id),
                    c.id.clone(),
                )
            })
        ...
        let mut role_items = vec![dory_components::controls::DropdownItem::with_value(
            dory_i18n::t!("connection_manager.placeholder.no_role"),
            "",
        )];
        role_items.extend(roles.iter().map(|r| {
            let label = dory_mcp::builtin_display_name(&r.id)
                .map(|name| format!("{} (built-in)", name))
                .unwrap_or_else(|| r.id.clone());
            dory_components::controls::DropdownItem::with_value(label, r.id.clone())
        }));

        let mut policy_items = vec![dory_components::controls::DropdownItem::with_value(
            dory_i18n::t!("connection_manager.placeholder.no_policy"),
            "",
        )];
        policy_items.extend(policies.iter().map(|p| {
            let label = dory_mcp::builtin_display_name(&p.id)
                .map(|name| format!("{} (built-in)", name))
                .unwrap_or_else(|| p.id.clone());
            dory_components::controls::DropdownItem::with_value(label, p.id.clone())
        }));
        ...
        let all_role_items: Vec<dory_components::controls::DropdownItem> = roles
            .iter()
            .map(|r| {
                let label = dory_mcp::builtin_display_name(&r.id)
                    .map(|name| format!("{} (built-in)", name))
        ...
        let all_policy_items: Vec<dory_components::controls::DropdownItem> = policies
            .iter()
            .map(|p| {
                let label = dory_mcp::builtin_display_name(&p.id)
                    .map(|name| format!("{} (built-in)", name))
    ...
    fn load_settings_tab(
        ...
        connection_settings: Option<&dory_core::FormValues>,
        ...
        let policy_index = match overrides.refresh_policy.unwrap_or(effective.refresh_policy) {
            dory_core::RefreshPolicySetting::Manual => 0,
            dory_core::RefreshPolicySetting::Interval => 1,
        };
        self.settings_tab
            .conn_refresh_policy_dropdown
        ...
        let boolean_items = vec![
            dory_components::controls::DropdownItem::with_value(
                dory_i18n::t!("connection_manager.placeholder.use_driver_default"),
                "default",
            ),
            dory_components::controls::DropdownItem::with_value(
        ...
        let mut hook_items = vec![dory_components::controls::DropdownItem::with_value(
            dory_i18n::t!("connection_manager.placeholder.no_hook"),
            "",
        )];
        ...
        hook_items.extend(hook_names.iter().filter_map(|hook_name| {
            ...
            Some(dory_components::controls::DropdownItem::with_value(
                label,
                definition_id,
            ))
        }));
    ...
    fn resolve_driver_effective_settings(
        &self,
        cx: &Context<Self>,
    ) -> dory_core::EffectiveSettings {
        let state = self.app_state.read(cx);
        if let Some(driver) = &self.form.selected_driver {
            state.effective_settings(&driver.driver_key())
        } else {
            let empty = dory_core::FormValues::new();
            dory_core::EffectiveSettings::resolve(
    ...
    fn collect_connection_settings(&self, cx: &Context<Self>) -> Option<dory_core::FormValues> {
    ...
    fn populate_auth_profile_dropdown(&mut self, cx: &mut Context<Self>) {
        ...
        let mut auth_items = vec![dory_components::controls::DropdownItem::with_value(
            dory_i18n::t!("connection_manager.placeholder.none"),
            "",
        )];
        let mut ssm_items = vec![dory_components::controls::DropdownItem::with_value(
            dory_i18n::t!("connection_manager.placeholder.use_connection_auth_profile"),
            "",
        )];
        ...
        for profile in &profiles {
            ...
            auth_items.push(dory_components::controls::DropdownItem::with_value(
                label,
                profile.id.to_string(),
            ));
            let ssm_label = format!("{} [{}]", profile.name, session_status);
            ssm_items.push(dory_components::controls::DropdownItem::with_value(
                ssm_label,
                profile.id.to_string(),
            ));
        ...
        auth_items.push(dory_components::controls::DropdownItem::with_value(
            dory_i18n::t!("connection_manager.new_auth_profile"),
    ...
    fn open_sso_wizard(&mut self, cx: &mut Context<Self>) {
        ...
        let mut options = WindowOptions {
            ...
            titlebar: Some(TitlebarOptions {
                title: Some(dory_i18n::t!("connection_manager.aws_sso_wizard_title").into()),
                ..Default::default()
            }),
        ...
        if let Err(error) = cx.open_window(options, move |window, cx| {
            ...
            report_error(
                UserFacingError::new(
                    ErrorKind::User,
                    dory_i18n::t!("connection_manager.aws_sso_open_failed"),
                )
        ...
        if self.auth_profile.pending_wizard_auth_profile_selection {
            ...
            if let Some(profile_id) = newest {
                ...
                self.auth_profile.auth_profile_action_message =
                    Some(dory_i18n::t!("connection_manager.auth.profile_created_sso"));
            }
    ...
    fn handle_auth_profile_created(&mut self, profile_id: Uuid, cx: &mut Context<Self>) {
        ...
        self.auth_profile.pending_wizard_auth_profile_selection = false;
        self.auth_profile.auth_profile_action_message = Some(dory_i18n::t!(
    ...
    fn refresh_auth_profile_statuses(&mut self, cx: &mut Context<Self>) {
        self.auth_profile.auth_profile_action_message =
            Some(dory_i18n::t!("connection_manager.auth.refreshing_sessions"));
        self.refresh_auth_profile_sessions(cx);
        cx.notify();
    }
    ...
    fn login_selected_auth_profile(&mut self, cx: &mut Context<Self>) {
        let Some(profile) = self.selected_auth_profile(cx) else {
            self.auth_profile.auth_profile_action_message =
                Some(dory_i18n::t!("connection_manager.auth.select_before_login"));
            cx.notify();
            return;
        };
                ...
                .update(|cx| {
                    this.update(cx, |this, cx| {
                        ...
                        this.auth_profile.auth_profile_action_message = Some(match result {
                            Ok(_) => {
                                dory_i18n::t!("connection_manager.auth.login_completed")
                            }
    ...
    fn selected_auth_profile_status_text(&self, cx: &App) -> Option<String> {
        ...
        let text = match status {
            AuthSessionState::Valid { expires_at } => {
                ...
                dory_i18n::t!("connection_manager.auth.session_status_valid")
            }
            AuthSessionState::Expired => {
                dory_i18n::t!("connection_manager.auth.session_status_expired")
            }
            AuthSessionState::LoginRequired => {
                dory_i18n::t!("connection_manager.auth.session_status_login_required")
            }
        };
    ...
    pub(super) fn apply_proxy(&mut self, proxy: &dory_core::ProxyProfile, _cx: &mut Context<Self>) {
    ...
    fn effective_ssh_test_target(
        &self,
        cx: &Context<Self>,
    ) -> Option<(dory_core::SshTunnelConfig, Option<String>)> {
    ...
    pub(super) fn test_ssh_connection(&mut self, _window: &mut Window, cx: &mut Context<Self>) {
        ...
        let Some((ssh_config, ssh_secret)) = self.effective_ssh_test_target(cx) else {
            ...
            self.ssh_test_error = Some(dory_i18n::t!(
        ...
        let task = cx.background_executor().spawn(async move {
            match dory_ssh::establish_session(&ssh_config, ssh_secret.as_deref()) {
                Ok(_session) => Ok(()),
                Err(e) => Err(format!("{:?}", e)),
            }
        });
    ...
    pub(super) fn browse_ssh_key(&mut self, _window: &mut Window, cx: &mut Context<Self>) {
        ...
        let task = cx.background_executor().spawn(async move {
            let dialog = rfd::FileDialog::new()
                .set_title(dory_i18n::t!("connection_manager.select_ssh_key_title"))
    ...
    pub(super) fn browse_ssl_cert(
        ...
        let title = match slot {
            SslCertSlot::CaCert => dory_i18n::t!("connection_manager.select_ca_cert"),
            SslCertSlot::ClientCert => dory_i18n::t!("connection_manager.select_client_cert"),
            SslCertSlot::ClientKey => dory_i18n::t!("connection_manager.select_client_key"),
        };
        ...
        let task = cx.background_executor().spawn(async move {
            let dialog = rfd::FileDialog::new()
                ...
                .set_directory(&start_dir)
                .add_filter(
                    dory_i18n::t!("connection_manager.filter_certificates"),
                    &["pem", "crt", "cer", "key", "der"],
                )
                .add_filter(dory_i18n::t!("connection_manager.filter_all_files"), &["*"]);
    ...
    pub(super) fn browse_file_path(&mut self, _window: &mut Window, cx: &mut Context<Self>) {
        ...
        let task = cx.background_executor().spawn(async move {
            let dialog = rfd::FileDialog::new()
                .set_title(dory_i18n::t!("connection_manager.select_database_file"))
                .set_directory(&start_dir);
            dialog.pick_file()
        });
    ...
    fn populate_access_method_dropdown(&mut self, cx: &mut Context<Self>) {
        let items = vec![
            dory_components::controls::DropdownItem::with_value(
                dory_i18n::t!("connection_manager.access_method.direct"),
                "direct",
            ),
            dory_components::controls::DropdownItem::with_value(
                dory_i18n::t!("connection_manager.access_method.ssh_tunnel"),
                "ssh",
            ),
            dory_components::controls::DropdownItem::with_value(
                dory_i18n::t!("connection_manager.access_method.proxy"),
                "proxy",
            ),
            dory_components::controls::DropdownItem::with_value(
                dory_i18n::t!("connection_manager.access_method.ssm"),
                "ssm",
            ),
        ];
...
mod tests {
    ...
    use dory_core::AuthSessionState;
    use std::collections::{HashMap, HashSet};
    ...
    fn form_has_auth_profile_ref_field_detects_kind() {
        use dory_core::{DriverFormDef, FormFieldDef, FormFieldKind, FormSection, FormTab};
    ...
    fn connection_manager_access_method_differs_between_locales() {
        let en = dory_i18n::t!("connection_manager.access_method.ssh_tunnel", locale = "en");
        let es = dory_i18n::t!("connection_manager.access_method.ssh_tunnel", locale = "es");
    ...
    fn connection_manager_new_auth_profile_exact_values() {
        let en = dory_i18n::t!("connection_manager.new_auth_profile", locale = "en");
        let es = dory_i18n::t!("connection_manager.new_auth_profile", locale = "es");
    ...
    fn access_method_dropdown_item_value_ids_stay_untranslated() {
        for locale in ["en", "es"] {
            let label = dory_i18n::t!(
                "connection_manager.access_method.ssh_tunnel",
                locale = locale
            );
            let item = dory_components::controls::DropdownItem::with_value(label, "ssh");
    ...
    fn connection_manager_auth_login_completed_differs_between_locales() {
        let en = dory_i18n::t!("connection_manager.auth.login_completed", locale = "en");
        let es = dory_i18n::t!("connection_manager.auth.login_completed", locale = "es");
    ...
    fn connection_manager_window_title_differs_between_locales() {
        let en = dory_i18n::t!("connection_manager.window_title", locale = "en");
        let es = dory_i18n::t!("connection_manager.window_title", locale = "es");
    ...
    fn connection_manager_banner_connection_failed_differs_between_locales() {
        let en = dory_i18n::t!("connection_manager.banner.connection_failed", locale = "en");
        let es = dory_i18n::t!("connection_manager.banner.connection_failed", locale = "es");
