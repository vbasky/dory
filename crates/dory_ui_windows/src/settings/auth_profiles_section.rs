...
use dory_components::primitives::{Icon as FluxIcon, Label, Text};
...
struct CachedOptions {
    options: Vec<dory_core::SelectOption>,
    expires_at: Instant,
}
...
fn mirror_label_fallback() -> String {
    dory_i18n::t!("settings.auth_profiles.mirror_label_fallback")
}
fn success_written_fallback() -> String {
    dory_i18n::t!("settings.auth_profiles.saved_fallback")
}
const NAME_HINT_FALLBACK: &str = "";

fn dangling_fallback() -> DanglingMessage {
    DanglingMessage {
        title: dory_i18n::t!("settings.auth_profiles.reference_invalid_title"),
        body: dory_i18n::t!("settings.auth_profiles.reference_invalid_body"),
    }
}
...
/// before saving again.
fn resolve_conflict_message(target: &dory_core::AuthEditTarget) -> String {
...
fn resolve_partial_saved_message(
    written: &dory_core::AuthEditTarget,
    conflicted: &dory_core::AuthEditTarget,
) -> String {
    ...
    match &error {
        FetchOptionsError::NeedsLogin => {
            field_login_hint.insert(
                field_id.to_string(),
                dory_i18n::t!("settings.auth_profiles.login_hint"),
            );

            if !login_in_progress {
                *provider_login_status = Some((
                    dory_i18n::t!("settings.auth_profiles.login_required_hint"),
                    false,
                ));
            }
        }
        FetchOptionsError::SessionExpired => {
            field_login_hint.insert(
                field_id.to_string(),
                dory_i18n::t!("settings.auth_profiles.session_expired_hint"),
            );

            if !login_in_progress {
                *provider_login_status = Some((
                    dory_i18n::t!("settings.auth_profiles.session_expired_click_hint"),
                    false,
                ));
            }
        }
...
impl AuthProfilesSection {
    pub(super) fn new(
        ...
        let input_name = cx.new(|cx| {
            InputState::new(window, cx)
                .placeholder(dory_i18n::t!("settings.auth_profiles.placeholder_name"))
        });

        let provider_dropdown = cx.new(|_cx| {
            Dropdown::new(SharedString::from("auth-provider-selector")).placeholder(dory_i18n::t!(
    ...
    fn selected_provider(&self, cx: &App) -> Option<Arc<dyn dory_core::DynAuthProvider>> {
                        ...
                        let placeholder_str = if placeholder.is_empty() {
                            match &kind {
                                FormFieldKind::AuthProfileRef { .. } => {
                                    dory_i18n::t!("settings.auth_profiles.select_none")
                                }
                                _ => dory_i18n::t!("settings.auth_profiles.select_ellipsis"),
                            }
                        } else {
                            placeholder
                        };
    ...
    fn fetch_dynamic_options_if_needed(
        ...
        let profile_snapshot = dory_core::auth::expand_auth_profile_refs(
            &raw_snapshot,
            provider.form_def(),
            &|target_id| {
    ...
    fn render_dynamic_dropdown_row(
        ...
        if !self.dynamic_dropdowns.contains_key(&field_id) {
            let placeholder = if field.placeholder.is_empty() {
                dory_i18n::t!("settings.auth_profiles.select_ellipsis")
            } else {
                field.placeholder.clone()
            };
            let dropdown_id = format!("auth-dynamic-{}", field_id);
        ...
        div()
            ...
            .flex_col()
            .gap_1()
            .child(dory_components::primitives::Label::new(label))
            .child(dropdown)
    ...
    fn render_auth_profile_ref_row(
        ...
        if !self.dynamic_dropdowns.contains_key(&field_id) {
            ...
            let dropdown = cx.new(|_cx| {
                Dropdown::new(SharedString::from(dropdown_id))
                    .placeholder(dory_i18n::t!("settings.auth_profiles.select_none"))
            });
            self.dynamic_dropdowns.insert(field_id.clone(), dropdown);
        }
    ...
    fn save_profile(&mut self, window: &mut Window, cx: &mut Context<Self>) {
        ...
        // instead of Dory's SQLite store. The provider returns `Some(result)`
        ...
        if has_secret_fields && !secrets_persisted {
            report_error(
                UserFacingError::new(
                    ErrorKind::Storage,
                    dory_i18n::t!("settings.auth_profiles.error.secret_store_failed"),
                ),
                cx,
            );
        }
    ...
    fn save_edit_profile(
        ...
        let raw_profile = AuthProfile {
            ...
            // writes them to the external config file), never to Dory storage.
            secret_fields: HashMap::new(),
            enabled: self.profile_enabled,
            read_only: false,
            dangling_origin: None,
        };
        let fields = dory_core::auth::expand_auth_profile_refs(
            &raw_profile,
            provider.form_def(),
            &|target_id| {
                registry_snapshot
    ...
    fn login_selected_profile(&mut self, cx: &mut Context<Self>) {
        let Some(provider) = self.selected_provider(cx) else {
            self.provider_login_status = Some((
                dory_i18n::t!("settings.auth_profiles.select_before_login"),
                false,
            ));
            cx.notify();
            return;
        ...
        if !provider.capabilities().login.supported {
            self.provider_login_status = Some((
                dory_i18n::t!("settings.auth_profiles.interactive_login_unavailable"),
                false,
            ));
            cx.notify();
            return;
        ...
        let Some(raw_profile) = self.current_form_profile(cx) else {
            self.provider_login_status = Some((
                dory_i18n::t!("settings.auth_profiles.fields_required_before_login"),
                false,
            ));
            cx.notify();
            return;
        ...
        let profile_registry_snapshot: Vec<AuthProfile> =
            self.app_state.read(cx).list_auth_profiles();
        let profile = dory_core::auth::expand_auth_profile_refs(
            &raw_profile,
            provider.form_def(),
            &|target_id| {
                profile_registry_snapshot
        ...
        let url_callback: dory_core::auth::UrlCallback = Box::new(move |url| {
            ...
            .child(
                div()
                    ...
                    .child(
                        Button::new(
                            "auth-login-open-url",
                            dory_i18n::t!("settings.auth_profiles.open_browser"),
                        )
                        .small()
                        .primary()
                    ...
                    .child(
                        Button::new(
                            "auth-login-copy-url",
                            dory_i18n::t!("settings.auth_profiles.copy_url"),
                        )
                        .small()
                        .on_click(cx.listener(move |_this, _, _, cx| {
                    ...
                    .child(
                        Button::new(
                            "auth-login-cancel",
                            dory_i18n::t!("settings.auth_profiles.cancel"),
                        )
                        .small()
                        .danger()
                        .on_click(cx.listener(|this, _, _, cx| {
                            ...
                            this.provider_login_status = Some((
                                dory_i18n::t!("settings.auth_profiles.login_cancelled"),
                                false,
                            ));
                            cx.notify();
                        })),
            ...
            .child(
                div()
                    ...
                    .child(
                        Button::new(
                            "new-auth-profile",
                            dory_i18n::t!("settings.auth_profiles.new_profile"),
                        )
                    ...
                    .child(
                        Button::new(
                            "import-auth-profile",
                            dory_i18n::t!("settings.auth_profiles.import"),
                        )
    ...
    fn render_editor_panel(&mut self, window: &mut Window, cx: &mut Context<Self>) -> AnyElement {
        ...
        let field_defs_for_fetch: Vec<dory_core::FormFieldDef> = self
            .selected_provider(cx)
            .map(|provider| {
                provider
                    .form_def()
        ...
        // from a prior `aws sso login` or Dory run see options without
        ...
        let provider_field_defs: Vec<dory_core::FormFieldDef> = field_defs_for_fetch;
                    ...
                    let extra_hint: Option<String> =
                        if field.kind == FormFieldKind::WriteOnly && !disabled {
                            Some(field.help.clone().unwrap_or_else(|| {
                                dory_i18n::t!("settings.auth_profiles.leave_blank_hint")
                            }))
                        } else {
                            None
                        };
        ...
        layout::sticky_form_shell(
            div()
                .child(Label::new(layout::editor_panel_title(
                    &dory_i18n::t!("settings.auth_profiles.auth_profile_label"),
                    is_editing,
                )))
            ...
            div()
                        ...
                        div()
                            ...
                            .when(show_reload, |panel| {
                                panel.child(
                                    Button::new(
                                        "edit-reload-profile",
                                        dory_i18n::t!("settings.auth_profiles.reload"),
                                    )
                                    .small()
                ...
                .child({
                    ...
                    let name_label = dory_i18n::t!("settings.auth_profiles.name_label");
                                ...
                                .child(
                                    Button::new(
                                        ...
                                        if self.provider_login_loading {
                                            dory_i18n::t!("settings.auth_profiles.logging_in")
                                        } else {
                                            dory_i18n::t!("settings.auth_profiles.login_action")
                                        },
                                    )
                                    .small()
                                    .primary()
                ...
                .when(self.edit_snapshot.is_none(), |content| {
                    ...
                    content.child(
                        div()
                            ...
                            .child(Text::body(dory_i18n::t!("settings.auth_profiles.enabled"))),
                    )
                }),
            None,
            &theme,
    ...
    fn render_section_footer_actions(&self, cx: &mut Context<Self>) -> AnyElement {
        ...
        // deleted from Dory (the file section is the source of truth).
        let is_reflected = self.edit_snapshot.is_some();
        ...
        div()
            ...
            .when(is_editing && !is_reflected, |root| {
                root.child(layout::footer_action_frame(
                    ...
                    Button::new(
                        "export-auth-profile",
                        dory_i18n::t!("settings.auth_profiles.export"),
                    )
                    .small()
                    .ghost()
                    .w_full()
                ...
                .child(layout::footer_action_frame(
                    ...
                    Button::new(
                        "delete-auth-profile",
                        dory_i18n::t!("settings.auth_profiles.delete"),
                    )
                    .small()
                    .danger()
                    .w_full()
            ...
            .child(layout::footer_action_frame(
                ...
                Button::new(
                    "save-auth-profile",
                    if is_reflected {
                        dory_i18n::t!("settings.auth_profiles.save_to_file")
                    } else if is_editing {
                        dory_i18n::t!("settings.auth_profiles.update")
                    } else {
                        dory_i18n::t!("settings.auth_profiles.create")
                    },
                )
                .small()
                .primary()
        ...
        layout::section_container(
            layout::split_section_shell(
                dory_components::composites::section_header(
                    ...
                    dory_i18n::t!("settings.auth_profiles.section_description"),
                    cx,
                ),
            ...
            .when(show_delete_dialog, |element| {
                ...
                element.child(
                    Dialog::new(window, cx)
                        .title(dory_i18n::t!("settings.auth_profiles.delete_dialog_title"))
                        .confirm()
...
mod tests {
    ...
    fn dangling_keyring_only_message_references_credentials_file() {
        let msg = format!(
            "Auth profile '{}' is only in the Dory keyring and no longer has a \
    ...
    /// state is managed by the AWS file, not Dory.
    #[::core::prelude::v1::test]
    ...
    /// This is also tested in dory_core, but we re-verify at the UI boundary
    /// where the type is matched and messages are constructed.
    #[::core::prelude::v1::test]
    fn auth_save_outcome_debug_has_no_secrets() {
        use dory_core::{AuthEditTarget, AuthSaveOutcome};
    ...
    fn resolve_success_text_reads_from_edit_caps() {
        use dory_core::AuthEditCapabilities;
    ...
    fn resolve_dangling_returns_caps_message_for_known_origin() {
        use dory_core::{AuthEditCapabilities, DanglingMessage};
        use std::collections::HashMap;
    ...
    fn auth_profiles_section_title_differs_between_locales() {
        let en = dory_i18n::t!("settings.auth_profiles.section_title", locale = "en");
        let es = dory_i18n::t!("settings.auth_profiles.section_title", locale = "es");
    ...
    fn auth_profiles_new_profile_exact_values() {
        let en = dory_i18n::t!("settings.auth_profiles.new_profile", locale = "en");
        let es = dory_i18n::t!("settings.auth_profiles.new_profile", locale = "es");
    ...
    fn auth_profiles_login_action_differs_between_locales() {
        let en = dory_i18n::t!("settings.auth_profiles.login_action", locale = "en");
        let es = dory_i18n::t!("settings.auth_profiles.login_action", locale = "es");
