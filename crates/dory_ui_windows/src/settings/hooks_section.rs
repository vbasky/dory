...
use dory_core::{HookExecutionMode, ScriptLanguage};
...
impl HookEditorSelectionState {
    ...
    pub(super) fn from_saved_hook(kind: &dory_core::HookKind, mode: HookExecutionMode) -> Self {
        let kind = match kind {
            dory_core::HookKind::Command { .. } => HookKindSelection::Command,
            dory_core::HookKind::Script { .. } => HookKindSelection::Script,
            dory_core::HookKind::Lua { .. } => HookKindSelection::Lua,
        };
...
impl HooksSection {
    pub(super) fn new(
        ...
        let hook_kind_dropdown = cx.new(|_cx| {
            #[cfg(feature = "lua")]
            let items = vec![
                DropdownItem::with_value(dory_i18n::t!("hooks.kind.command"), "command"),
                DropdownItem::with_value(dory_i18n::t!("hooks.kind.script"), "script"),
        ...
        let hook_execution_mode_dropdown = cx.new(|_cx| {
            Dropdown::new("hook-execution-mode")
                .items(vec![
                    DropdownItem::with_value(dory_i18n::t!("hooks.execution.blocking"), "blocking"),
                    DropdownItem::with_value(dory_i18n::t!("hooks.execution.detached"), "detached"),
                ])
                .selected_index(Some(0))
        });
        let input_hook_ready_signal =
            cx.new(|cx| InputState::new(window, cx).placeholder("DORY_READY"));
        let input_hook_cwd =
            ...
            Dropdown::new("hook-failure-mode")
                .items(vec![
                    DropdownItem::with_value(
                        dory_i18n::t!("hooks.failure.disconnect"),
                        "disconnect",
                    ),
                    DropdownItem::with_value(dory_i18n::t!("hooks.failure.warn"), "warn"),
                    DropdownItem::with_value(dory_i18n::t!("hooks.failure.ignore"), "ignore"),
                ])
                .selected_index(Some(0))
        });
    ...
    fn render(&mut self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        div()
            ...
            .when(show_hook_delete, |element| {
                ...
                element.child(
                    Dialog::new(window, cx)
                        .title(dory_i18n::t!("hooks.delete.title"))
                        .confirm()
            ...
            .when(show_protected_delete, |element| {
                ...
                element.child(
                    Dialog::new(window, cx)
                        .title(dory_i18n::t!("hooks.delete_unreadable.title"))
                        .confirm()
                        .on_ok(move |_, _, cx| {
                            entity.update(cx, |section, cx| {
...
mod tests {
    ...
    use dory_core::{HookExecutionMode, HookKind, ScriptLanguage, ScriptSource};
    ...
    fn lua_selection_round_trips_and_refreshes_visible_rows() {
        ...
        let reopened = HookEditorSelectionState::from_saved_hook(
            &HookKind::Lua {
                ...
                capabilities: dory_core::LuaCapabilities::default(),
            },
            saved.execution_mode,
        );
    ...
    fn hooks_failure_warn_differs_between_locales() {
        let english = dory_i18n::t!("hooks.failure.warn", locale = "en");
        let spanish = dory_i18n::t!("hooks.failure.warn", locale = "es");
    ...
    fn hooks_kind_script_is_script_in_both_locales() {
        let english = dory_i18n::t!("hooks.kind.script", locale = "en");
        let spanish = dory_i18n::t!("hooks.kind.script", locale = "es");
