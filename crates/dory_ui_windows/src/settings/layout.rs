...
pub(super) fn editor_panel_title(noun: &str, is_editing: bool) -> String {
    if is_editing {
        dory_i18n::t!("settings.editor_panel.title.edit", noun = noun)
    } else {
        dory_i18n::t!("settings.editor_panel.title.new", noun = noun)
    }
}
...
mod tests {
    ...
    fn editor_panel_title_uses_the_translated_catalog_templates() {
        assert_eq!(
            ...
            dory_i18n::t!("settings.editor_panel.title.new", noun = "Proxy")
        );
        assert_eq!(
            editor_panel_title("Proxy", true),
            dory_i18n::t!("settings.editor_panel.title.edit", noun = "Proxy")
        );
    }
    ...
    fn representative_settings_sections_call_the_canonical_section_header_directly() {
        for (file_name, source) in representative_settings_section_sources() {
            assert!(
                source.contains("dory_components::composites::section_header("),
