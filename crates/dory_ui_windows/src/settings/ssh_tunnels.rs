...
use dory_core::{SshAuthMethod, SshTunnelProfile};
...
impl SshTunnelsSection {
    ...
    pub(super) fn save_tunnel(&mut self, window: &mut Window, cx: &mut Context<Self>) {
        ...
        if let Err(e) = dory_app::config_loader::save_ssh_tunnels(
            runtime,
            ...
            report_error(
                UserFacingError::new(
                    ErrorKind::Storage,
                    dory_i18n::t!("settings.ssh_tunnels.error.save_failed", error = e),
                ),
                cx,
            );
        }
    ...
    pub(super) fn test_ssh_tunnel(&mut self, cx: &mut Context<Self>) {
        ...
        let task = cx.background_executor().spawn(async move {
            match dory_ssh::establish_session(&config, secret.as_deref()) {
                Ok(_session) => Ok(()),
