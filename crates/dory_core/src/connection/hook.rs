...
impl ConnectionHook {
    ...
    fn context_env(&self, context: &HookContext) -> HashMap<String, String> {
        ...
        environment.insert(
            "DORY_PROFILE_ID".to_string(),
            context.profile_id.to_string(),
        );
        environment.insert(
            "DORY_PROFILE_NAME".to_string(),
            context.profile_name.clone(),
        );
        environment.insert("DORY_DB_KIND".to_string(), context.db_kind.clone());

        if let Some(host) = &context.host {
            environment.insert("DORY_HOST".to_string(), host.clone());
        }

        if let Some(port) = context.port {
            environment.insert("DORY_PORT".to_string(), port.to_string());
        }

        if let Some(database) = &context.database {
            environment.insert("DORY_DATABASE".to_string(), database.clone());
        }
...
mod tests {
    ...
    fn execute_injects_context_env_vars() {
        let hook = ConnectionHook {
            kind: HookKind::Command {
                ...
                args: vec![
                    "-c".to_string(),
                    "echo $DORY_PROFILE_NAME:$DORY_HOST:$DORY_PORT:$DORY_DATABASE".to_string(),
                ],
            },
            ..echo_hook("")
        };
    ...
    fn execute_custom_env_overrides_context() {
        let hook = ConnectionHook {
            kind: HookKind::Command {
                command: "sh".to_string(),
                args: vec!["-c".to_string(), "echo $DORY_HOST".to_string()],
            },
            env: HashMap::from([("DORY_HOST".to_string(), "override-host".to_string())]),
            ..echo_hook("")
        };
