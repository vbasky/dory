...
/// can be imported into Dory as an `AuthProfile`.
...
pub trait DynAuthProvider: Send + Sync {
    ...
    /// that have not yet been imported into Dory.
    ///
    ...
    /// normally in Dory (the UI then falls back to `add_auth_profile`).
    ///
    ...
    /// a safe no-op for providers that store profiles in Dory's SQLite store.
    fn save_edit(
        &self,
        _name: &str,
