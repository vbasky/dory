//! Column metadata shared by the data-transfer engine (`dory_transfer`) and
//! by [`crate::query::generator::CreateTableSpec`].
//!
//! Lives in `dory_core` rather than `dory_transfer` because
//! `CreateTableSpec` is part of the `QueryGenerator` trait (implemented by
//! every SQL driver, which depends on `dory_core`, not the reverse).
...
/// (`dory_transfer::manifest`), letting Import recreate a table without
