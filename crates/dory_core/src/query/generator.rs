...
impl<'a> SqlSelectBuilder<'a> {
    ...
    /// Builds a `SELECT COUNT(*) FROM (<grouped query without LIMIT/OFFSET>) AS _dory_count_subq`.
    ///
    ...
    pub(crate) fn build_count_of_grouped(
        ...
        let subq_alias = self.dialect.quote_identifier("_dory_count_subq");
        Ok(SelectQuery {
...
mod tests {
    ...
    fn grouped_count_subquery_alias_is_quoted() {
        ...
        assert!(
            q.sql.contains("\"_dory_count_subq\""),
