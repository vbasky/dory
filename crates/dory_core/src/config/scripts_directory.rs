...
/// Manages the centralized scripts directory at `~/.local/share/dory/scripts/`.
///
...
impl ScriptsDirectory {
    pub fn new() -> Result<Self, DbError> {
        ...
        let root = data_dir.join("dory").join("scripts");
...
mod tests {
    ...
    fn test_hook_script_path_uses_language_extension() {
        let hooks_dir = PathBuf::from("/tmp/dory-hooks");
