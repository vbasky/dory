...
pub struct RiskedChange {
    ...
    pub risk: dory_policy::ExecutionClassification,
}
...
/// `dory_policy`. `PrimaryKeyChanged` and `ForeignKeyChanged` are constraint
/// mutations under the hood (DROP/ADD CONSTRAINT), so both classify as
/// `AddConstraint`.
fn classify_schema_change(change: &SchemaChange) -> dory_policy::ExecutionClassification {
    use dory_policy::{SchemaAlterKind, classify_schema_alter};
...
/// `dory_policy` governance ladder as `classify_schema_change` for
/// column/index changes. Exposed publicly (unlike `classify_schema_change`)
/// so callers outside `dory_core` — which may not depend on `dory_policy`
/// directly, e.g. non-`mcp` builds of `dory_ui_document` — can classify
/// `TableChange::TableAdded`/`TableRemoved` without a direct `dory_policy`
/// dependency.
pub fn classify_table_added() -> dory_policy::ExecutionClassification {
    use dory_policy::{SchemaAlterKind, classify_schema_alter};
...
pub fn classify_table_removed() -> dory_policy::ExecutionClassification {
    use dory_policy::{SchemaAlterKind, classify_schema_alter};
