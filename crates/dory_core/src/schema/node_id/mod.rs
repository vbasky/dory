...
mod tests {
    ...
    fn schema_node_kind_flags_remain_stable_for_representative_nodes() {
        ...
        assert!(SchemaNodeKind::ConnectionFolder.offers_context_menu());
        ...
        assert!(!SchemaNodeKind::Column.offers_context_menu());
        assert!(!SchemaNodeKind::Schema.offers_context_menu());
        assert!(SchemaNodeKind::Database.offers_context_menu());
        assert!(SchemaNodeKind::InstanceOverviewLeaf.offers_context_menu());
        assert!(SchemaNodeKind::InstanceMetricsFolder.offers_context_menu());
        assert!(SchemaNodeKind::InstanceInspectorsFolder.offers_context_menu());
    }

    #[test]
    fn pipe_preserving_paths_round_trip_for_script_nodes() {
        roundtrip(SchemaNodeId::ScriptsFolder {
            path: Some("/tmp/dory|scripts|archive".into()),
        });
        roundtrip(SchemaNodeId::ScriptFile {
            path: "/tmp/dory|scripts|query.sql".into(),
        });
    }
