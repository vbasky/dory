use dory_core::{ColumnKind, ColumnMeta, DefaultSqlDialect, DriverKey, KeySetRequest};
use dory_core::{
...
impl dory_core::Connection for PlannedExecutionConnection {
    fn metadata(&self) -> &dory_core::DriverMetadata {
        static METADATA: std::sync::LazyLock<dory_core::DriverMetadata> =
            std::sync::LazyLock::new(|| dory_core::DriverMetadata {
    ...
    fn execute(&self, req: &dory_core::QueryRequest) -> Result<QueryResult, dory_core::DbError> {
    ...
    ) -> Result<dory_core::SemanticPlan, dory_core::DbError> {
        Ok(dory_core::SemanticPlan::single_query(
            dory_core::SemanticPlanKind::MutationPreview,
