...
use dory_core::{Connection, DbError, KeyValueApi};
...
pub fn dispatch(conn: &dyn Connection, body: DriverRequestBody) -> DriverResponseBody {
    match body {
        ...
        DriverRequestBody::Cancel { handle_id } => {
            let handle = dory_core::QueryHandle { id: handle_id };
...
fn db_error_to_response(err: DbError) -> DriverResponseBody {
    ...
    DriverResponseBody::Error(dory_ipc::driver_protocol::DriverRpcError {
        code,
...
mod tests {
    ...
    use dory_ipc::driver_protocol::{DriverRequestBody, DriverResponseBody};
    ...
    fn dispatch_returns_semantic_plan_response() {
        let plan = SemanticPlan::single_query(
            ...
            dory_core::PlannedQuery::new(QueryLanguage::Sql, "SELECT 1"),
        );
