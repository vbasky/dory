...
/// profiles exist without ever loading secret material into Dory memory.
...
///
/// Dory mutates the config from several concurrent contexts: dropdown
...
fn create_backup_for_path(path: &std::path::Path) -> Result<PathBuf, io::Error> {
    let base = path.with_extension("dory-backup");
    let backup_path = if base.exists() {
        let timestamp = chrono::Utc::now().timestamp();
        let file_name = format!("config.dory-backup.{}", timestamp);
...
fn restore_backup_for_path(path: &std::path::Path) -> Result<(), io::Error> {
    let default_backup = path.with_extension("dory-backup");
    ...
    for entry in fs::read_dir(parent)? {
        ...
        if !name.starts_with("config.dory-backup.") {
            continue;
        }
...
/// `~/.aws/credentials`, which Dory does not manage.
