...
//!
//! These types are private to `dory_aws` — they implement the
...
impl AwsEditFileKind {
    ...
    pub(crate) fn to_target(self) -> dory_core::auth::AuthEditTarget {
        match self {
            AwsEditFileKind::Config => dory_core::auth::AuthEditTarget {
                id: AWS_TARGET_ID_CONFIG.to_string(),
                label: AWS_CONFIG_PATH.to_string(),
            },
            AwsEditFileKind::Credentials => dory_core::auth::AuthEditTarget {
...
mod tests {
    use super::*;
    use dory_core::auth::AuthEditTarget;
