...
use dory_app::mcp_command::run_mcp_command;
use dory_audit::AuditService;
use dory_core::ShutdownPhase;
use dory_core::observability::actions::{SYSTEM_SHUTDOWN, SYSTEM_STARTUP};
use dory_core::observability::tracing_bridge::{
...
use dory_driver_ipc::shutdown_managed_hosts;
use dory_ipc::{
...
use dory_ui::AppStateEntity;
use dory_ui::assets::Assets;
use dory_ui::ipc_server::IpcServer;
use dory_ui::keymap::{input_context_keybindings, workspace_keybindings};
use dory_ui::platform;
use dory_ui::ui::overlays::command_palette::command_palette_keybindings;
use dory_ui::ui::views::workspace::Workspace;
use gpui::*;
            ...
            match audit_service.record_panic_best_effort(&panic_info_str) {
                ...
                None => {
                    let _ = std::io::stderr().write_all(
                        b"[dory_audit] panic hook: record_panic_best_effort returned None\n",
                    );
                }
            }
        } else {
            let _ = std::io::stderr()
                .write_all(b"[dory_audit] panic hook: audit service not available\n");
        }
...
fn run_gui() {
    let fmt_writer = if let Some(path) = std::env::var_os("DORY_LOG_FILE").map(PathBuf::from) {
    ...
    match dory_core::observability::tracing_bridge::init_tracing(bridge_config) {
        Ok(handle) => {
    ...
    Application::new().with_assets(Assets).run(|cx: &mut App| {
        dory_ui::theme::init(cx);
        dory_ui::ui::components::data_table::init(cx);
        dory_ui::ui::components::document_tree::init(cx);

        let app_state_inner = match AppStateEntity::new() {
            Ok(state) => state,
            Err(e) => {
                eprintln!(
                    "Dory: failed to initialize storage — cannot open database: {e}\n\
                     Check that ~/.local/share/dory is accessible and not corrupted."
                );
                cx.quit();
                return;
            }
        ...
        if let Some(handle) = BRIDGE_HANDLE.lock().unwrap().as_ref() {
            ...
            let seeded_level =
                dory_core::observability::EventSeverity::from_str_repr(&persisted_min_level)
                    .unwrap_or(dory_core::observability::EventSeverity::Info);
        ...
        *AUDIT_SERVICE_FOR_PANIC.lock().unwrap() = Some(audit_service.clone());
        ...
        let theme_setting = general_settings
            .resolved_theme(dory_ui::theme::appearance_is_dark(cx.window_appearance()));
        let style_setting = general_settings.style;
        let font_setting = general_settings.ui_font;

        let language = dory_i18n::resolve(
            Some(general_settings.language.as_str()),
            dory_i18n::detect_system_locale().as_deref(),
        );
        dory_i18n::set_locale(language);

        // Set up the density global and apply the persisted theme+style so
        // radius tokens are correct from the very first frame.
        dory_ui::theme::init_with_settings(theme_setting, style_setting, font_setting, cx);
        cx.activate(true);
        cx.bind_keys(command_palette_keybindings());
        cx.bind_keys(input_context_keybindings());
        cx.bind_keys(workspace_keybindings());
        dory_ui::ui::app_menus::install_app_menus(cx);

        let channel = dory_core::ReleaseChannel::current();
    ...
    if let Some(handle) = BRIDGE_HANDLE.lock().unwrap().take() {
        match handle.shutdown() {
            ...
            Err(ShutdownError::DrainTimeout {
                ...
                eprintln!(
                    "dory: audit bridge shutdown timed out, dropped {} in-flight events",
                    remaining_in_flight
                );
            }
            Err(ShutdownError::JoinPanic) => {
                eprintln!("dory: audit bridge drain thread panicked during shutdown");
            }
        }
    }
