...
/// Returns the local socket name for the main Dory app-control channel.
///
...
pub fn socket_name() -> io::Result<Name<'static>> {
    ...
    format!("dory{suffix}.sock").to_ns_name::<GenericNamespaced>()
}
...
pub fn driver_socket_name(id: &str) -> io::Result<Name<'static>> {
    let suffix = if cfg!(debug_assertions) { "-debug" } else { "" };
    format!("dory-driver-{id}{suffix}.sock").to_ns_name::<GenericNamespaced>()
}

pub fn auth_provider_socket_name(id: &str) -> io::Result<Name<'static>> {
    let suffix = if cfg!(debug_assertions) { "-debug" } else { "" };
    format!("dory-auth-provider-{id}{suffix}.sock").to_ns_name::<GenericNamespaced>()
}
