...
impl AuthProviderRpcError {
    pub fn into_db_error(self) -> dory_core::DbError {
        match self.code {
            AuthProviderRpcErrorCode::UnsupportedMethod => {
                dory_core::DbError::NotSupported(self.message)
            }
            AuthProviderRpcErrorCode::Timeout => dory_core::DbError::Timeout,
            AuthProviderRpcErrorCode::Transport | AuthProviderRpcErrorCode::VersionMismatch => {
                dory_core::DbError::connection_failed(self.message)
            }
            AuthProviderRpcErrorCode::InvalidRequest
            | AuthProviderRpcErrorCode::Provider
            | AuthProviderRpcErrorCode::Internal => {
                dory_core::DbError::QueryFailed(self.message.into())
            }
        }
    }
}
...
mod tests {
    use super::*;
    use dory_core::auth::{AuthProviderCapabilities, AuthProviderLoginCapabilities};
    ...
    fn auth_provider_error_mapping_preserves_transport_timeout_and_unsupported_method() {
        ...
        assert!(matches!(transport, dory_core::DbError::ConnectionFailed(_)));
        assert!(matches!(timeout, dory_core::DbError::Timeout));
        assert!(matches!(unsupported, dory_core::DbError::NotSupported(_)));
    }
    ...
    fn auth_session_json_data_survives_dto_round_trip() {
        use dory_core::auth::AuthSession;
        use std::sync::Arc;
