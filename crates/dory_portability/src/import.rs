...
pub fn apply(
    ...
    let mut out_auth_profiles: Vec<dory_core::AuthProfile> = Vec::new();
    let mut out_ssh_tunnels: Vec<dory_core::SshTunnelProfile> = Vec::new();
    let mut out_proxies: Vec<dory_core::ProxyProfile> = Vec::new();
    let mut out_connections: Vec<dory_core::ConnectionProfile> = Vec::new();
    ...
    for auth_entry in &parsed.bundle.auth_profiles {
        ...
        for field_name in &auth_entry.secret_field_names {
            ...
            let new_ref = dory_core::auth_field_secret_ref(&new_id, field_name);
        ...
        for rref in &auth_entry.required_refs {
            ...
            if let Some(supplied) = choices.secret_values.get(&key) {
                let new_ref = dory_core::auth_field_secret_ref(&new_id, &rref.field);
        ...
        out_auth_profiles.push(dory_core::AuthProfile {
            id: new_id,
    ...
    for ssh_entry in &parsed.bundle.ssh_tunnels {
        ...
        if let Some((ref_str, secret)) = key_secret_write {
            ...
            for rref in &ssh_entry.required_refs {
                ...
                if let Some(supplied) = choices.secret_values.get(&key) {
                    let new_ref = dory_core::ssh_tunnel_secret_ref(&new_id);
                    secret_writes.push((new_ref, supplied.clone()));
        ...
        out_ssh_tunnels.push(dory_core::SshTunnelProfile {
            id: new_id,
            name: ssh_entry.name.clone(),
            config: dory_core::SshTunnelConfig {
    ...
    for proxy_entry in &parsed.bundle.proxies {
        ...
        for rref in &proxy_entry.required_refs {
            ...
            if let Some(supplied) = choices.secret_values.get(&key) {
                let new_ref = dory_core::proxy_secret_ref(&new_id);
                secret_writes.push((new_ref, supplied.clone()));
            }
        }

        let kind = parse_proxy_kind(&proxy_entry.kind);
        out_proxies.push(dory_core::ProxyProfile {
            id: new_id,
    ...
    for conn_entry in &parsed.bundle.connections {
        ...
        if let Some(secret_map) = secrets {
            ...
            for (staged_key, value) in secret_map {
                if staged_key.starts_with(&prefix) {
                    let new_ref = dory_core::connection_secret_ref(&new_conn_id);
        ...
        let values: dory_core::FormValues = conn_entry
            .fields
            .iter()
        ...
        let config = dory_core::DbConfig::External { kind, values };
        let mut profile = dory_core::ConnectionProfile::new(&conn_entry.name, config);
        ...
        if let Some(dory_core::AccessKind::Proxy { proxy_profile_id }) = &profile.access_kind {
...
fn db_kind_from_bundle(conn: &crate::bundle::ConnectionEntry) -> Option<dory_core::DbKind> {
...
) -> std::collections::HashMap<String, dory_core::values::ValueRef> {
    conn.value_refs
        .iter()
        .filter_map(|(field, tv)| {
            let jv = serde_json::to_value(tv).ok()?;
            let vref: dory_core::values::ValueRef = serde_json::from_value(jv).ok()?;
...
) -> Option<dory_core::ConnectionHooks> {
    let jv = serde_json::to_value(payload).ok()?;
    let mut hooks: dory_core::ConnectionHooks = serde_json::from_value(jv).ok()?;
...
fn restore_hook_env(
    phase_hooks: &mut [dory_core::ConnectionHook],
    phase: &str,
    bundle_local_id: &str,
...
fn restore_settings_overrides(payload: &toml::Value) -> Option<dory_core::GlobalOverrides> {
    let jv = serde_json::to_value(payload).ok()?;
...
fn build_ssh_auth_method(
    ...
    choices: &ResolutionChoices,
) -> (dory_core::SshAuthMethod, Option<(String, SecretString)>) {
    ...
    match entry.auth_method {
        SshAuthMethodKind::Password => {
            ...
            (dory_core::SshAuthMethod::Password, secret_write)
        }
        ...
        Some(username) => {
            if entry.has_secret {
                ...
                if let Some(value) = secrets.and_then(|m| m.get(&old_key)) {
                    ...
                    let key = (entry.local_id.clone(), "password".to_string());
                    if let Some(supplied) = choices.secret_values.get(&key) {
                        let new_ref2 = dory_core::proxy_secret_ref(new_id);
...
enum AccessRewrite {
    ...
    Resolved(Option<dory_core::AccessKind>),
...
fn rewrite_access_kind(
    ...
    match access {
        AccessEntry::Ssh { ssh_local_id } => match id_map.get(ssh_local_id) {
            Some(&new_id) => AccessRewrite::Resolved(Some(dory_core::AccessKind::Ssh {
                ssh_tunnel_profile_id: new_id,
            })),
            None => AccessRewrite::Dangling,
        },
        AccessEntry::Proxy { proxy_local_id } => match id_map.get(proxy_local_id) {
            Some(&new_id) => AccessRewrite::Resolved(Some(dory_core::AccessKind::Proxy {
                proxy_profile_id: new_id,
            })),
            None => AccessRewrite::Dangling,
        },
        AccessEntry::Managed { provider, params } => {
            AccessRewrite::Resolved(Some(dory_core::AccessKind::Managed {
...
fn parse_proxy_kind(kind: &str) -> dory_core::ProxyKind {
    match kind {
        "https" => dory_core::ProxyKind::Https,
        "socks5" => dory_core::ProxyKind::Socks5,
    ...
    fn empty_bundle(encryption: EncryptionMode) -> Bundle {
        Bundle {
            bundle: BundleMeta {
                ...
                dory_version: "0.7.0-dev.0".to_string(),
                encryption,
            },
            drivers: vec![],
            connections: vec![],
...
"#;
    ...
    fn plan_aws_reference_in_dest_is_not_a_required_resolution() {
        use dory_core::auth::{AuthProfile, aws_profile_uuid};
    ...
    fn apply_aws_reference_present_in_dest_gets_deterministic_uuid() {
        use dory_core::auth::{AuthProfile as CoreAuth, aws_profile_uuid};
    ...
    fn apply_connection_form_values_carried_in_config() {
        ...
        let dory_core::DbConfig::External { values, .. } = &conn.config else {
            panic!(
    ...
    fn apply_connection_value_refs_are_preserved() {
        ...
        assert!(
            matches!(vref, dory_core::values::ValueRef::Env { key } if key == "DB_HOST"),
    ...
    fn hooks_toml_payload(hooks: &dory_core::ConnectionHooks) -> toml::Value {
    ...
    fn apply_connection_kind_matches_bundle_kind() {
        ...
        if let dory_core::DbConfig::External { kind, .. } = &conn.config {
            assert_ne!(
                *kind,
                dory_core::DbKind::Postgres,
                "External config kind must not be Postgres for a MySQL bundle entry"
            );
            assert_eq!(
                *kind,
                dory_core::DbKind::MySQL,
    ...
    fn apply_connection_password_is_restored_from_staged_secret() {
        ...
        let conn_id = actions.connections.first().expect("connection").id;
        let expected_ref = dory_core::connection_secret_ref(&conn_id);
    ...
    fn apply_prefix_scan_is_deterministic_with_multi_key_bundle() {
        ...
        let conn_writes: Vec<_> = actions
            .secret_writes
            .iter()
            .filter(|(r, _)| r.starts_with("dory:conn:"))
            .collect();
    ...
    fn apply_uri_secret_field_routes_staged_secret_to_connection_secret_ref() {
        ...
        if let dory_core::DbConfig::External { values, .. } = &conn_out.config {
    ...
    fn conn_conflict_same_name_same_driver_returns_dest_id() {
        ...
        use dory_core::{ConnectionProfile, DbConfig, DbKind, FormValues};
    ...
    fn apply_ssh_password_written_exactly_once() {
        ...
        let ssh_writes: Vec<_> = actions
            .secret_writes
            .iter()
            .filter(|(r, _)| r.starts_with("dory:ssh_tunnel:"))
            .collect();
