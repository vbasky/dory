...
pub fn export(
    ...
    let bundle = Bundle {
        bundle: BundleMeta {
            ...
            dory_version: env!("CARGO_PKG_VERSION").to_string(),
...
fn build_access_entry(profile: &dory_core::ConnectionProfile) -> Option<AccessEntry> {
...
fn build_sanitized_hooks_payload(
    ...
    use dory_core::{ConnectionHook, ConnectionHooks};
...
mod tests {
    ...
    impl crate::ExportTransformResolver for NoTransforms {
        fn transform(
            ...
            _f: &str,
            _v: &FormValues,
        ) -> dory_core::FieldExportTransform {
            dory_core::FieldExportTransform::None
        }
    }
                ...
                && uri.contains('@')
            {
                ...
                if let Some(at_pos) = uri.find('@') {
                    ...
                    if let Some(colon_pos) = before_at.rfind(':') {
                        ...
                        return dory_core::FieldExportTransform::SplitSecret {
                            skeleton,
    ...
    fn secret_field_absent_from_cleartext_connections_fields_section() {
        ...
        let secrets = FixedSecrets({
            ...
            m.insert(format!("dory:conn:{}", profile.id), "sekret".to_string());
            m
        });
    ...
    fn aws_reference_recorded_as_auth_ref_not_cleartext() {
        ...
        profile.auth_profile_id = Some(dory_core::auth::aws_profile_uuid(
            &aws_ref.provider_id,
            &aws_ref.name,
        ));
    ...
    fn ssh_key_embedded_in_secrets_when_opted_in() {
        ...
        let ssh = SshTunnelProfile::new(
            ...
            SshTunnelConfig {
                ...
                auth_method: dory_core::SshAuthMethod::PrivateKey { key_path: None },
            },
        );
    ...
    fn two_secret_fields_lex_first_receives_secret_second_becomes_required_ref() {
        ...
        let conn_ref = dory_core::connection_secret_ref(&profile.id);
        let secrets = FixedSecrets({
            let mut m = HashMap::new();
    ...
    fn auth_secret_from_secret_fields_is_exported() {
        use dory_core::auth::AuthProfile;
        use secrecy::SecretString;
    ...
    fn hook_env_does_not_appear_in_cleartext() {
        use dory_core::{ConnectionHook, ConnectionHooks, HookKind};
    ...
    fn proxy_kind_serializes_as_stable_scheme() {
        use dory_core::{ProxyAuth, ProxyKind, ProxyProfile};
    ...
    fn plaintext_without_force_returns_error() {
        ...
        let secrets = FixedSecrets({
            let mut m = HashMap::new();
            m.insert(
                dory_core::connection_secret_ref(&profile.id),
                "sekret".to_string(),
            );
            m
        });
    ...
    fn ssh_missing_password_records_required_ref() {
        use dory_core::{SshAuthMethod, SshTunnelConfig, SshTunnelProfile};
