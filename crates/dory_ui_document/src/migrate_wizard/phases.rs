    ...
    pub fn label(self) -> String {
        match self {
            WizardPhase::SourceTarget => {
                dory_i18n::t!("document.migrate_wizard.rail.source_target")
            }
            WizardPhase::TablesMapping => {
                dory_i18n::t!("document.migrate_wizard.rail.tables_mapping")
            }
            WizardPhase::Options => dory_i18n::t!("document.migrate_wizard.rail.options"),
            WizardPhase::Confirm => dory_i18n::t!("document.migrate_wizard.rail.confirm"),
            WizardPhase::Run => dory_i18n::t!("document.migrate_wizard.rail.run"),
        }
    }
}
...
mod tests {
    ...
    fn phase_labels_cover_every_rail_entry() {
        ...
        assert_eq!(
            ...
            dory_i18n::t!("document.migrate_wizard.rail.options")
        );
        assert_eq!(
            WizardPhase::Confirm.label(),
            dory_i18n::t!("document.migrate_wizard.rail.confirm")
        );
        assert_eq!(
            WizardPhase::Run.label(),
            dory_i18n::t!("document.migrate_wizard.rail.run")
        );
    }
