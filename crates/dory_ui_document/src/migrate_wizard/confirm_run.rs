...
use dory_components::controls::{Button, Checkbox};
...
use dory_transfer::migration::{MigrationOutcome, MigrationTablePlan, run_migration};
...
pub fn mapping_mode_label(mode: TableMappingMode) -> String {
    match mode {
        TableMappingMode::Create => {
            dory_i18n::t!("document.migrate_wizard.confirm.mode_label.create")
        }
        TableMappingMode::Existing => {
            dory_i18n::t!("document.migrate_wizard.confirm.mode_label.existing")
        }
        TableMappingMode::Recreate => {
            dory_i18n::t!("document.migrate_wizard.confirm.mode_label.recreate")
        }
        TableMappingMode::Skip => {
            dory_i18n::t!("document.migrate_wizard.confirm.mode_label.skip")
        }
        TableMappingMode::Truncate => {
            dory_i18n::t!("document.migrate_wizard.confirm.mode_label.truncate")
        }
    }
}
    ...
    fn spawn_run(&self, run: RunTaskContext, task_id: TaskId, cx: &mut Context<Self>) {
        ...
        cx.spawn(async move |this, cx| {
            ...
            cx.update(|cx| {
                ...
                if toast_success {
                    Toast::success(dory_i18n::t!("document.migrate_wizard.toast.success")).push(cx);
                }
            })
            .ok();
...
fn resolve_run_outcome(
    result: Result<MigrationOutcome, dory_transfer::TransferError>,
) -> RunResolution {
    match result {
        Ok(MigrationOutcome::Completed(outcome)) if outcome.cancelled => RunResolution {
            ...
            report: None,
            toast_success: false,
            summary: dory_i18n::t!("document.migrate_wizard.status.cancelled"),
            warnings: Vec::new(),
        },
        ...
        Ok(MigrationOutcome::CyclicOrderRequired { .. }) => {
            let message = dory_i18n::t!("document.migrate_wizard.error.cyclic_order");
            RunResolution {
        ...
        Err(e) => {
            let message = dory_i18n::t!("document.migrate_wizard.error.failed", error = e);
            RunResolution {
            ...
            .child(
                div().flex().justify_end().child(
                    Button::new(
                        ...
                        dory_i18n::t!("document.migrate_wizard.confirm.start_migration"),
                    )
                    .small()
                    .primary()
            ...
            div()
                ...
                .child(
                    Button::new(
                        ...
                        dory_i18n::t!("document.migrate_wizard.confirm.reorder.up"),
                    )
                    .small()
                    .ghost()
                    .disabled(index == 0)
                ...
                .child(
                    Button::new(
                        ...
                        dory_i18n::t!("document.migrate_wizard.confirm.reorder.down"),
                    )
                    .small()
                    .ghost()
                    .disabled(is_last)
            ...
            .child(
                div().flex().justify_end().child(
                    Button::new(
                        ...
                        dory_i18n::t!("document.migrate_wizard.confirm.reorder.accept"),
                    )
                    .small()
                    .primary()
...
mod tests {
    ...
    use crate::migrate_wizard::column_mapping::TableMigrationConfig;
    use dory_core::{OrderResult, TableRef, TransferColumn};
    use dory_transfer::TableMappingMode;
