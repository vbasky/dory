...
use dory_components::components::tree_nav::{
...
use dory_core::{Connection, TableRef, transfer_compatible};
    ...
    match load {
        ...
        NodeLoad::NotLoaded | NodeLoad::Loading => {
            let mut node = TreeNavNode::leaf(
                ...
                dory_i18n::t!("document.migrate_wizard.footer.loading"),
                Some(AppIcon::Loader),
            );
        ...
        NodeLoad::Failed(error) => {
            let mut node = TreeNavNode::leaf(
                ...
                dory_i18n::t!("document.migrate_wizard.source_target.retry", error = error),
...
impl SourceTargetPhase {
    fn render_tree_panel(&self, side: TreeSide, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        let (title, id_segment) = match side {
            TreeSide::Source => (
                dory_i18n::t!("document.migrate_wizard.source_target.source_title"),
                "source",
            ),
            TreeSide::Target => (
                dory_i18n::t!("document.migrate_wizard.source_target.target_title"),
                "target",
            ),
        };
        let subtitle = match side {
            ...
            TreeSide::Target => self
                ...
                .unwrap_or_else(|| {
                    dory_i18n::t!("document.migrate_wizard.source_target.no_target_selected")
                }),
        };
    ...
    fn render_row(
        &self,
        side: TreeSide,
        row: &dory_components::components::tree_nav::FlatRow,
        is_cursor: bool,
        gutter: AnyElement,
        theme: &gpui_component::Theme,
        cx: &mut Context<Self>,
...
fn relational_schema(snapshot: &dory_core::SchemaSnapshot) -> Option<&dory_core::RelationalSchema> {
    match &snapshot.structure {
        dory_core::DataStructure::Relational(relational) => Some(relational),
        _ => None,
    }
}
...
fn source_database_node(
    ...
    relational: Option<&dory_core::RelationalSchema>,
...
fn current_database_node(database: &str, relational: &dory_core::RelationalSchema) -> DbNode {
