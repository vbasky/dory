...
use dory_transfer::migration::{MigratedTable, MigrationOptions, MigrationTablePlan};
...
/// handling (`crates/dory_ui_sidebar/src/table_loading.rs`), where the same
...
impl MigrateWizard {
    ...
    fn advance_from_source_target(&mut self, window: &mut Window, cx: &mut Context<Self>) {
        ...
        let Some(source_connection) = self.resolve_source_connection(cx) else {
            self.report_advance_error(
                ErrorKind::Storage,
                dory_i18n::t!("document.migrate_wizard.error.no_source_connection"),
                cx,
            );
            return;
        };
        let Some(target_connection) =
            ...
            self.report_advance_error(
                ErrorKind::Storage,
                dory_i18n::t!("document.migrate_wizard.error.no_target_connection"),
                cx,
            );
            return;
        };
    ...
    fn summarize(outcome: &dory_transfer::migration::MigrationRunOutcome) -> String {
...
impl Render for MigrateWizard {
    fn render(&mut self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        let frame = ModalFrame::new("migrate-wizard", &self.focus_handle, close)
            .title(dory_i18n::t!("document.migrate_wizard.title"))
...
impl MigrateWizard {
    ...
    fn render_footer(&self, cx: &mut Context<Self>) -> AnyElement {
        ...
        let continue_label = if self.advancing {
            ...
            dory_i18n::t!("document.migrate_wizard.footer.continue")
        };
        ...
        let actions = div()
            ...
            .when(shows_back, |parent| {
                parent.child(
                    Button::new(
                        "migrate-wizard-back",
                        dory_i18n::t!("document.migrate_wizard.footer.back"),
                    )
                    .small()
                    .ghost()
            ...
            .when(running, |parent| {
                parent.child(
                    Button::new(
                        ...
                        dory_i18n::t!("document.migrate_wizard.footer.cancel"),
                    )
                    .small()
                    .ghost()
            ...
            .when(done, |parent| {
                parent.child(
                    Button::new(
                        ...
                        dory_i18n::t!("document.migrate_wizard.footer.close"),
                    )
                    .small()
                    .primary()
...
mod tests {
    ...
    use dory_core::{ColumnInfo, TableRef, TransferColumn};
