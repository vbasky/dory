...
//! auto-detected pairing (by name, mirroring `dory_transfer`'s
...
use dory_transfer::{ColumnMappingOverride, TableMappingMode};
...
pub fn mapping_mode_option_label(mode: TableMappingMode) -> String {
    match mode {
        TableMappingMode::Create => dory_i18n::t!("document.migrate_wizard.mapping_mode.create"),
        TableMappingMode::Existing => {
            dory_i18n::t!("document.migrate_wizard.mapping_mode.existing")
        }
        TableMappingMode::Recreate => {
            dory_i18n::t!("document.migrate_wizard.mapping_mode.recreate")
        }
        TableMappingMode::Skip => dory_i18n::t!("document.migrate_wizard.mapping_mode.skip"),
        TableMappingMode::Truncate => {
            dory_i18n::t!("document.migrate_wizard.mapping_mode.truncate")
        }
    }
}
...
mod tests {
    ...
    fn mapping_mode_option_label_resolves_and_differs_in_both_locales() {
        ...
        for mode in modes {
            ...
            assert_eq!(label, dory_i18n::t!(key));
