...
//! destructive plan, then run the import via `dory_transfer::import::run_import`.
//!
...
use dory_components::composites::{RailItem, render_wizard_rail};
use dory_components::controls::{Button, Dropdown, DropdownItem, DropdownSelectionChanged};
use dory_components::icons::AppIcon;
use dory_components::primitives::Text;
use dory_components::tokens::Spacing;
use dory_core::{Connection, DriverCapabilities, TaskId, TaskKind, TaskStatus, TaskTarget};
use dory_transfer::TableTransferStatus;
use dory_transfer::import::{
    ImportOptions, ImportOutcome, ImportTablePlan, ImportedTable, run_import,
};
use dory_transfer::manifest::read_manifest;
use dory_ui_base::app_state_entity::{AppStateChanged, AppStateEntity};
use dory_ui_base::modal_frame::ModalFrame;
use dory_ui_base::toast::Toast;
...
impl ImportWizard {
    ...
    fn choose_folder(&mut self, cx: &mut Context<Self>) {
        let Some(connection) = self.resolve_connection(cx) else {
            self.manifest_error = Some(dory_i18n::t!(
        ...
        cx.spawn(async move |this, cx| {
            let dir = if dialog_available {
                ...
                this.update(cx, |this, cx| {
                    this.loading = false;
                    this.manifest_error = Some(dory_i18n::t!(
                ...
                let target_exists_and_columns = connection
                    ...
                    .map(|info| {
                        info.columns
                            .unwrap_or_default()
                            .into_iter()
                            .map(|c| dory_core::TransferColumn {
                                name: c.name,
    ...
    fn build_rows(&mut self, configs: Vec<TableImportConfig>, cx: &mut Context<Self>) {
        ...
        for (table_index, config) in configs.into_iter().enumerate() {
            ...
            let mut source_items = vec![DropdownItem::new(dory_i18n::t!(
    ...
    fn start_import(&mut self, cx: &mut Context<Self>) {
        let Some(connection) = self.resolve_connection(cx) else {
            report_error(
                UserFacingError::new(
                    ErrorKind::Storage,
                    dory_i18n::t!("document.import_wizard.error.no_connection"),
                ),
                cx,
            );
            return;
            ...
            this.update(cx, |this, cx| {
                ...
                match import_result {
                    Ok(outcome) if outcome.cancelled => {
                        ...
                        this.result_summary =
                            Some(dory_i18n::t!("document.import_wizard.toast.cancelled"));
                    }
                    Ok(outcome) => {
                        ...
                        if let Some((table, error)) = &failed_table {
                            ...
                            Toast::success(dory_i18n::t!("document.import_wizard.toast.completed"))
                                .push(cx);
                        }
...
impl Render for ImportWizard {
    fn render(&mut self, _window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        let mut frame = ModalFrame::new("import-wizard", &self.focus_handle, close)
            .title(dory_i18n::t!("document.import_wizard.title"))
        ...
        div()
            ...
            .child(
                Button::new(
                    ...
                    if self.loading {
                        dory_i18n::t!("document.import_wizard.pick_folder.reading_manifest")
                    } else {
                        dory_i18n::t!("document.import_wizard.pick_folder.choose_folder")
                    },
                )
                    ...
                    div()
                        ...
                        .child(
                            Button::new(
                                ...
                                dory_i18n::t!("document.import_wizard.configure.apply_mapping"),
                            )
                            .ghost()
        ...
        div()
            ...
            .child(
                Button::new(
                    ...
                    dory_i18n::t!("document.import_wizard.configure.continue"),
                )
                ...
                div()
                    ...
                    .child(
                        Button::new(
                            ...
                            dory_i18n::t!("document.import_wizard.confirm.back"),
                        )
                        .ghost()
                    ...
                    .child(
                        Button::new(
                            ...
                            dory_i18n::t!("document.import_wizard.confirm.proceed"),
                        )
                        .on_click(
    ...
    fn render_done(&self, cx: &mut Context<Self>) -> AnyElement {
        div()
            ...
            .child(
                Button::new(
                    "import-wizard-close",
                    dory_i18n::t!("document.import_wizard.done.close"),
                )
...
mod tests {
    ...
    use dory_transfer::manifest::ManifestTable;
