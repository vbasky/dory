...
pub fn render_columns(
    ...
    use dory_components::controls::{Button, Checkbox, Input, completion_input_keys_wrapper};
    ...
    let mut container = div().flex().flex_col().gap_1().child(
        Checkbox::new("qb-all-columns")
            .checked(all_active)
            .label(dory_i18n::t!("document.query_builder.columns.all_columns"))
    ...
    if let Some(add_state) = panel.add_column_input_state.as_ref() {
        container = container.child(
            div()
                ...
                .child(
                    Button::new("qb-add-col", dory_i18n::t!("document.shared.add"))
                        .small()
