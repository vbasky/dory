...
use dory_components::common::time_range::view::{TimeRangeChanged, TimeRangePanel};
use dory_components::components::filter_bar::{FilterBarItem, FilterBarMode, FilterBarState};
use dory_components::components::multi_select::{MultiSelect, MultiSelectChanged};
...
use dory_storage::repositories::audit::{AuditEventDto, AuditQueryFilter, AuditRepository};
...
impl AuditDocument {
    ...
    fn new_with_source(
        ...
        let dropdown_timestamp_mode = cx.new(|_cx| {
            Dropdown::new("audit-timestamp-mode")
                .placeholder(dory_i18n::t!("document.audit.filter.placeholder.local"))
        ...
        let multi_select_level = cx.new(|cx| {
            ...
            let mut ms = MultiSelect::new("audit-level")
                .placeholder(dory_i18n::t!("document.audit.detail.level"));
        ...
        let multi_select_category = cx.new(|cx| {
            ...
            let mut ms = MultiSelect::new("audit-category")
                .placeholder(dory_i18n::t!("document.audit.detail.category"));
            ms.set_items(items, cx);
            ms
        });
        ...
        let multi_select_outcome = cx.new(|cx| {
            ...
            let mut ms = MultiSelect::new("audit-outcome")
                .placeholder(dory_i18n::t!("document.audit.detail.outcome"));
            ms.set_items(items, cx);
            ms
        });
        ...
        let chart_group_by_sub = cx.subscribe(
            ...
            |this, _, event: &DropdownSelectionChanged, cx| {
                use dory_components::chart::AuditGroupBy;
    ...
    fn source_loading_label(&self) -> String {
        if self.is_external_event_stream() {
            dory_i18n::t!("document.audit.source.loading.external")
        } else {
            dory_i18n::t!("document.audit.source.loading.internal")
        }
    }

    fn source_error_heading(&self) -> String {
        if self.is_external_event_stream() {
            dory_i18n::t!("document.audit.source.error_heading.external")
        } else {
            dory_i18n::t!("document.audit.source.error_heading.internal")
        }
    }

    fn source_empty_label(&self) -> String {
        if self.is_external_event_stream() {
            dory_i18n::t!("document.audit.source.empty.external")
        } else {
            dory_i18n::t!("document.audit.source.empty.internal")
        }
    }

    fn source_row_label(&self) -> String {
        if self.is_external_event_stream() {
            dory_i18n::t!("document.audit.row.unit.events")
        } else {
            dory_i18n::t!("document.audit.row.unit.rows")
        }
    }

    fn external_page_to_loaded(page: dory_core::EventPage) -> LoadedEventPage {
        let events = page
            .events
            .into_iter()
...
mod tests {
    ...
    fn audit_source_loading_internal_label_differs_between_locales() {
        let en = dory_i18n::t!("document.audit.source.loading.internal", locale = "en");
        let es = dory_i18n::t!("document.audit.source.loading.internal", locale = "es");
    ...
    fn audit_row_unit_rows_label_differs_between_locales() {
        let en = dory_i18n::t!("document.audit.row.unit.rows", locale = "en");
        let es = dory_i18n::t!("document.audit.row.unit.rows", locale = "es");
