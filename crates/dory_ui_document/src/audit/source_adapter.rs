...
use dory_audit::{AuditAggregateParams, AuditGroupColumn, pivot_long_to_wide};
use dory_core::QueryResult;
use dory_core::observability::query::EventDetail;
use dory_core::observability::source::{EventSource as _, EventSourceError};
...
impl AuditSourceAdapter {
    ...
    ///
    /// Delegates pivot logic to `dory_audit::pivot_long_to_wide` so the wide-format
    ...
    pub fn query_abstract(
        &self,
        query: &EventQuery,
    ) -> Result<dory_core::observability::query::EventPage, EventSourceError> {
        ...
        Ok(dory_core::observability::query::EventPage::new(
            records,
            Some(total),
            has_more,
            offset,
