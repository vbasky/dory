...
use dory_components::chart::YScale;
use dory_components::controls::{GpuiInput as Input, InputState, ReadonlyTextView, SelectableText};
use dory_components::icons::AppIcon;
use dory_components::primitives::{Icon, Label, Text, surface_raised};
...
use dory_components::composites::refresh_split_button;
...
impl AuditDocument {
    ...
    pub(super) fn render_toolbar(
        ...
        let search_control = div()
            ...
            .child(
                div().flex_1().child(
                    dory_components::controls::GpuiInput::new(&self.search_input)
                        .small()
                        .h(Heights::BUTTON),
                ),
            );
        ...
        let custom_apply_button = ToolbarButton::new("audit-custom-time-apply")
            .label(dory_i18n::t!("document.audit.filter.apply"))
        ...
        let clear_btn = ToolbarButton::new("audit-clear-btn")
            .label(dory_i18n::t!("document.audit.filter.clear"))
        ...
        compact_top_bar(&theme, {
            ...
            if !self.is_external_event_stream() {
                ...
                let toggle_label = if is_chart {
                    dory_i18n::t!("document.audit.filter.view_mode.table")
                } else {
                    dory_i18n::t!("document.audit.filter.view_mode.chart")
                };
                let view_toggle = div()
                ...
                if is_chart {
                    ...
                    let y_scale_label = match current_y_scale {
                        YScale::Linear => dory_i18n::t!("document.audit.filter.y_scale.linear"),
                        YScale::Log => dory_i18n::t!("document.audit.filter.y_scale.log"),
                    };
            ...
            return div()
                ...
                .child(
                    gpui_component::button::Button::new("audit-retry")
                        .label(dory_i18n::t!("document.audit.filter.retry"))
                        .small()
                        .ghost()
    ...
    pub(super) fn render_inline_detail(
        ...
        let level = event
            .level
            .as_deref()
            .and_then(dory_core::EventSeverity::from_str_repr)
            .map(crate::labels::audit_level_label)
            .or_else(|| event.level.clone());
        let category = event
            .category
            .as_deref()
            .and_then(dory_core::EventCategory::from_str_repr)
            .map(crate::labels::audit_category_label)
            .or_else(|| event.category.clone());
        let outcome = event
            .outcome
            .as_deref()
            .and_then(dory_core::EventOutcome::from_str_repr)
            ...
            .is_some()
        {
            let actor_type_label = event
                .actor_type
                .as_deref()
                .and_then(dory_core::EventActorType::from_str_repr)
                ...
                div()
                    ...
                    .children(vec![
                        self.render_detail_field(
                            dory_i18n::t!("document.audit.detail.time"),
                            Some(timestamp),
                            &theme,
                        )
                        .into_any_element(),
                        self.render_detail_field(
                            dory_i18n::t!("document.audit.detail.level"),
                            level,
                            &theme,
                        )
                        .into_any_element(),
                        self.render_detail_field(
                            dory_i18n::t!("document.audit.detail.category"),
                            category,
                            &theme,
                        )
                        .into_any_element(),
                        self.render_detail_field(
                            dory_i18n::t!("document.audit.detail.outcome"),
                            outcome,
                            &theme,
                        )
                        .into_any_element(),
                        self.render_detail_field(
                            dory_i18n::t!("document.audit.detail.actor"),
                            Some(actor),
                            &theme,
                        )
                        .into_any_element(),
                        self.render_detail_field(
                            dory_i18n::t!("document.audit.detail.action"),
                            action,
                            &theme,
                        )
                        .into_any_element(),
                        self.render_detail_field(
                            dory_i18n::t!("document.audit.detail.source"),
                            source,
                            &theme,
                        )
                        .into_any_element(),
                    ...
                    .when_some(connection_driver, |row, value| {
                        row.child(self.render_detail_field(
                            dory_i18n::t!("document.audit.detail.connection_driver"),
                            Some(value),
                            &theme,
                        ))
                    })
                    .when_some(duration, |row, value| {
                        row.child(self.render_detail_field(
                            dory_i18n::t!("document.audit.detail.duration"),
                            Some(value),
                            &theme,
                        ))
                    }),
            ...
            .when_some(summary, |root, value| {
                root.child(
                    div()
                        .flex_col()
                        .gap_1p5()
                        .child(Label::new(dory_i18n::t!("document.audit.detail.summary")))
            ...
            .when_some(error_message, |root, value| {
                root.child(
                    div()
                        ...
                        .gap_1p5()
                        .child(
                            Label::new(dory_i18n::t!("document.audit.detail.error"))
            ...
            .when_some(details_json, |root, value| {
                root.child(
                    div()
                        .flex_col()
                        .gap_1p5()
                        .child(Label::new(dory_i18n::t!("document.audit.detail.details")))
                        .child(
                            div()
                                .bg(theme.secondary)
                                .p_2()
            ...
            .child(
                div()
                    ...
                    .children(vec![
                        ...
                        .into_any_element(),
                        self.render_detail_field(
                            dory_i18n::t!("document.audit.detail.partition"),
                            source_partition,
                            &theme,
                        )
                        .into_any_element(),
                        self.render_detail_field(
                            dory_i18n::t!("document.audit.detail.event_id"),
                            event_id,
                            &theme,
                        )
                        .into_any_element(),
                    ...
                    .when_some(secondary_timestamp, |row, value| {
                        row.child(self.render_detail_field(
                            dory_i18n::t!("document.audit.detail.secondary_time"),
                            Some(value),
                            &theme,
                        ))
                    }),
            ...
            .when_some(message, |root, value| {
                ...
                root.child(
                    div()
                        .flex_col()
                        .gap_1p5()
                        .child(Label::new(dory_i18n::t!("document.audit.detail.message")))
    ...
    pub(super) fn render_export_button(
        ...
        div()
            ...
            .child(Text::caption(dory_i18n::t!("document.audit.menu.export")))
    ...
    pub(super) fn render_export_menu(
        ...
        let items = [
            (
                dory_i18n::t!("document.audit.menu.export_format.csv"),
                "csv",
            ),
            (
                dory_i18n::t!("document.audit.menu.export_format.json"),
                "json",
            ),
        ]
        .into_iter()
                ...
                |d, _| {
                    d.child(
                        div()
                            ...
                            .child(Text::dim(dory_i18n::t!("document.audit.filter.loading"))),
                    )
                },
            );
...
mod tests {
    ...
    fn audit_filter_clear_label_differs_between_locales() {
        let en = dory_i18n::t!("document.audit.filter.clear", locale = "en");
        let es = dory_i18n::t!("document.audit.filter.clear", locale = "es");
    ...
    fn audit_detail_correlation_id_label_differs_between_locales() {
        let en = dory_i18n::t!("document.audit.detail.correlation_id", locale = "en");
        let es = dory_i18n::t!("document.audit.detail.correlation_id", locale = "es");
    ...
    fn render_detail_field_accepts_a_translated_string_label() {
        ...
        // `dory_i18n::t!` can be passed directly without an intermediate
        // leak or a `&'static str` catalog. This compiles only if the
        // widened signature is in place.
        let label: String = dory_i18n::t!("document.audit.detail.time");
