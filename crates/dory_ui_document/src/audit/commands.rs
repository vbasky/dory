...
impl AuditDocument {
    ...
    pub(super) fn context_menu_items(has_correlation: bool) -> Vec<AuditMenuItem> {
        let mut items = vec![
            AuditMenuItem::item(
                dory_i18n::t!("document.audit.menu.copy_row_as_csv"),
                AuditContextMenuAction::CopyRowAsCsv,
                AppIcon::Layers,
            ),
            AuditMenuItem::item(
                dory_i18n::t!("document.audit.menu.copy_summary"),
        ...
        if has_correlation {
            ...
            items.push(AuditMenuItem::item(
                dory_i18n::t!("document.audit.menu.filter_by_correlation"),
...
mod tests {
    ...
    fn audit_context_menu_copy_row_as_csv_label_differs_between_locales() {
        let en = dory_i18n::t!("document.audit.menu.copy_row_as_csv", locale = "en");
        let es = dory_i18n::t!("document.audit.menu.copy_row_as_csv", locale = "es");
    ...
    fn audit_context_menu_label_round_trips_translated_value() {
        let item = AuditMenuItem::item(
            dory_i18n::t!("document.audit.menu.copy_row_as_csv"),
            AuditContextMenuAction::CopyRowAsCsv,
            dory_components::icons::AppIcon::Layers,
        );
