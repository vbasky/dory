...
use dory_components::common::time_range::{TimestampDisplayMode, format_timestamp_ms};
...
use dory_components::primitives::{Badge, BadgeVariant, Icon, Text};
...
use dory_ui_base::sql_preview_modal::SqlPreviewModal;
use dory_ui_base::toast::{PendingToast, flush_pending_toast};
...
pub struct SchemaDiffDocument {
    ...
    snapshots: Vec<dory_storage::repositories::sch_schema_snapshots::SchemaSnapshotSummary>,
...
impl SchemaDiffDocument {
    pub fn new(
        ...
        let title = match &database {
            Some(db) => dory_i18n::t!("document.schema_diff.view.title", database = db),
            None => dory_i18n::t!("document.schema_diff.view.title_default"),
        };
    ...
    fn compute_diff(&mut self, cx: &mut Context<Self>) {
        ...
        let Some(target) = state.connections().get(&self.profile_id) else {
            self.compute_state = ComputeState::Error(dory_i18n::t!(
    ...
    fn request_apply(&mut self, cx: &mut Context<Self>) {
        ...
        if selected.is_empty() {
            self.pending_toast = Some(PendingToast {
                message: dory_i18n::t!("document.schema_diff.toast.select_at_least_one_to_apply"),
                is_error: true,
            });
            cx.notify();
            return;
        ...
        let target = self
            .database
            .clone()
            .unwrap_or_else(|| dory_i18n::t!("document.schema_diff.summary.this_connection"));
        let summary = dory_i18n::t!(
    ...
    fn run_apply(&mut self, cx: &mut Context<Self>) {
        ...
        let (connection, event_sink, policy) = {
            ...
            let Some(connected) = state.connections().get(&self.profile_id) else {
                self.pending_toast = Some(PendingToast {
                    message: dory_i18n::t!("document.schema_diff.toast.connection_unavailable"),
                    is_error: true,
                });
                cx.notify();
                return;
        ...
        if matches!(policy, MutationPolicy::ReadOnly) {
            self.pending_toast = Some(PendingToast {
                message: dory_i18n::t!("document.schema_diff.toast.read_only"),
                is_error: true,
            });
            cx.notify();
            return;
    ...
    fn route_to_approval(&mut self, selected: &[SelectedTableWork], cx: &mut Context<Self>) {
        ...
        match enqueue {
            Ok(_) => {
                self.pending_toast = Some(PendingToast {
                    message: dory_i18n::t!("document.schema_diff.toast.approval_queued"),
                    is_error: false,
                });
            }
            Err(e) => {
    ...
    fn route_to_approval(&mut self, _selected: &[SelectedTableWork], cx: &mut Context<Self>) {
        self.pending_toast = Some(PendingToast {
            message: dory_i18n::t!("document.schema_diff.toast.approval_unavailable"),
            is_error: true,
        });
        cx.notify();
    }
...
fn build_groups(
    connection: &dyn Connection,
    table_changes: Vec<dory_core::TableChange>,
) -> Vec<TableDiffGroup> {
    use dory_core::TableChange;
...
impl SchemaDiffDocument {
    ...
    fn render_source_picker(&self, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        let mode_toggle = div()
            ...
            .child(self.mode_chip(
                "mode-live",
                dory_i18n::t!("document.schema_diff.view.mode.live"),
            ...
            .child(self.mode_chip(
                "mode-snapshot",
                dory_i18n::t!("document.schema_diff.view.mode.snapshot"),
        ...
        div()
            ...
            .border_color(border)
            .child(
                Text::label_sm(dory_i18n::t!("document.schema_diff.view.compare_against"))
                    .muted_foreground(),
            )
            .child(mode_toggle)
            .child(reference)
            .child(
                div()
                    ...
                    .child(self.primary_button(
                        "compute-diff",
                        dory_i18n::t!("document.schema_diff.action.compute_diff"),
    ...
    fn render_live_reference_list(&self, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        if database_candidates.is_empty() && connection_candidates.is_empty() {
            return div()
                .child(
                    Text::caption(dory_i18n::t!("document.schema_diff.view.source.empty"))
                        .muted_foreground(),
                )
                .into_any_element();
        }
        ...
        match &self.compute_state {
            ComputeState::Loading => {
                return diff_message_container()
                    .child(
                        Text::body(dory_i18n::t!("document.schema_diff.status.computing"))
                            .muted_foreground(),
                    )
                    .into_any_element();
            }
            ComputeState::Idle => {
                return diff_message_container()
                    .child(
                        Text::body(dory_i18n::t!("document.schema_diff.status.idle"))
                            .muted_foreground(),
                    )
                    .into_any_element();
            }
            ...
            ComputeState::Empty => {
                return diff_message_container()
                    .child(
                        Text::body(dory_i18n::t!("document.schema_diff.status.empty"))
                            .muted_foreground(),
                    )
                    .into_any_element();
            }
        ...
        match outcome {
            ...
            TableActionOutcome::Unsupported {
                ...
                let description = if *is_create {
                    dory_i18n::t!("document.schema_diff.view.unsupported_action.create")
                } else {
                    dory_i18n::t!("document.schema_diff.view.unsupported_action.drop")
                };
                ...
                div()
                    ...
                    .py(Spacing::XS)
                    .child(Badge::new(
                        dory_i18n::t!("document.schema_diff.status.unsupported"),
    ...
    fn render_footer(&self, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        div()
            ...
            .child(self.secondary_button(
                "preview-ddl",
                dory_i18n::t!("document.schema_diff.action.preview_ddl"),
            ...
            .child(self.primary_button(
                "apply-ddl",
                dory_i18n::t!("document.schema_diff.action.apply"),
                has_selection && !self.is_busy(),
                cx,
...
mod tests {
    ...
    fn schema_diff_title_interpolates_database_name() {
        let title = dory_i18n::t!("document.schema_diff.view.title", database = "app_db");
    ...
    fn schema_diff_title_differs_between_locales() {
        let en = dory_i18n::t!("document.schema_diff.view.title", locale = "en");
        let es = dory_i18n::t!("document.schema_diff.view.title", locale = "es");
    ...
    fn schema_diff_status_unsupported_differs_between_locales() {
        let en = dory_i18n::t!("document.schema_diff.status.unsupported", locale = "en");
        let es = dory_i18n::t!("document.schema_diff.status.unsupported", locale = "es");
    ...
    fn schema_diff_action_compute_diff_differs_between_locales() {
        let en = dory_i18n::t!("document.schema_diff.action.compute_diff", locale = "en");
        let es = dory_i18n::t!("document.schema_diff.action.compute_diff", locale = "es");
    ...
    fn schema_diff_toast_read_only_differs_between_locales() {
        let en = dory_i18n::t!("document.schema_diff.toast.read_only", locale = "en");
        let es = dory_i18n::t!("document.schema_diff.toast.read_only", locale = "es");
    ...
    fn schema_diff_apply_read_only_matches_toast_read_only_in_english() {
        ...
        let apply = dory_i18n::t!("document.schema_diff.apply.read_only", locale = "en");
        let toast = dory_i18n::t!("document.schema_diff.toast.read_only", locale = "en");
    ...
    fn schema_diff_toast_applied_differs_between_locales() {
        let en = dory_i18n::t!("document.schema_diff.toast.applied", locale = "en");
        let es = dory_i18n::t!("document.schema_diff.toast.applied", locale = "es");
    ...
    fn schema_diff_summary_apply_differs_between_locales() {
        let en = dory_i18n::t!("document.schema_diff.summary.apply", locale = "en");
        let es = dory_i18n::t!("document.schema_diff.summary.apply", locale = "es");
