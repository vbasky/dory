...
impl RiskBadge {
    ...
    pub fn label(self) -> String {
        match self {
            RiskBadge::Safe => dory_i18n::t!("document.schema_diff.source.risk.safe"),
            RiskBadge::Warning => dory_i18n::t!("document.schema_diff.source.risk.warning"),
            RiskBadge::Destructive => {
                dory_i18n::t!("document.schema_diff.source.risk.destructive")
            }
        }
    }
}
...
mod tests {
    ...
    fn db_schema_with_tables(name: &str, table_names: &[&str]) -> dory_core::DbSchemaInfo {
        dory_core::DbSchemaInfo {
            name: name.to_string(),
            tables: table_names
                .iter()
                .map(|t| TableInfo {
    ...
    fn resolve_same_connection_shallow_errors_when_the_database_is_not_loaded() {
        let schemas: std::collections::HashMap<String, dory_core::DbSchemaInfo> =
    ...
    fn schema_not_loaded_message_keys_resolve_in_both_locales_and_interpolates() {
        ...
        let mut schemas: HashMap<String, dory_core::DbSchemaInfo> = HashMap::new();
        let err = resolve_same_connection_shallow(&schemas, "atlas_test")
