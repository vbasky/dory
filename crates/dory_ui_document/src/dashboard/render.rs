...
use dory_components::composites::render_menu_overlay;
use dory_components::controls::Button;
use dory_components::primitives::{Text, surface_card};
use gpui::prelude::*;
                ...
                div()
                    ...
                    .child(
                        div()
                            ...
                            .text_sm()
                            .child(dory_i18n::t!("document.dashboard.status.empty_hint")),
                    )
                    .child(
                        Button::new(
                            "dashboard-add-panel-cta",
                            dory_i18n::t!("document.dashboard.toolbar.add_panel"),
                        )
                        .primary()
                        .on_click(on_add),
                    )
            ...
            for (slot_idx, slot) in self.panel_slots.iter().enumerate() {
                ...
                let panel_title = match slot {
                    ...
                    DashboardPanelSlot::Orphan { .. } => {
                        dory_i18n::t!("document.dashboard.panel.chart_not_found.title")
                    }
...
mod tests {
    ...
    fn dashboard_status_empty_hint_differs_between_locales() {
        let en = dory_i18n::t!("document.dashboard.status.empty_hint", locale = "en");
        let es = dory_i18n::t!("document.dashboard.status.empty_hint", locale = "es");
        assert_ne!(en, es);
    }
