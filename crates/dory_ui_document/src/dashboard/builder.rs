...
pub(super) fn preset_label(preset: TimeRangePreset) -> String {
    match preset {
        TimeRangePreset::Last15min => {
            dory_i18n::t!("document.dashboard.builder.preset.last_15_min")
        }
        TimeRangePreset::LastHour => dory_i18n::t!("document.dashboard.builder.preset.last_hour"),
        TimeRangePreset::Last6Hours => {
            dory_i18n::t!("document.dashboard.builder.preset.last_6_hours")
        }
        TimeRangePreset::Last24Hours => {
            dory_i18n::t!("document.dashboard.builder.preset.last_24_hours")
        }
        TimeRangePreset::Last7Days => {
            dory_i18n::t!("document.dashboard.builder.preset.last_7_days")
        }
    }
}
...
pub(super) fn dashboard_toolbar(
    ...
    use dory_components::common::time_range::TimeRange;
    ...
    // height that matches every other Dory toolbar (data grid, audit, code).
    ...
    let add_btn = ToolbarButton::new("dash-add-panel-toolbar")
        .label(dory_i18n::t!("document.dashboard.toolbar.add_panel"))
    ...
    let (mode_icon, mode_tooltip) = if in_edit_mode {
        (
            AppIcon::Eye,
            dory_i18n::t!("document.dashboard.toolbar.exit_edit_mode_tooltip"),
        )
    } else {
        (
            AppIcon::Pencil,
            dory_i18n::t!("document.dashboard.toolbar.edit_mode_tooltip"),
        )
    };
    ...
    let right_group = if is_read_only {
        ...
        let save_as_btn = ToolbarButton::new("dash-save-as-editable")
            .label(dory_i18n::t!("document.dashboard.toolbar.save_as_editable"))
            .variant(ToolbarButtonVariant::Default)
            .tooltip(dory_i18n::t!(
...
fn build_custom_time_controls(
    panel: &Entity<dory_components::common::time_range::view::TimeRangePanel>,
    cx: &mut Context<DashboardDocument>,
) -> impl IntoElement {
    ...
    div()
        ...
        .child(
            ToolbarButton::new("dashboard-custom-time-apply")
                .label(dory_i18n::t!("document.dashboard.toolbar.apply"))
                .variant(ToolbarButtonVariant::Default)
                .disabled(!can_apply)
                .on_click(on_apply),
        )
...
pub(super) fn panel_header(
    ...
    if let Some(input_state) = editing_input {
        ...
        header = header.child(
            dory_components::controls::Input::new(input_state)
                .w_full()
                .small(),
        );
    } else {
...
fn panel_kebab_menu(
    ...
    use dory_components::composites::{MenuItem, render_menu_items};
    use dory_components::icons::AppIcon;
    ...
    let mut menu_items: Vec<MenuItem> = vec![
        MenuItem::new(dory_i18n::t!("document.dashboard.panel.menu.configure"))
            .icon(AppIcon::Settings),
    ];
    if has_editable_title {
        menu_items.push(
            MenuItem::new(dory_i18n::t!("document.dashboard.panel.menu.edit_title"))
                .icon(AppIcon::Pencil),
        );
    }
    menu_items.push(MenuItem::separator());
    menu_items.push(
        MenuItem::new(dory_i18n::t!("document.dashboard.panel.menu.remove"))
            .icon(AppIcon::Delete)
            .danger(),
    );
...
mod tests {
    ...
    fn preset_label_returns_correct_string() {
        ...
        assert_eq!(
            ...
            dory_i18n::t!("document.dashboard.builder.preset.last_hour")
        );
        assert_eq!(
    ...
    fn preset_label_keys_resolve_and_differ_between_locales() {
        ...
        let en = dory_i18n::t!("document.dashboard.builder.preset.last_hour", locale = "en");
        let es = dory_i18n::t!("document.dashboard.builder.preset.last_hour", locale = "es");
        assert_ne!(en, es);
    }
    ...
    fn dashboard_toolbar_add_panel_differs_between_locales() {
        let en = dory_i18n::t!("document.dashboard.toolbar.add_panel", locale = "en");
        let es = dory_i18n::t!("document.dashboard.toolbar.add_panel", locale = "es");
        assert_ne!(en, es);
    }
