...
use dory_components::controls::{Dropdown, DropdownItem, DropdownSelectionChanged, InputState};
...
use dory_ui_base::{AppStateChanged, AppStateEntity, DashboardPanel, DashboardPanelDraft};
use gpui::prelude::*;
use gpui::{
...
impl DashboardDocument {
    ...
    /// `dory_core::RefreshPolicy` used by `refresh_split_button`.
    ///
    ...
    pub fn start_panel_title_edit(
        ...
        let subscription = cx.subscribe_in(
            &input,
            window,
            move |this, entity, event: &dory_components::controls::InputEvent, _window, cx| {
                match event {
                    dory_components::controls::InputEvent::PressEnter { secondary: false } => {
                    ...
                    dory_components::controls::InputEvent::Blur => {
                        let value = entity.read(cx).value().to_string();
                        if let Some(idx) = this.editing_title_panel_index.take() {
                            this.update_panel_title(idx, value, cx);
                        }
    ...
    pub fn start_dashboard_name_edit(&mut self, window: &mut Window, cx: &mut Context<Self>) {
        ...
        let subscription = cx.subscribe_in(
            &input,
            window,
            |this, entity, event: &dory_components::controls::InputEvent, _window, cx| match event {
                dory_components::controls::InputEvent::PressEnter { secondary: false } => {
                    let value = entity.read(cx).value().to_string();
    ...
    pub fn end_panel_drag(&mut self, cx: &mut Context<Self>) {
        ...
        if self.collides_with_other_panels(from, &proposed) {
            Toast::info(dory_i18n::t!("document.dashboard.toast.position_overlap")).push(cx);
            cx.notify();
            return;
        }
    ...
    pub fn reconcile_panels_from_manager(
        ...
        for panel in persisted_panels.iter() {
            ...
            if let dory_ui_base::DashboardPanelKind::Divider { markdown } = &panel.kind {
            ...
            if let dory_ui_base::DashboardPanelKind::Inspector { metric_id } = &panel.kind {
...
mod tests {
    ...
    fn dashboard_toast_position_overlap_differs_between_locales() {
        let en = dory_i18n::t!("document.dashboard.toast.position_overlap", locale = "en");
        let es = dory_i18n::t!("document.dashboard.toast.position_overlap", locale = "es");
        assert_ne!(en, es);
    }
