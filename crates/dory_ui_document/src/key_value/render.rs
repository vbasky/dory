...
mod tests {
    ...
    fn key_value_page_label_interpolates_page_number() {
        let page = dory_i18n::t!("document.key_value.render.page", locale = "en", page = 3);
...
impl Render for super::KeyValueDocument {
    fn render(&mut self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        let (delete_title, delete_message) = if let Some(pending) = &self.pending_key_delete {
            (
                dory_i18n::t!("document.key_value.render.delete_key.title"),
                dory_i18n::t!(
                    "document.key_value.render.delete_key.message",
                    key = pending.key
                ),
            ...
            (
                dory_i18n::t!("document.key_value.render.delete_member.title"),
                dory_i18n::t!(
                    "document.key_value.render.delete_member.message",
        ...
        let right_panel = if let Some(value) = &self.selected_value {
            ...
            let type_label = value
                ...
                .key_type
                .map(key_type_label)
                .unwrap_or_else(|| dory_i18n::t!("document.key_value.render.unknown_type"));
                ...
                && self.supports_document_view()
            {
                ...
                if let Some(tree) = &self.document_tree {
                    ...
                    panel = panel.child(
                        div()
                            ...
                            .border_l_1()
                            .border_color(theme.border)
                            .child(Text::muted(dory_i18n::t!("document.data.grid.empty"))),
                    );
                }
                ...
                header = header.child(div().flex_1().child(Text::caption(if is_stream {
                    dory_i18n::t!("document.key_value.render.id_header")
                } else {
                    dory_i18n::t!("document.key_value.render.value_header")
                })));
                if needs_value_col {
                    header = header.child(div().w(px(200.0)).child(Text::caption(if is_stream {
                        dory_i18n::t!("document.key_value.render.fields_header")
                    } else {
                        dory_i18n::t!("document.key_value.render.field_score_header")
                    })));
                }
                ...
                let is_editable = matches!(
                    ...
                    Some(dory_core::KeyType::String) | Some(dory_core::KeyType::Json)
                );
                ...
                panel = panel.child(
                    div()
                        ...
                        .child(value_preview)
                        .when(is_editable, |d| {
                            d.child(div().pt(Spacing::SM).child(Text::caption(dory_i18n::t!(
                                "document.key_value.render.click_to_edit"
                            ))))
                        }),
                );
            ...
            div()
                ...
                .child(if self.runner.is_primary_active() {
                    ...
                    Text::muted(dory_i18n::t!("document.key_value.render.select_key_prompt"))
                        .into_any_element()
                })
                .into_any_element()
        };
                    ...
                    .child(
                        div()
                            ...
                            .child(Text::caption(if self.runner.is_primary_active() {
                                dory_i18n::t!("document.data.grid.loading")
                            } else if key_count == 1 {
                                dory_i18n::t!(
                                    "document.key_value.render.keys_count.one",
                                    count = key_count
                    ...
                    if is_renaming {
                        ...
                        row = row.child(Text::caption(
                            key.key_type.map(key_type_label).unwrap_or_else(|| {
                                dory_i18n::t!("document.key_value.parsing.type.unknown")
                            }),
                        ));
                    }
