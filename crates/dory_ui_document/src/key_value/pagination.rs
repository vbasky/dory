...
use dory_core::{DbError, KeyGetRequest, KeyScanRequest, TaskKind};
...
impl super::KeyValueDocument {
    ...
    pub(super) fn reload_selected_value(&mut self, cx: &mut Context<Self>) {
        ...
        let description = format!("GET {}", dory_core::truncate_string_safe(&key, 60));
                    ...
                    match result {
                        Ok(value) => {
                            ...
                            let is_hash_or_stream = matches!(
                                key_type,
                                Some(dory_core::KeyType::Hash | dory_core::KeyType::Stream)
                            );
