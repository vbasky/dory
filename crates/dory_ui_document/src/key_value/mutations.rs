...
impl super::KeyValueDocument {
    ...
    fn delete_key_async(&mut self, key: String, cx: &mut Context<Self>) {
        ...
        let description = format!("DEL {}", dory_core::truncate_string_safe(&key, 60));
            ...
            cx.update(|cx| {
                entity.update(cx, |this, cx| {
                    match result {
                        ...
                        Ok(false) => {
                            let message =
                                dory_i18n::t!("document.key_value.mutation.error.key_not_deleted");
    ...
    fn delete_member_async(&mut self, member: MemberEntry, cx: &mut Context<Self>) {
        ...
        let (task_id, _cancel_token) = self.runner.start_mutation(
            ...
            dory_i18n::t!(
                ...
                key = dory_core::truncate_string_safe(&key, 40)
            ),
            cx,
        );
    ...
    fn commit_rename(&mut self, window: &mut Window, cx: &mut Context<Self>) {
        ...
        let (task_id, _cancel_token) = self.runner.start_mutation(
            ...
            format!(
                "RENAME {} -> {}",
                dory_core::truncate_string_safe(&old_name, 30),
                dory_core::truncate_string_safe(&new_name, 30)
            ),
            cx,
        );
    ...
    fn commit_string_edit(&mut self, window: &mut Window, cx: &mut Context<Self>) {
        ...
        let (task_id, _cancel_token) = self.runner.start_mutation(
            TaskKind::KeyMutation,
            format!("SET {}", dory_core::truncate_string_safe(&key, 60)),
            cx,
        );
    ...
    pub(super) fn handle_new_key_created(
        ...
        let (task_id, _cancel_token) = self.runner.start_mutation(
            ...
            dory_i18n::t!(
                ...
                key = dory_core::truncate_string_safe(&event.key_name, 40)
            ),
            cx,
        );
                    ...
                    match event.value {
                        ...
                        NewKeyValue::HashFields(fields) => {
                            ...
                            if let Some(ttl) = event.ttl {
                                api.expire_key(&dory_core::KeyExpireRequest {
                                    key: event.key_name.clone(),
                                    ttl_seconds: ttl,
                                    keyspace,
                                })?;
...
mod tests {
    ...
    fn key_value_mutation_task_descriptions_interpolate_key_name() {
        let delete_member = dory_i18n::t!(
            "document.key_value.mutation.task.delete_member",
            locale = "en",
            key = "session:42"
        );
        let create_key = dory_i18n::t!(
            "document.key_value.mutation.task.create_key",
            locale = "en",
            key = "session:42"
        );
