...
use dory_components::components::document_tree::{DocumentTree, DocumentTreeState};
...
use dory_core::{CancelToken, KeyEntry, KeyGetResult, KeyType, RefreshPolicy};
...
impl KeyValueDocument {
    ...
    pub(super) fn apply_ttl_from_entry(&mut self, entry: &KeyEntry, cx: &mut Context<Self>) {
        ...
        match entry.ttl_seconds {
            None | Some(-1) => {
                ...
                self.ttl_display = dory_i18n::t!("document.key_value.render.ttl.no_limit");
            }
            Some(-2) => {
                self.ttl_state = TtlState::Missing;
                self.ttl_display = dory_i18n::t!("document.key_value.render.ttl.missing");
            }
            Some(0) => {
                self.ttl_state = TtlState::Expired;
                self.ttl_display = dory_i18n::t!("document.key_value.render.ttl.expired");
            }
            Some(secs) if secs > 0 => {
    ...
    pub(super) fn get_connection(
        &self,
        cx: &Context<Self>,
    ) -> Option<Arc<dyn dory_core::Connection>> {
        self.app_state
            .read(cx)
            .connections()
            .get(&self.profile_id)
