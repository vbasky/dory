...
//! reuses `dory_transfer::export::{run_export, ExportOptions}` unchanged —
//! see [`run`].
...
use dory_core::{Connection, TableRef};
use dory_transfer::FileFormat;
use dory_ui_base::app_state_entity::AppStateEntity;
...
pub struct ExportWizard {
    ...
    cancel_token: Option<dory_core::CancelToken>,
        ...
        cx.spawn(async move |this, cx| {
            let picked = if dialog_available {
                rfd::AsyncFileDialog::new()
                    .set_title(dory_i18n::t!("document.export_wizard.format_options.dialog_title"))
                    .pick_folder()
                    .await
                ...
                match dory_ui_base::file_dialog::fallback_export_dir() {
                    ...
                    Err(err) => {
                        this.update(cx, |this, cx| {
                            ...
                            this.folder_error = Some(dory_i18n::t!(
...
impl Render for ExportWizard {
    fn render(&mut self, _window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        let frame = ModalFrame::new("export-wizard", &self.focus_handle, close)
            .title(dory_i18n::t!("document.export_wizard.title"))
...
impl ExportWizard {
    ...
    fn render_format_options(&self, cx: &mut Context<Self>) -> AnyElement {
        let folder_label = self.output_dir.as_ref().map_or_else(
            || dory_i18n::t!("document.export_wizard.format_options.no_folder_chosen"),
                    ...
                    .child(
                        Button::new(
                            ...
                            if self.choosing_folder {
                                dory_i18n::t!("document.export_wizard.format_options.choosing")
                            } else {
                                dory_i18n::t!("document.export_wizard.format_options.choose_folder")
                            },
                        )
                        .small()
        ...
        div()
            ...
            .child(
                div().flex().justify_end().child(
                    Button::new(
                        "export-wizard-start",
                        dory_i18n::t!("document.export_wizard.confirm.start_export"),
                    )
                    .small()
                    .primary()
            ...
            .child(
                div().flex().justify_end().child(
                    Button::new(
                        ...
                        dory_i18n::t!("document.export_wizard.running.cancel"),
                    )
                    .small()
                    .ghost()
        ...
        let actions = div()
            ...
            .when(shows_back, |parent| {
                parent.child(
                    Button::new(
                        "export-wizard-back",
                        dory_i18n::t!("document.export_wizard.footer.back"),
                    )
                    .small()
                    .ghost()
            ...
            .when(shows_continue, |parent| {
                parent.child(
                    Button::new(
                        ...
                        dory_i18n::t!("document.export_wizard.footer.continue"),
                    )
                    .small()
                    .primary()
            ...
            .when(done, |parent| {
                parent.child(
                    Button::new(
                        "export-wizard-close",
                        dory_i18n::t!("document.export_wizard.footer.close"),
                    )
                    .small()
                    .primary()
