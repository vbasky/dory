...
//!
//! Reused unchanged: `dory_transfer::export::{run_export, ExportOptions}`
//! and the timestamped `dory-export-<ts>` folder-bundle + `manifest.json`
...
use dory_core::{CancelToken, Connection, TaskId, TaskKind, TaskStatus, TaskTarget};
use dory_transfer::TableTransferStatus;
use dory_transfer::export::{ExportOptions, ExportOutcome, ExportTable, ExportedTable, run_export};
use dory_transfer::{FileFormat, TransferError};
use dory_ui_base::app_state_entity::AppStateChanged;
...
fn to_transfer_columns(columns: Vec<dory_core::ColumnInfo>) -> Vec<dory_core::TransferColumn> {
    columns
        .into_iter()
...
fn resolve_run_outcome(result: Result<ExportOutcome, TransferError>) -> RunResolution {
    match result {
        Ok(outcome) if outcome.cancelled => RunResolution {
            ...
            report: None,
            toast_success: false,
            summary: dory_i18n::t!("document.export_wizard.toast.cancelled"),
            warnings: Vec::new(),
        },
        Ok(outcome) => {
        ...
        Err(e) => {
            let message = dory_i18n::t!("document.export_wizard.toast.failed", error = e);
            RunResolution {
    ...
    pub(crate) fn start_export(&mut self, cx: &mut Context<Self>) {
        ...
        let Some(connection) = self.resolve_connection(cx) else {
            report_error(
                UserFacingError::new(
                    ErrorKind::Storage,
                    dory_i18n::t!("document.export_wizard.error.no_connection"),
                ),
                cx,
            );
            return;
        ...
        *self.progress.lock().unwrap_or_else(|p| p.into_inner()) = RunProgress::default();
        ...
        let output_dir = base_dir.join(format!(
            "dory-export-{}",
            dory_core::chrono::Utc::now().format("%Y%m%d-%H%M%S%.3f")
        ));
                ...
                match build_export_tables(&connection, database.as_deref(), &table_specs) {
                    ...
                    Err(err) => {
                        ...
                        this.update(cx, |this, cx| {
                            ...
                            this.result_summary = Some(dory_i18n::t!(
...
mod tests {
    ...
    use dory_transfer::{TableTransferStatus, TransferError};
