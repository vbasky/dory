...
// `DbError` is a large type defined in `dory_core`; we cannot change its size.
...
impl MetricPickerState {
    ...
    pub fn ensure_dimensions_loaded(
        ...
        let task = match conn_result {
            None => {
                self.dimensions_state = DimensionsState::Error(dory_i18n::t!(
                            ...
                            match result {
                                ...
                                None => {
                                    picker.dimensions_state =
                                        DimensionsState::Error(dory_i18n::t!(
