...
pub enum ChartShellEvent {
    ...
    MetricPickerApplied(Box<dyn dory_components::chart::ChartDataSource>),
}
...
use dory_core::{ColumnKind, ColumnMeta, QueryResult};
use gpui::prelude::*;
...
impl ChartShell {
    ...
    pub fn current_chart_spec(&self, columns: &[dory_core::ColumnMeta]) -> ChartSpec {
    ...
    pub fn active_bindings(&self) -> BindingSpec {
        if let Some(manual) = &self.chart_manual_selection {
            BindingSpec {
                ...
                group_by: None,
                filter: None,
                aggregation: dory_components::chart::AggKind::None,
            }
