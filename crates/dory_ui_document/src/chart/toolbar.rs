...
use dory_components::chart::{ChartKind, format_resolution, format_x_value};
...
use dory_components::tokens::{FontSizes, Radii, Spacing};
...
pub fn render_chart_toolbar(
    ...
    let stats_btn = toolbar_btn(
        "chart-toolbar-stats",
        AppIcon::ChartBar,
        dory_i18n::t!("document.chart.toolbar.stats").into(),
        is_stats_active,
    )
    .on_mouse_down(MouseButton::Left, move |_, window, cx| {
        on_stats(window, cx);
    ...
    let save_btn = toolbar_btn(
        "chart-toolbar-save",
        AppIcon::Save,
        dory_i18n::t!("document.chart.toolbar.save_chart").into(),
        false,
    )
    .on_mouse_down(MouseButton::Left, move |_, window, cx| {
        on_save(window, cx);
            ...
            div()
                ...
                .child(
                    div()
                        ...
                        .child(dory_i18n::t!("document.chart.toolbar.type_label")),
                )
                .child(kind_chips),
        )
        .child(vdivider(border))
