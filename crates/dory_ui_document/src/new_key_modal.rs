...
use dory_components::components::form_navigation::{
...
impl NewKeyType {
    ...
    pub fn label(self) -> String {
        match self {
            Self::String => dory_i18n::t!("document.key_value.new_key.type.string"),
            Self::Hash => dory_i18n::t!("document.key_value.new_key.type.hash"),
            Self::List => dory_i18n::t!("document.key_value.new_key.type.list"),
            Self::Set => dory_i18n::t!("document.key_value.new_key.type.set"),
            Self::SortedSet => dory_i18n::t!("document.key_value.new_key.type.sorted_set"),
            Self::Json => dory_i18n::t!("document.key_value.new_key.type.json"),
            Self::Stream => dory_i18n::t!("document.key_value.new_key.type.stream"),
        }
    }
}
...
impl NewKeyModal {
    pub fn new(window: &mut Window, cx: &mut Context<Self>) -> Self {
        ...
        let ttl_input = cx.new(|cx| {
            InputState::new(window, cx)
                .placeholder(dory_i18n::t!("document.key_value.new_key.ttl.placeholder"))
        });
    ...
    fn add_value_row(&mut self, window: &mut Window, cx: &mut Context<Self>) {
        let field_placeholder = match self.selected_type {
            ...
            _ => dory_i18n::t!("document.key_value.new_key.member_placeholder"),
        };
        let value_placeholder = match self.selected_type {
            ...
            _ => dory_i18n::t!("document.key_value.new_key.value.placeholder"),
        };
    ...
    fn submit(&mut self, _window: &mut Window, cx: &mut Context<Self>) {
        ...
        if key_name.is_empty() {
            self.error_message = Some(dory_i18n::t!(
...
impl Render for NewKeyModal {
    fn render(&mut self, _window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        if let Some(error) = &self.error_message {
            body = body.child(Text::caption(dory_i18n::t!(
                ...
                .child(
                    focus_ring(cancel_focused, ring_color).child(
                        Button::new("new-key-cancel")
                            .small()
                            .label(dory_i18n::t!("document.key_value.new_key.cancel"))
                ...
                .child(
                    focus_ring(create_focused, ring_color).child(
                        Button::new("new-key-create")
                            .small()
                            .label(dory_i18n::t!("document.key_value.new_key.create"))
        ...
        ModalFrame::new("new-key-modal", &self.focus_handle, close)
            .title(dory_i18n::t!("document.key_value.new_key.title"))
            .icon(AppIcon::Plus)
            .width(px(600.0))
            .max_height(px(500.0))
