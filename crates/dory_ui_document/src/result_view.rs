use dory_components::chart::{AggKind, BindingSpec, ChartDetection};
use dory_core::{ColumnKind, ColumnMeta, QueryResultShape};

// `ResultViewMode` now lives in `dory_components` so it can be used by
// `ResultPanel` without a circular dependency. Re-exported here for all
// callers in `dory_ui` that import from this module.
pub use dory_components::result_view::ResultViewMode;
...
mod tests {
    ...
    use dory_components::chart::{AggKind, ChartDetection};
    use dory_core::{ColumnKind, ColumnMeta, QueryResultShape};
