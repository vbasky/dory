...
use dory_components::components::document_tree::NodeId;
use dory_core::{
...
impl DataGridPanel {
    ...
    pub(super) fn handle_document_tree_inline_edit(
        ...
        cx.spawn(async move |_this, cx| {
            ...
            let result: Result<dory_core::CrudResult, dory_core::DbError> = cx
                .background_executor()
    ...
    pub(super) fn handle_save_row(&mut self, row_idx: usize, cx: &mut Context<Self>) {
        ...
        let changes_ref: Vec<(
            usize,
            &dory_components::components::data_table::model::CellValue,
    ...
    pub(super) fn save_table_row(
        ...
        {
            let message = dory_i18n::t!("document.data.mutation.error.save_row_unsupported_pk");
        ...
        if pk_columns.len() != pk_indices.len() || pk_values.len() != pk_indices.len() {
            report_error(
                UserFacingError::new(
                    ErrorKind::User,
                    dory_i18n::t!("document.data.mutation.error.save_row_identity_failed"),
                ),
                cx,
            );
            return;
        ...
        {
            let message = dory_i18n::t!("document.data.mutation.error.save_row_unsupported_values");
    ...
    pub(super) fn handle_save_result(
        &mut self,
        row_idx: usize,
        result: Result<dory_core::CrudResult, dory_core::DbError>,
        cx: &mut Context<Self>,
    ) {
        let Some(table_state) = &self.grid_table.table_state else {
            return;
        ...
        match result {
            Ok(mut crud_result) => {
                ...
                self.pending.toast = Some(PendingToast {
                    message: dory_i18n::t!("document.data.mutation.toast.saved"),
                    is_error: false,
                });
            }
            Err(e) => {
                ...
                report_error(
                    UserFacingError::new(
                        ErrorKind::Driver,
                        dory_i18n::t!("document.data.mutation.error.save_failed", error = e),
                    ),
                    cx,
                );
            }
    ...
    pub(super) fn commit_insert_collection(
        ...
        let insert = dory_core::DocumentInsert::one(collection.name.clone(), doc.into())
            .with_database(collection.database.clone());
    ...
    pub(super) fn commit_insert_table(
        ...
        if assignments.is_empty() {
            report_error(
                UserFacingError::new(
                    ErrorKind::Driver,
                    dory_i18n::t!("document.data.mutation.error.insert_no_values"),
                ),
                cx,
            );
            return;
                ...
                entity.update(cx, |panel, cx| {
                    match result {
                        Ok(mut crud_result) => {
                            ...
                            panel.pending.toast = Some(PendingToast {
                                message: dory_i18n::t!("document.data.mutation.toast.row_inserted"),
                                is_error: false,
                            });
                            panel.queue_refresh_after_mutation_success(cx);
                        }
    ...
    pub(super) fn commit_delete_collection(
        ...
        let filter = if pk_columns.is_empty() {
            ...
            match &id_value {
                Value::ObjectId(oid) => {
                    dory_core::DocumentFilter::new(serde_json::json!({"_id": {"$oid": oid}}))
                }
                Value::Text(s) => dory_core::DocumentFilter::new(serde_json::json!({"_id": s})),
                _ => {
                    report_error(
                        UserFacingError::new(
                            ErrorKind::User,
            ...
            dory_core::DocumentFilter::new(serde_json::Value::Object(filter_obj))
        };

        let delete = dory_core::DocumentDelete::new(collection.name.clone(), filter)
            .with_database(collection.database.clone());
    ...
    pub(super) fn commit_delete_table(
        ...
        if pk_count == 0 {
            report_error(
                UserFacingError::new(
                    ErrorKind::Driver,
                    dory_i18n::t!("document.data.mutation.error.delete_no_primary_key"),
                ),
                cx,
            );
            return;
        ...
        if pk_columns.len() != pk_count || pk_values.len() != pk_count {
            report_error(
                UserFacingError::new(
                    ErrorKind::Driver,
                    dory_i18n::t!("document.data.mutation.error.delete_identity_failed"),
                ),
                cx,
            );
            return;
                ...
                entity.update(cx, |panel, cx| {
                    match result {
                        Ok(mut crud_result) => {
                            ...
                            panel.pending.toast = Some(PendingToast {
                                message: dory_i18n::t!("document.data.mutation.toast.row_deleted"),
                                is_error: false,
                            });
                            panel.pending.refresh = true;
                        }
    ...
    pub(super) fn commit_bulk_deletes_table(
        ...
        if identities.is_empty() {
            report_error(
                UserFacingError::new(
                    ErrorKind::Driver,
                    dory_i18n::t!("document.data.mutation.error.bulk_delete_no_rows_identified"),
                ),
                cx,
            );
            return;
        ...
        cx.spawn(async move |_this, cx| {
            ...
            let mut last_error: Option<dory_core::DbError> = None;
    ...
    pub(super) fn commit_bulk_deletes_collection(
        ...
        let mut filters = Vec::with_capacity(count);
        {
            ...
            for &row_idx in &row_indices {
                let filter = if pk_columns.is_empty() {
                    ...
                    match &id_value {
                        Value::ObjectId(oid) => dory_core::DocumentFilter::new(
                            serde_json::json!({"_id": {"$oid": oid}}),
                        ),
                        Value::Text(s) => {
                            dory_core::DocumentFilter::new(serde_json::json!({"_id": s}))
                        }
                        _ => {
                            log::error!(
        ...
        cx.spawn(async move |_this, cx| {
            ...
            for (_row_idx, filter) in &filters {
                let delete =
                    dory_core::DocumentDelete::new(collection_name.clone(), filter.clone())
