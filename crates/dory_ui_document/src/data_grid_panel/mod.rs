...
pub enum DataGridEvent {
    ...
    RequestSqlPreview {
        ...
        generation_type: dory_components::SqlGenerationType,
    },
...
struct TableContextMenu {
    ...
    doc_field_value: Option<dory_components::components::document_tree::NodeValue>,
    ...
    row_actions: Vec<dory_core::InspectorRowAction>,
}
...
struct ContextMenuItem {
    ...
    icon: Option<dory_components::icons::AppIcon>,
    is_separator: bool,
    is_danger: bool,
}
...
type RowActionProvider = Arc<dyn Fn(&str) -> Vec<dory_core::InspectorRowAction> + Send + Sync>;
...
struct ChartState {
    ...
    chart_source_time_range_panel:
        Option<Entity<dory_components::common::time_range::view::TimeRangePanel>>,
}

/// Mutation confirmation modal pair (light + hard variants).
struct MutationConfirmState {
    /// Light variant for small row counts.
    pub(crate) mutation_confirm_light: Entity<dory_components::modals::ModalMutationConfirm>,
    /// Hard variant for large row counts / DELETE.
    pub(crate) mutation_confirm_hard: Entity<dory_components::modals::ModalMutationConfirmHard>,
}
...
pub(crate) struct BuilderState {
    ...
    pub(crate) builder_editable_binding: Option<dory_core::EditableBinding>,
}
...
pub(crate) struct PendingMutationExec {
    pub(crate) spec: dory_core::VisualMutationSpec,
...
pub(super) struct CollectionChartSaveState {
    pub(super) name_input: Entity<dory_components::controls::InputState>,
    pub(super) chart_spec: dory_components::chart::ChartSpec,
    pub(super) bindings: dory_components::chart::BindingSpec,
...
impl DataGridPanel {
        ...
        cx.spawn(async move |_this, cx| {
            ...
            cx.update(|cx| {
                let fetch_result = match result {
                    Ok(r) => r,
                    Err(e) => {
                        dory_ui_base::user_error::report_error(
                            dory_ui_base::user_error::UserFacingError::new(
                                dory_ui_base::user_error::ErrorKind::Driver,
    ...
    fn new_internal(
        ...
        {
            let source_columns: Vec<dory_core::ColumnInfo> = {
        ...
        let mutation_confirm_light =
            cx.new(|_cx| dory_components::modals::ModalMutationConfirm::new(window, _cx));
        let mutation_confirm_hard =
            cx.new(|cx| dory_components::modals::ModalMutationConfirmHard::new(window, cx));

        cx.subscribe_in(
            &mutation_confirm_light,
            window,
            |this, _, outcome: &dory_components::modals::MutationConfirmOutcome, window, cx| {
        ...
        let refresh_policy_sub = cx.subscribe_in(
            ...
            |this, _, event: &DropdownSelectionChanged, _window, cx| {
                ...
                if policy.is_auto() && !this.supports_auto_refresh() {
                    ...
                    dory_ui_base::toast::Toast::warning(
    ...
    fn collect_row_values(&self, visual_row: usize, cx: &App) -> Vec<Value> {
        use dory_components::components::data_table::model::VisualRowSource;
    ...
    pub fn open_collection_chart_save(&mut self, window: &mut Window, cx: &mut Context<Self>) {
        ...
        let name_input = cx.new(|cx| {
            dory_components::controls::InputState::new(window, cx)
                .placeholder(dory_i18n::t!("document.data.grid.placeholder.chart_name"))
        });
        ...
        let sub = cx.subscribe_in(
            ...
            window,
            |_this: &mut Self,
             _input: &Entity<dory_components::controls::InputState>,
    ...
    pub fn confirm_collection_chart_save(&mut self, cx: &mut Context<Self>) {
        ...
        let chart = match &self.source {
            DataSource::Collection {
                ...
                dory_components::saved_chart::SavedChart::new_collection(
                    name.clone(),
                    *profile_id,
                    collection.clone(),
                    time_window,
            ...
            DataSource::QueryResult {
                ...
                let Some(profile_id) = profile_id else {
                    self.pending.toast = Some(dory_ui_base::toast::PendingToast {
                ...
                dory_components::saved_chart::SavedChart::new_query(
                    name.clone(),
                    *profile_id,
        ...
        self.pending.toast = Some(match persist_result {
            Ok(_) => dory_ui_base::toast::PendingToast {
                message: crate::labels::chart_saved_toast(&name),
                is_error: false,
            },
            Err(e) => dory_ui_base::toast::PendingToast {
    ...
    pub fn set_chart_time_range_panel(
        &mut self,
        panel: Option<Entity<dory_components::common::time_range::view::TimeRangePanel>>,
        cx: &mut Context<Self>,
    ) {
    ...
    pub fn into_view_handle(
        entity: Entity<Self>,
        cx: &mut App,
    ) -> dory_components::result_panel::ViewHandle {
        use dory_components::result_panel::{SegmentPosition, ToolbarSegment, ViewHandle};
    ...
    fn build_visual_select(
        ...
        cx: &App,
    ) -> Result<Option<dory_core::SelectQuery>, String> {
        ...
        match generator.generate_select(spec) {
            ...
            Ok(None) => {}
            Err(dory_core::QueryGenError::Unsupported) => {}
            Err(error) => return Err(error.to_string()),
        }

        match generator.generate_read_from_spec(spec) {
            Ok(Some(generated)) => Ok(Some(dory_core::SelectQuery {
                sql: generated.text,
                params: Vec::new(),
            })),
            Ok(None) => Ok(None),
    ...
    pub(crate) fn compute_builder_binding(
        ...
        database: Option<&str>,
        cx: &App,
    ) -> Option<dory_core::EditableBinding> {
    ...
    pub fn open_query_builder(&mut self, window: &mut Window, cx: &mut Context<Self>) {
        ...
        let (profile_id, database, source) = match &self.source {
            DataSource::Table {
                ...
                let source = dory_core::SourceTable {
        ...
        let connection_arc: Option<std::sync::Arc<dyn dory_core::Connection>> = self
            .app_state
            .read(cx)
            .connections()
            .get(&profile_id)
        ...
        let generate_mutation_preview: Box<
            dyn Fn(&dory_core::VisualMutationSpec) -> String + Send + Sync,
        > = if let Some(conn) = connection_arc {
            Box::new(move |spec: &dory_core::VisualMutationSpec| {
            ...
            Box::new(|_spec: &dory_core::VisualMutationSpec| String::new())
        };

        let available_columns: Vec<String> =
            self.result.columns.iter().map(|c| c.name.clone()).collect();

        let column_kinds: std::collections::HashMap<String, dory_core::ColumnKind> = self
            .result
            .columns
            .iter()
    ...
    pub(crate) fn apply_fk_result(
        &mut self,
        foreign_keys: Vec<dory_core::SchemaForeignKeyInfo>,
        cx: &mut Context<Self>,
    ) {
    ...
    fn save_builder_query(&mut self, name: String, cx: &mut Context<Self>) {
        ...
        match result {
            Ok(summary) => {
                ...
                dory_ui_base::toast::Toast::success(crate::labels::saved_query_saved_as_toast(
                    &name,
                ))
                .meta_right(dory_ui_base::toast::now_hms())
                .push(cx);
            ...
            Err(e) => {
                dory_ui_base::user_error::report_error(
                    dory_ui_base::user_error::UserFacingError::new(
                        dory_ui_base::user_error::ErrorKind::Storage,
    ...
    fn import_builder_query(&mut self, source_id: String, cx: &mut Context<Self>) {
        use dory_ui_base::saved_query_manager::ConnectionTableProbe;
        ...
        let conn = {
            ...
            let Some(connected) = state.connections().get(&profile_id) else {
                dory_ui_base::user_error::report_error(
                    dory_ui_base::user_error::UserFacingError::new(
                        dory_ui_base::user_error::ErrorKind::User,
                        dory_i18n::t!(
        ...
        match result {
            Ok(_summary) => {
                dory_ui_base::toast::Toast::success(dory_i18n::t!(
            ...
            Err(e) => {
                dory_ui_base::user_error::report_error(
                    dory_ui_base::user_error::UserFacingError::new(
                        dory_ui_base::user_error::ErrorKind::User,
                        dory_i18n::t!("document.data.saved_query.error.import_failed"),
                    )
                    .with_cause(e.to_string()),
                    cx,
                );
    ...
    fn connection_generator<'a>(&self, cx: &'a App) -> Option<&'a dyn dory_core::QueryGenerator> {
        let profile_id = match &self.source {
            DataSource::Table { profile_id, .. } => *profile_id,
            _ => return None,
        };
    ...
    pub(crate) fn on_mutation_run_requested(
        ...
        match policy {
            MutationPolicy::ReadOnly => {
                dory_ui_base::user_error::report_error(
                    dory_ui_base::user_error::UserFacingError::new(
                        dory_ui_base::user_error::ErrorKind::User,
                        dory_i18n::t!("document.data.mutation.error.read_only_connection"),
                    ),
                    cx,
                );
                return;
            ...
            MutationPolicy::ApprovalRequired => {
                #[cfg(feature = "mcp")]
                {
                    ...
                    match enqueue_result {
                        Ok(_) => {
                            dory_ui_base::toast::Toast::info(dory_i18n::t!(
                ...
                #[cfg(not(feature = "mcp"))]
                {
                    dory_ui_base::user_error::report_error(
                        dory_ui_base::user_error::UserFacingError::new(
                            dory_ui_base::user_error::ErrorKind::User,
                            dory_i18n::t!("document.data.mutation.error.approval_requires_mcp"),
                        ),
                        cx,
                    );
                    return;
        ...
        let (sample_columns, sample_rows) =
            crate::data_grid_panel::mutation_confirm::fetch_sample_rows(
                connection,
                &spec,
                |warning| dory_ui_base::user_error::report_error(warning, cx),
            );
    ...
    fn handle_mutation_confirm_outcome(
        ...
        let (connection, event_sink, policy) = {
            ...
            let connected = match state.connections().get(&pending.profile_id) {
                ...
                None => {
                    dory_ui_base::user_error::report_error(
                        dory_ui_base::user_error::UserFacingError::new(
                            dory_ui_base::user_error::ErrorKind::Driver,
                            dory_i18n::t!("document.data.mutation.error.connection_not_found"),
                        ),
                        cx,
                    );
                    return;
            ...
            let event_sink: Option<Arc<dyn dory_core::EventSink>> =
                Some(Arc::new(state.audit_service().clone()) as Arc<dyn dory_core::EventSink>);
        ...
        #[cfg(feature = "mcp")]
        if matches!(policy, dory_core::MutationPolicy::ApprovalRequired) {
            use dory_core::MutationKind;
            use dory_policy::ExecutionClassification;
        ...
        if is_chunked {
            ...
            if pk_columns.is_empty() {
                dory_ui_base::user_error::report_error(
                    dory_ui_base::user_error::UserFacingError::new(
                        dory_ui_base::user_error::ErrorKind::User,
                        dory_i18n::t!("document.data.mutation.error.chunked_requires_primary_key"),
                    ),
                    cx,
                );
                return;
            ...
            // any reduction to the user via Toast while we still have a cx handle.
            {
                ...
                use dory_core::render_filter_node_sql;
                ...
                if max_params > 0 {
                    ...
                    let assignment_param_count = match &spec.kind {
                        dory_core::MutationKind::Update { assignments } => {
                            count_assignment_params(assignments)
                        }
                        dory_core::MutationKind::Delete => 0,
                    };
...
mod tests {
    ...
    use dory_ui_base::AppStateEntity;
    use dory_ui_base::toast::{ToastGlobal, ToastHost};
    ...
    fn make_grouped_spec() -> VisualQuerySpec {
        use dory_core::{AggFn, GroupByEntry, VisualAggregateSpec};
    ...
    fn context_menu_row_actions_field_stores_provider_actions() {
        use dory_core::InspectorRowAction;
        use std::sync::Arc;
    ...
    impl dory_core::Connection for StubBuilderConnection {
        ...
        ) -> Result<QueryResult, dory_core::DbError> {
            Err(dory_core::DbError::NotSupported("stub".to_string()))
        }
    ...
    fn register_builder_stub_connection(
        ...
        query_language: dory_core::QueryLanguage,
        query: Option<dory_core::QueryCapabilities>,
    ) -> (gpui::Entity<AppStateEntity>, Uuid) {
        init_test_runtime(cx);

        let metadata = dory_core::DriverMetadata {
            id: "stub-builder".to_string(),
            display_name: "Stub".to_string(),
            description: "test stub".to_string(),
            category,
            transfer_family: dory_core::TransferFamily::Incompatible,
            deployment_class: None,
            query_language,
            capabilities: dory_core::DriverCapabilities::empty(),
            default_port: None,
        ...
        cx.update(|cx| {
            app_state.update(cx, |app, _cx| {
                let profile = dory_core::ConnectionProfile::new(
                    "test",
                ...
                let connected = dory_core::ConnectedProfile {
                    profile,
    ...
    fn can_open_builder_true_for_document_collection_source(cx: &mut TestAppContext) {
        let (app_state, profile_id) = register_builder_stub_connection(
            ...
            dory_core::QueryLanguage::MongoQuery,
            Some(dory_core::QueryCapabilities::default()),
        );
    ...
    fn can_open_builder_false_for_keyvalue_collection_source(cx: &mut TestAppContext) {
        ...
        let no_predicate_caps = dory_core::QueryCapabilities {
            where_operators: Vec::new(),
            ..dory_core::QueryCapabilities::default()
        };
        let (app_state, profile_id) = register_builder_stub_connection(
            cx,
            dory_core::DatabaseCategory::KeyValue,
            dory_core::QueryLanguage::RedisCommands,
    ...
    fn can_open_builder_false_for_document_collection_without_predicates(cx: &mut TestAppContext) {
        let no_predicate_caps = dory_core::QueryCapabilities {
            where_operators: vec![dory_core::WhereOperator::And, dory_core::WhereOperator::Or],
            ..dory_core::QueryCapabilities::default()
        };
        let (app_state, profile_id) = register_builder_stub_connection(
            cx,
    ...
    fn fk_cache_lives_on_data_grid_panel(cx: &mut TestAppContext) {
        ...
        window.update(|_, app| {
            panel.update(app, |panel, cx| {
                ...
                let fk = dory_core::SchemaForeignKeyInfo {
    ...
    fn parse_failure_leaves_relational_state_inactive(cx: &mut TestAppContext) {
        ...
        window.update(|_, app| {
            panel.update(app, |panel, cx| {
                ...
                use dory_core::{DefaultSqlDialect, SourceTable, parse_and_resolve};
    ...
    fn h3_approval_required_does_not_open_confirmation_modal(cx: &mut TestAppContext) {
        ...
        impl Connection for StubSqlConnection2 {
            ...
            fn execute(&self, _req: &dory_core::QueryRequest) -> Result<CoreQueryResult, DbError> {
        ...
        let spec = dory_core::VisualMutationSpec {
            from: TableRef {
                schema: Some("public".to_string()),
                name: "orders".to_string(),
            },
            filter: None,
            kind: dory_core::MutationKind::Delete,
        };
    ...
    fn rebuild_table_marks_joined_cols_readonly_and_disables_insert(cx: &mut TestAppContext) {
        use dory_core::{ColumnOrigin, EditableBinding, TableRef as CoreTableRef};
    ...
    fn cold_cache_upgrades_to_editable_on_table_details_arrival(cx: &mut TestAppContext) {
        use dory_core::{ColumnInfo, TableInfo};
        ...
        cx.update(|cx| {
            app_state.update(cx, |app, _| {
                use dory_core::{ConnectedProfile, DbConfig, MutationPolicy};
                use std::path::PathBuf;
                ...
                let connected = ConnectedProfile {
                    ...
                    connection: {
                        ...
                        impl Connection for StubConn {
                            ...
                            fn execute(
                                &self,
                                _: &dory_core::QueryRequest,
                            ) -> Result<CoreQueryResult, DbError> {
                                Err(DbError::NotSupported("stub".to_string()))
                            }
                            fn cancel(&self, _: &dory_core::QueryHandle) -> Result<(), DbError> {
                                Ok(())
                            }
                            fn schema(&self) -> Result<SchemaSnapshot, DbError> {
                                Ok(SchemaSnapshot::default())
    ...
    fn builder_result_mutation_sets_pending_refresh_with_visual_select(cx: &mut TestAppContext) {
        ...
        let (_, window) = cx.add_window_view(|window, cx| {
            let panel = cx.new(|cx| {
                ...
                panel.builder.builder_editable_binding = Some(dory_core::EditableBinding {
    ...
    fn aliased_pk_builder_result_computes_no_editable_binding() {
        use dory_core::{ProjectedColumn, Projection, SourceTable, VisualQuerySpec};
    ...
    fn save_table_row_issues_one_update_for_editable_builder_result(cx: &mut TestAppContext) {
        use dory_components::components::data_table::model::CellValue;
        use dory_core::{
            ColumnKind, ColumnMeta, ConnectedProfile, DbConfig, MutationPolicy, TableRef,
        };
        use dory_test_support::fake_driver::{CrudOp, FakeDriver};
        use std::path::PathBuf;

        init_test_runtime(cx);

        let profile_id = Uuid::new_v4();
        let fake_driver = FakeDriver::new(dory_core::DbKind::SQLite);
