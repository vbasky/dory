...
use dory_components::chart::detect_chart_columns;
use dory_components::components::data_table::{ContextMenuAction, FilterOperator};
use dory_components::components::data_table::{HEADER_HEIGHT, ROW_HEIGHT};
use dory_components::icons::AppIcon;
use dory_components::primitives::{Icon, Text, overlay_bg, surface_panel, surface_raised};
...
impl DataGridPanel {
        ...
        match &self.source {
            DataSource::Table { profile_id, .. } => {
                let is_sql = self
                    ...
                    .get(profile_id)
                    .map(|c| {
                        c.connection.metadata().query_language == dory_core::QueryLanguage::Sql
                    })
                    .unwrap_or(false);
    ...
    fn mongo_filter_field_info(&self, _cx: &App) -> Option<(String, Value)> {
        use dory_components::components::document_tree::NodeValue;
                ...
                if let Some((row, position, action)) = pending_row_action {
                    ...
                    if menu.filter_submenu_open {
                        ...
                        let action = match menu.submenu_selected_index {
                            0 => ContextMenuAction::Order(dory_core::SortDirection::Ascending),
                            1 => ContextMenuAction::Order(dory_core::SortDirection::Descending),
    ...
    pub fn export_results(&mut self, window: &mut Window, cx: &mut Context<Self>) {
        ...
        {
            let message = dory_i18n::t!("document.data.context_menu.error.no_results_to_export");
        ...
        let _ = window;
        let _formats = dory_export::available_formats(&self.result.shape);
    ...
    pub fn export_with_format(
        ...
        cx.spawn(async move |_this, cx| {
            ...
            let export_result = (|| {
                ...
                dory_export::export(&result, format, &mut writer)?;
                Ok::<_, dory_export::ExportError>(())
            })();
    ...
    pub fn copy_to_clipboard_with_format(
        ...
        let export_result = dory_export::export(&self.result, format, &mut buffer);
    ...
    pub(super) fn open_row_inspector(&mut self, row: usize, col: usize, cx: &mut Context<Self>) {
        ...
        use dory_components::components::data_table::model::ColumnKind;
        ...
        let cells: Vec<InspectorCell> = model
            ...
            .map(|(ix, spec)| {
                let value = model
                    ...
                    .unwrap_or(dory_core::Value::Null);
    ...
    fn build_fk_references(
        ...
        use dory_components::primitives::LoadingState;
    ...
    /// Convert a `CellValue` to a `dory_core::Value` for the inspector.
    fn cell_to_value(
        &self,
        cell: &dory_components::components::data_table::model::CellValue,
    ) -> dory_core::Value {
        use dory_components::components::data_table::model::CellKind;

        match &cell.kind {
            CellKind::Null => dory_core::Value::Null,
            CellKind::Bool(b) => dory_core::Value::Bool(*b),
            CellKind::Int(i) => dory_core::Value::Int(*i),
            CellKind::Float(f) => dory_core::Value::Float(*f),
            CellKind::Text(s) | CellKind::Json(s) => dory_core::Value::Text(s.to_string()),
            CellKind::Bytes(len) => dory_core::Value::Bytes(vec![0u8; *len]),
            CellKind::AutoGenerated(s) => dory_core::Value::Text(s.to_string()),
            CellKind::Unsupported(s) => dory_core::Value::Text(s.to_string()),
        }
    }
        ...
        let row_values: Vec<String> = match visual_order.get(row).copied() {
            Some(VisualRowSource::Base(base_idx)) => self
                ...
                .map(|r| {
                    r.iter()
                        .map(|val| {
                            dory_components::components::data_table::clipboard::format_cell(
                                &dory_components::components::data_table::model::CellValue::from(
                                    val,
                                ),
                            )
                        })
            ...
            Some(VisualRowSource::Insert(insert_idx)) => buffer
                ...
                .map(|cells| {
                    cells
                        .iter()
                        .map(dory_components::components::data_table::clipboard::format_cell)
                        .collect()
                })
                .unwrap_or_default(),
            None => return,
    ...
    pub(super) fn handle_paste(&mut self, _window: &mut Window, cx: &mut Context<Self>) {
        ...
        table_state.update(cx, |state, cx| {
            if let Some(coord) = state.selection().active {
                let cell_value =
                    dory_components::components::data_table::model::CellValue::text(&text);
                state
                    .edit_buffer_mut()
    ...
    pub(super) fn handle_edit(
        ...
        if let Some(table_state) = &self.grid_table.table_state {
            table_state.update(cx, |state, cx| {
                let coord =
                    dory_components::components::data_table::selection::CellCoord::new(row, col);
    ...
    pub(super) fn handle_edit_in_modal(&mut self, row: usize, col: usize, cx: &mut Context<Self>) {
        use dory_components::components::data_table::model::{ColumnKind, VisualRowSource};
        ...
        let null_cell = dory_components::components::data_table::model::CellValue::null();
    ...
    pub(super) fn handle_set_default(&mut self, row: usize, col: usize, cx: &mut Context<Self>) {
        ...
        table_state.update(cx, |state, cx| {
            ...
            let cell_value = if let Some(default) = default_value {
                dory_components::components::data_table::model::CellValue::text(&default)
            } else {
                dory_components::components::data_table::model::CellValue::null()
            };
    ...
    pub(super) fn handle_set_null(&mut self, row: usize, col: usize, cx: &mut Context<Self>) {
        ...
        table_state.update(cx, |state, cx| {
            let buffer = state.edit_buffer_mut();
            let visual_order = buffer.compute_visual_order();
            let cell_value = dory_components::components::data_table::model::CellValue::null();
    ...
    pub(super) fn handle_cell_editor_save(
        ...
        table_state.update(cx, |state, cx| {
            let buffer = state.edit_buffer_mut();
            let visual_order = buffer.compute_visual_order();
            let cell_value = dory_components::components::data_table::model::CellValue::text(value);
    ...
    pub(super) fn handle_document_preview_save(
        ...
        use dory_components::modals::document_preview::DOC_INDEX_NEW;

        let new_doc: serde_json::Value = match serde_json::from_str(document_json) {
            Ok(v) => v,
            Err(e) => {
                let toast_body = e.to_string();
                let title = dory_i18n::t!("document.data.context_menu.error.invalid_json");
        ...
        let filter = if pk_columns.is_empty() {
            ...
            match new_doc.get("_id") {
                ...
                None => {
                    let message =
                        dory_i18n::t!("document.data.context_menu.error.document_missing_id");
                    Toast::error(message.clone())
                        .meta_right(now_hms())
                        .action(copy_action(message))
                        .push(cx);
            ...
            let Some(table_state) = &self.grid_table.table_state else {
                let message =
                    dory_i18n::t!("document.data.context_menu.error.table_state_not_available");
                Toast::error(message.clone())
                    .meta_right(now_hms())
                    .action(copy_action(message))
                    .push(cx);
            ...
            if filter_obj.is_empty() {
                let message =
                    dory_i18n::t!("document.data.context_menu.error.primary_key_not_determined");
                Toast::error(message.clone())
                    .meta_right(now_hms())
                    .action(copy_action(message))
                    .push(cx);
    ...
    pub(super) fn handle_add_row(
        ...
        let new_row: Vec<dory_components::components::data_table::model::CellValue> =
            if is_collection {
                self.result
                    .columns
                    .iter()
                    .map(|col| {
                        if col.is_primary_key {
                            ...
                            dory_components::components::data_table::model::CellValue::text(&new_id)
                        } else {
                            dory_components::components::data_table::model::CellValue::null()
                        }
                    })
                ...
                .map(|(idx, _)| {
                    if let Some(default_expr) = column_defaults.get(idx).and_then(|d| d.as_ref()) {
                        dory_components::components::data_table::model::CellValue::auto_generated(
                            default_expr,
                        )
                    } else {
                        dory_components::components::data_table::model::CellValue::null()
    ...
    pub(super) fn handle_duplicate_row(
        ...
        let (source_values, insert_after_base): (
            Vec<dory_components::components::data_table::model::CellValue>,
            usize,
                            ...
                            .map(|(idx, val)| {
                                ...
                                if pk_indices.contains(&idx) {
                                    ...
                                    if is_table {
                                        ...
                                        if let Some(default_expr) = column_defaults.get(idx).and_then(|d| d.as_ref()) {
                                            dory_components::components::data_table::model::CellValue::auto_generated(default_expr)
                                        } else {
                                            dory_components::components::data_table::model::CellValue::null()
                                        }
                                    } else {
                                        // For collections, generate new ID
                                        dory_components::components::data_table::model::CellValue::text(&new_id_for_column(col_name))
                                    }
                                } else {
                                    dory_components::components::data_table::model::CellValue::from(val)
                                }
                            })
                            .collect()
                    })
    ...
    fn handle_mongo_filter_by_value(
        ...
        field_value: &Option<dory_components::components::document_tree::NodeValue>,
        operator: FilterOperator,
        window: &mut Window,
        cx: &mut Context<Self>,
    ) {
    ...
    pub(super) fn handle_generate_sql(
        ...
        use dory_components::components::data_table::model::VisualRowSource;
        use dory_components::{SqlGenerationType, SqlPreviewContext};
    ...
    fn can_chart_from_context_menu(&self, _cx: &App) -> bool {
        ...
        matches!(
            ...
            dory_components::chart::ChartDetection::Ok { .. }
        )
    }
    ...
    fn build_sql_mutation(
        ...
        match action {
            ContextMenuAction::CopyAsInsert => {
                ...
                let assignments: Vec<dory_core::ColumnAssignment> = col_names
                    .iter()
                    .zip(row_values.iter())
                    .zip(col_types.iter())
                    .map(|((name, value), type_name)| dory_core::ColumnAssignment {
                        name: name.clone(),
                        value: value.clone(),
            ...
            ContextMenuAction::CopyAsUpdate => {
                ...
                let changes: Vec<dory_core::ColumnAssignment> = col_names
                    .into_iter()
                    .zip(row_values)
                    .zip(col_types)
                    .enumerate()
                    ...
                    .map(
                        |(_, ((name, value), type_name))| dory_core::ColumnAssignment {
                            name,
                            value,
