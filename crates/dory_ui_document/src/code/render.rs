...
use dory_components::composites::split_toolbar_action;
use dory_components::controls::Button;
use dory_components::helpers::text_color_for_active;
...
impl CodeDocument {
    fn render_toolbar(&self, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        let refresh_label = if auto_refresh_enabled {
            ...
            dory_i18n::t!("document.code.toolbar.refresh")
        };
        ...
        let (run_icon, run_label, run_enabled) = if is_executing {
            (
                AppIcon::X,
                dory_i18n::t!("document.code.toolbar.cancel"),
                true,
            )
        } else if is_preflight {
            (
                AppIcon::Loader,
                dory_i18n::t!("document.code.toolbar.checking"),
                false,
            )
        } else {
            (
                AppIcon::Play,
                dory_i18n::t!("document.code.toolbar.run"),
                true,
            )
        };
        ...
        compact_top_bar(&theme, std::iter::empty::<AnyElement>())
            ...
            .when(!is_read_only && is_db_language && !is_executing, |el| {
                el.child(
                    ToolbarButton::new("run-in-new-tab-btn")
                        ...
                        .label(dory_i18n::t!("document.code.toolbar.new_tab"))
                ...
                .child(
                    ToolbarButton::new("run-selection-btn")
                        ...
                        .label(dory_i18n::t!("document.code.toolbar.selection"))
                        .on_click(cx.listener(|this, _, window, cx| {
            ...
            .when(is_read_only, |el| {
                el.child(
                    Text::caption(dory_i18n::t!("document.code.toolbar.read_only"))
                        .muted_foreground(),
                )
            })
            ...
            .when(self.session.show_saved_label, |el| {
                el.child(Text::caption(dory_i18n::t!("document.code.toolbar.saved")))
            })
    }
            ...
            .when(!is_read_only, |el| {
                el.child(
                    ToolbarButton::new("toolbar-save-btn")
                        .icon(AppIcon::Save)
                        .tooltip(dory_i18n::t!("document.code.toolbar.save"))
                        .on_click(cx.listener(|this, _, window, cx| {
            ...
            .when(!is_read_only, |el| {
                el.child(
                    ToolbarButton::new("toolbar-format-btn")
                        .icon(AppIcon::Zap)
                        .tooltip(dory_i18n::t!("document.code.toolbar.formatter_unavailable"))
                        .disabled(true),
                )
            })
            ...
            .when(!is_read_only, |el| {
                el.child(
                    ToolbarButton::new("toolbar-history-btn")
                        ...
                        .tooltip(dory_i18n::t!("document.code.toolbar.query_history"))
                        .on_click(cx.listener(|this, _, window, cx| {
            ...
            .when(!is_read_only && is_db_language, |el| {
                el.child(
                    ToolbarButton::new("toolbar-explain-btn")
                        .icon(AppIcon::Info)
                        .tooltip(dory_i18n::t!("document.code.toolbar.explain_query"))
                        .on_click(cx.listener(|this, _, window, cx| {
            ...
            .when(!is_read_only, |el| {
                el.child(
                    ToolbarButton::new("toolbar-chart-btn")
                        ...
                        .tooltip(dory_i18n::t!("document.code.toolbar.open_in_chart"))
    ...
    fn render_live_output(&self, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        let status = if self.state == DocumentState::Executing {
            dory_i18n::t!("document.code.output.running")
        } else if live_output.is_finished() {
            dory_i18n::t!("document.code.output.stopped")
        } else {
            dory_i18n::t!("document.code.output.output")
        };
    ...
    fn render_loading_results(&self, _cx: &mut Context<Self>) -> impl IntoElement {
        ...
        div().p(Spacing::MD).size_full().child(
            BannerBlock::new(
                BannerVariant::Info,
                dory_i18n::t!("document.code.result.loading.title"),
            )
            .with_icon(icon)
            .with_body(dory_i18n::t!("document.code.result.loading.body")),
        )
    }
    ...
    fn render_error_state(&self, error: &str, _cx: &mut Context<Self>) -> impl IntoElement {
        ...
        div().p(Spacing::MD).size_full().overflow_y_hidden().child(
            BannerBlock::new(
                ...
                dory_i18n::t!("document.code.result.error.title"),
            )
            .with_icon(icon)
    ...
    fn render_empty_results(&self, _cx: &mut Context<Self>) -> impl IntoElement {
        div()
            ...
            .items_center()
            .justify_center()
            .child(Text::muted(dory_i18n::t!("document.code.result.empty")))
    }
    ...
    fn render_script_confirm_modal(&self, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        let footer = div()
            ...
            .child(
                Button::new(
                    ...
                    dory_i18n::t!("document.code.script_confirm.cancel"),
                )
            ...
            .child(
                Button::new(
                    ...
                    dory_i18n::t!("document.code.script_confirm.run"),
                )
        ...
        ModalShell::new(
            dory_i18n::t!("document.code.script_confirm.title"),
            body,
            footer,
        )
        .width(px(460.0))
    ...
    fn render_dangerous_query_modal(&self, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        let (title, message) = self
            ...
            .unwrap_or_else(|| {
                (
                    dory_i18n::t!("document.code.dangerous_query.fallback.title"),
                    dory_i18n::t!("document.code.dangerous_query.fallback.body"),
                )
            });
        ...
        let cancel_btn = Button::new(
            ...
            dory_i18n::t!("document.code.dangerous_query.cancel"),
        )
        .on_click(move |_, _, cx| {
            entity_cancel.update(cx, |doc, cx| {
        ...
        let run_anyway_btn = Button::new(
            ...
            dory_i18n::t!("document.code.dangerous_query.run_anyway"),
        )
        .danger()
        .on_click(move |_, window, cx| {
            entity_run.update(cx, |doc, cx| {
