...
impl CodeDocument {
    ...
    pub(super) fn schedule_diagnostic_refresh(&mut self, cx: &mut Context<Self>) {
        self.editor.diagnostic_request_id += 1;
        let request_id = self.editor.diagnostic_request_id;

        let entity = cx.entity().clone();
        self.editor._diagnostic_debounce = Some(cx.spawn(async move |_this, cx| {
            ...
            cx.update(|cx| {
                entity.update(cx, |this, cx| {
                    if this.editor.diagnostic_request_id != request_id {
                        return;
                    }
