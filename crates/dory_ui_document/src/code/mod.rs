...
use dory_components::modals::schema_drift::{
...
fn format_source_datetime_input(timestamp_ms: i64) -> String {
    dory_core::chrono::DateTime::from_timestamp_millis(timestamp_ms)
...
fn query_request_for_execution(
    ...
    let sql = dory_core::substitute_time_macros(&query, window, query_language);
...
struct PendingDriftQuery {
    ...
    cache_updates: Vec<(dory_core::TableKey, dory_core::TableInfo)>,
}
...
impl CodeDocument {
    ...
    pub fn new_with_language(
        ...
        let source_query_mode_dropdown = cx.new(|_cx| {
            Dropdown::new("ctx-source-query-mode")
                .placeholder(dory_i18n::t!("document.code.context_bar.fallback.syntax"))
                .toolbar_style(true)
        });
        ...
        let source_targets = cx.new(|_cx| {
            MultiSelect::new("ctx-source-targets")
                .bare()
                .placeholder(dory_i18n::t!("document.code.context_bar.fallback.sources"))
        });
    ...
    pub fn can_auto_refresh(&self, cx: &App) -> bool {
        dory_core::is_safe_read_query(&self.editor.input_state.read(cx).value())
    }
    ...
    /// Status-bar segments: cursor position and query dialect.
    pub fn status_segments(&self, cx: &App) -> Vec<crate::pane::StatusSegment> {
        let position = self.editor.input_state.read(cx).cursor_position();
        vec![
            crate::pane::StatusSegment {
                text: format!("Ln {}, Col {}", position.line + 1, position.character + 1).into(),
                tooltip: Some("Cursor position".into()),
            },
            crate::pane::StatusSegment {
                text: self.query_language().display_name().to_string().into(),
                tooltip: Some("Query language".into()),
            },
        ]
    }
    ...
    pub fn title(&self) -> String {
        if let Some(path) = &self.editor.path {
            let untitled = dory_i18n::t!("document.code.title.untitled");
            let name = path
                .file_name()
                .and_then(|n| n.to_str())
...
mod tests {
    ...
    use dory_core::{ExecutionSourceContext, QueryLanguage};
    ...
    fn code_title_untitled_differs_between_locales() {
        let en = dory_i18n::t!("document.code.title.untitled", locale = "en");
        let es = dory_i18n::t!("document.code.title.untitled", locale = "es");
