use super::*;
use dory_core::observability::actions as audit_actions;
...
fn resolve_source_context(
    ...
    fallback: Result<dory_core::ExecutionSourceContext, &'static str>,
) -> Result<dory_core::ExecutionSourceContext, &'static str> {
    if let Some((start_ms, end_ms)) = override_bounds {
        return Ok(dory_core::ExecutionSourceContext::CollectionWindow {
            targets,
            start_ms,
            end_ms,
            query_mode,
...
fn evaluate_dangerous_with_effective_settings(
    ...
    if effective.requires_preview {
        return dory_core::DangerousAction::Confirm(kind);
    }
    ...
    dory_core::DangerousAction::Confirm(kind)
}
...
impl CodeDocument {
    ...
    pub fn run_selected_query(&mut self, window: &mut Window, cx: &mut Context<Self>) {
        let Some(query) = self.selected_query(window, cx) else {
            Toast::warning(dory_i18n::t!("document.code.execution.toast.select_query"))
                .meta_right(now_hms())
                .push(cx);
            return;
        };
    ...
    fn run_query_text(
        ...
        if query.trim().is_empty() {
            Toast::warning(dory_i18n::t!("document.code.execution.toast.enter_query"))
                .meta_right(now_hms())
                .push(cx);
            return;
        }
    ...
    fn execute_query_internal(
        ...
        let Some(conn_id) = self.connection_id else {
            let msg = dory_i18n::t!("document.code.execution.toast.no_active_connection");
        ...
        let (connection, active_database, task_target) = {
            ...
            let Some(connected) = connections.get(&conn_id) else {
                let msg = dory_i18n::t!("document.code.execution.toast.connection_not_found");
                Toast::error(msg.clone())
                    .meta_right(now_hms())
                    .action(copy_action(msg))
                    .push(cx);
        ...
        let description = dory_core::truncate_string_safe(query.trim(), 80);
    ...
    pub(super) fn process_pending_auto_refresh(
        ...
        if !self.can_auto_refresh(cx) {
            self.refresh.refresh_policy = dory_core::RefreshPolicy::Manual;
            self.refresh._refresh_timer = None;
            self.refresh.refresh_dropdown.update(cx, |dd, cx| {
                dd.set_selected_index(Some(dory_core::RefreshPolicy::Manual.index()), cx);
            });
        ...
        match pending.result {
            ...
            Err(e) => {
                ...
                let title: SharedString = if is_script {
                    dory_i18n::t!("document.code.execution.result_title.script_failed").into()
                } else {
                    dory_i18n::t!("document.code.execution.result_title.query_failed").into()
                };
                let now = dory_core::chrono::Local::now()
                    .format("%H:%M:%S")
                    .to_string();
                ...
                let mut toast = match e.formatted() {
                    Some(f) => {
                        let mut t = dory_ui_base::toast::Toast::error(title)
                            .meta_right(now)
                    ...
                    None => dory_ui_base::toast::Toast::error(title)
                        .meta_right(now)
                        .body(error_msg.clone()),
                };

                toast = toast.action(
                    dory_ui_base::toast::ToastAction::new(
                        "copy-error",
                        dory_i18n::t!("document.code.execution.copy_error"),
                    )
                    .primary()
    ...
    fn run_script(&mut self, _window: &mut Window, cx: &mut Context<Self>) {
        use dory_app::hook_executor::CompositeExecutor;
        use dory_core::{
        ...
        let (output_sender, output_receiver) = dory_core::output_channel();
        let (task_id, cancel_token) =
            self.runner
                .start_primary(dory_core::TaskKind::Query, description, cx);
        ...
        let task = cx.background_executor().spawn(async move {
            ...
            match result {
                Ok(hook_result) => {
                    ...
                    Ok(QueryResult {
                        shape: dory_core::QueryResultShape::Text,
                        columns: Vec::new(),
                        rows: Vec::new(),
                        affected_rows: None,
...
mod tests {
    ...
    use dory_core::{ExecutionContext, ExecutionSourceContext, QueryLanguage};
    use uuid::Uuid;
