...
//!
//! This file lives in `dory_ui` (not `dory_components`) to avoid circular
//! dependencies: `DataView` references `ContextId` and `ResultViewMode`, which
//! are defined in `dory_ui` and would require `dory_components` to depend
//! on `dory_ui` if moved there.
//!
