...
impl LuaEngine {
    pub fn create_vm(config: LuaVmConfig<'_>) -> LuaResult<LuaVm> {
        ...
        if capabilities.logging {
            api::dory::register_logging_api(&lua, state.clone())?;
        }

        if capabilities.env_read {
            api::dory::register_env_api(&lua)?;
        }

        if capabilities.process_run {
            api::dory::register_process_api(&lua, state.clone())?;
        }
...
mod tests {
    use super::*;
    use dory_core::{CancelToken, HookPhase};
    use uuid::Uuid;
    ...
    fn capabilities_hide_optional_apis() {
        ...
        let logging_is_nil: bool = vm
            .lua
            .load("return dory == nil or dory.log == nil")
            .eval()
            .unwrap();
        let env_is_nil: bool = vm
            .lua
            .load("return dory == nil or dory.env == nil")
            .eval()
            .unwrap();
    ...
    fn logging_and_env_api_are_available_when_enabled() {
        ...
        let has_logging: bool = vm
            .lua
            .load("return dory ~= nil and dory.log ~= nil")
            .eval()
            .unwrap();
        let has_env: bool = vm
            .lua
            .load("return dory ~= nil and dory.env ~= nil")
            .eval()
            .unwrap();
        let path_value: Option<String> = vm.lua.load("return dory.env.get('PATH')").eval().unwrap();
    ...
    fn env_api_is_hidden_by_default() {
        ...
        assert!(
            env_is_nil,
            "env_read defaults to false; dory.env must be nil"
        );
    }
    ...
    fn process_api_is_hidden_when_capability_is_disabled() {
        ...
        let has_process: bool = vm
            .lua
            .load("return dory == nil or dory.process ~= nil")
            .eval()
            .unwrap();
    ...
    fn process_api_is_available_when_capability_is_enabled() {
        ...
        let has_process: bool = vm
            .lua
            .load("return dory ~= nil and dory.process ~= nil and dory.process.run ~= nil")
            .eval()
            .unwrap();
