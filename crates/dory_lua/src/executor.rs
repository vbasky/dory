...
impl HookExecutor for LuaExecutor {
    fn execute_hook(
        ...
        let script_content = match source {
            dory_core::ScriptSource::Inline { content } => content.clone(),
            dory_core::ScriptSource::File { path } => {
...
mod tests {
    ...
    fn executes_file_backed_lua_hook() {
        let result = LuaExecutor::new()
            .execute_hook(
                &file_lua_hook("dory.log.info('from-file')"),
                &test_context(),
                &CancelToken::new(),
                None,
                None,
        ...
        let result = LuaExecutor::new()
            .execute_hook(
                &lua_hook_with_capabilities(
                    &format!(
                        "local result = dory.process.run({{ program = '{python}', allowlist = 'python_cli', timeout_ms = 5000, args = {{'-c', 'print(\"hello-process\")'}} }})\nif not result.ok then hook.fail(result.stderr) end"
                    ),
                    LuaCapabilities {
                        process_run: true,
    ...
    fn controlled_process_run_is_unavailable_without_capability() {
        let result = LuaExecutor::new()
            .execute_hook(
                &lua_hook(
                    "return dory.process.run({ program = 'python3', allowlist = 'python_cli' })",
                ),
                &test_context(),
                &CancelToken::new(),
                None,
    ...
    fn controlled_process_run_rejects_unknown_allowlist() {
        let result = LuaExecutor::new()
            .execute_hook(
                &lua_hook_with_capabilities(
                    "return dory.process.run({ program = 'python3', allowlist = 'nope' })",
                    LuaCapabilities {
                        process_run: true,
                        ..LuaCapabilities::default()
                    },
    ...
    fn controlled_process_run_rejects_program_outside_allowlist() {
        let result = LuaExecutor::new()
            .execute_hook(
                &lua_hook_with_capabilities(
                    "return dory.process.run({ program = 'python3', allowlist = 'aws_cli' })",
                    LuaCapabilities {
                        process_run: true,
                        ..LuaCapabilities::default()
                    },
    ...
    fn controlled_process_run_reports_process_timeout() {
        ...
        let result = LuaExecutor::new()
            .execute_hook(
                &lua_hook_with_capabilities(
                    &format!(
                        "local result = dory.process.run({{ program = '{python}', allowlist = 'python_cli', timeout_ms = 10, args = {{'-c', 'import time; time.sleep(10)'}} }})\nif not result.timed_out then hook.fail('expected process timeout') end"
                    ),
                    LuaCapabilities {
                        process_run: true,
                        ..LuaCapabilities::default()
    ...
    fn lua_executor_streams_logs_and_process_output() {
        ...
        let hook = lua_hook_with_capabilities(
            &format!(
                "dory.log.info('hello-log')\nlocal result = dory.process.run({{ program = '{python}', allowlist = 'python_cli', timeout_ms = 5000, stream = true, args = {{'-c', 'print(\"hello-stream\")'}} }})\nif not result.ok then hook.fail(result.stderr) end"
            ),
            LuaCapabilities {
                process_run: true,
                ..LuaCapabilities::default()
    ...
    fn lua_executor_only_streams_process_output_when_requested() {
        ...
        let hook = lua_hook_with_capabilities(
            &format!(
                "dory.log.info('hello-log')\nlocal result = dory.process.run({{ program = '{python}', allowlist = 'python_cli', timeout_ms = 5000, args = {{'-c', 'print(\"hello-buffered\")'}} }})\nif not result.ok then hook.fail(result.stderr) end"
            ),
            LuaCapabilities {
                process_run: true,
                ..LuaCapabilities::default()
    ...
    fn controlled_process_run_can_detach_explicitly() {
        ...
        let hook = lua_hook_with_capabilities(
            &format!(
                "local result = dory.process.run({{ program = '{python}', allowlist = 'python_cli', detached = true, args = {{'-c', 'import time; time.sleep(0.1)'}} }})\nif not result.detached then hook.fail('expected detached process') end"
            ),
            LuaCapabilities {
                process_run: true,
                ..LuaCapabilities::default()
