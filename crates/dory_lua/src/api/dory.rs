...
pub fn register_logging_api(lua: &Lua, state: LuaRuntimeState) -> LuaResult<()> {
    let dory = ensure_dory_table(lua)?;
    ...
    dory.set("log", logging)
}

/// IPC auth-token env-var names that must never be returned to Lua scripts, even
/// when env_read is enabled. These map to dory_ipc::{DRIVER_RPC_AUTH_TOKEN_ENV,
/// APP_CONTROL_AUTH_TOKEN_ENV, AUTH_PROVIDER_RPC_AUTH_TOKEN_ENV}.
const BLOCKED_ENV_VARS: &[&str] = &[
    "DORY_DRIVER_IPC_TOKEN",
    "DORY_IPC_TOKEN",
    "DORY_AUTH_PROVIDER_IPC_TOKEN",
];
...
pub fn register_process_api(lua: &Lua, state: LuaRuntimeState) -> LuaResult<()> {
    ...
    dory.set("process", process)
}

fn ensure_dory_table(lua: &Lua) -> LuaResult<Table> {
    let globals = lua.globals();

    if let Ok(existing) = globals.get::<Table>("dory") {
        return Ok(existing);
    }

    let dory = lua.create_table()?;
    globals.set("dory", dory.clone())?;
    Ok(dory)
}
...
fn run_process(lua: &Lua, state: &LuaRuntimeState, options: Table) -> LuaResult<Table> {
    ...
    if !detached && timeout.is_none() && state.hook_timeout.is_none() {
        return Err(mlua::Error::RuntimeError(
            "dory.process.run requires a timeout_ms when no hook-level timeout is set".to_string(),
        ));
    }
...
fn ensure_program_allowed(program: &str, allowlist: &str) -> LuaResult<()> {
    ...
    let Some(allowed_programs) = allowlist_programs(allowlist) else {
        return Err(mlua::Error::RuntimeError(format!(
            "dory.process.run allowlist '{allowlist}' is not recognized"
        )));
    };
...
fn read_required_string(options: &Table, key: &str) -> LuaResult<String> {
    match options.get::<Value>(key)? {
        ...
        Value::Nil => Err(mlua::Error::RuntimeError(format!(
            "dory.process.run requires '{key}'"
        ))),
        _ => Err(mlua::Error::RuntimeError(format!(
            "dory.process.run field '{key}' must be a string"
        ))),
    }
}
...
fn read_optional_u64(options: &Table, key: &str) -> LuaResult<Option<u64>> {
    match options.get::<Value>(key)? {
        ...
        Value::Nil => Ok(None),
        _ => Err(mlua::Error::RuntimeError(format!(
            "dory.process.run field '{key}' must be a non-negative integer"
        ))),
    }
}
...
fn read_optional_bool(options: &Table, key: &str) -> LuaResult<Option<bool>> {
    match options.get::<Value>(key)? {
        ...
        Value::Nil => Ok(None),
        _ => Err(mlua::Error::RuntimeError(format!(
            "dory.process.run field '{key}' must be a boolean"
        ))),
    }
}
...
fn read_string_list(options: &Table, key: &str) -> LuaResult<Vec<String>> {
    match options.get::<Value>(key)? {
        ...
        _ => Err(mlua::Error::RuntimeError(format!(
            "dory.process.run field '{key}' must be an array of strings"
        ))),
    }
}
...
mod tests {
    ...
    fn test_state(
        ...
        output: Option<dory_core::OutputSender>,
    ) -> LuaRuntimeState {
        LuaRuntimeState {
            ...
            output,
            detached: None,
            cancel_token: dory_core::CancelToken::new(),
    ...
    fn append_log_keeps_buffer_plain_but_streams_newline() {
        let (sender, receiver) = dory_core::output_channel();
    ...
    // Streaming of a child's stdout/stderr is covered directly in dory_core by
    ...
    fn env_get_blocks_driver_ipc_token_exact_case() {
        ...
        unsafe { std::env::set_var("DORY_DRIVER_IPC_TOKEN", "should-be-blocked") };
        let lua = make_env_lua();
        let value: Option<String> = lua
            .load("return dory.env.get('DORY_DRIVER_IPC_TOKEN')")
            .eval()
            .unwrap();
        unsafe { std::env::remove_var("DORY_DRIVER_IPC_TOKEN") };
        assert!(
            value.is_none(),
            "DORY_DRIVER_IPC_TOKEN must be blocked even when env_read is enabled"
        );
    }

    #[test]
    fn env_get_blocks_ipc_token_case_insensitive() {
        unsafe { std::env::set_var("DORY_IPC_TOKEN", "should-be-blocked") };
        let lua = make_env_lua();
        let lower: Option<String> = lua
            .load("return dory.env.get('dory_ipc_token')")
            .eval()
            .unwrap();
        let upper: Option<String> = lua
            ...
            .eval()
            .unwrap();
        unsafe { std::env::remove_var("DORY_IPC_TOKEN") };
    ...
    fn env_get_blocks_auth_provider_ipc_token() {
        unsafe { std::env::set_var("DORY_AUTH_PROVIDER_IPC_TOKEN", "should-be-blocked") };
        let lua = make_env_lua();
        let value: Option<String> = lua
            .load("return dory.env.get('DORY_AUTH_PROVIDER_IPC_TOKEN')")
            .eval()
            .unwrap();
        unsafe { std::env::remove_var("DORY_AUTH_PROVIDER_IPC_TOKEN") };
        assert!(
            value.is_none(),
            "DORY_AUTH_PROVIDER_IPC_TOKEN must be blocked"
        );
    }

    #[test]
    fn env_get_allows_unrelated_vars() {
        unsafe { std::env::set_var("DORY_TEST_SAFE_VAR_12345", "visible") };
        let lua = make_env_lua();
        let value: Option<String> = lua
            .load("return dory.env.get('DORY_TEST_SAFE_VAR_12345')")
            .eval()
            .unwrap();
        unsafe { std::env::remove_var("DORY_TEST_SAFE_VAR_12345") };
