...
AWS DynamoDB driver for Dory, built on the [`aws-sdk-dynamodb`](https://crates.io/crates/aws-sdk-dynamodb) SDK.
...
- Table discovery with `ListTables` and `DescribeTable`, mapping partition key (PK), sort key (SK), and Global/Local Secondary Index (GSI/LSI) key metadata into Dory schema abstractions.
