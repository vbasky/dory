...
fn plan_dynamo_mutation(mutation: &dory_core::MutationRequest) -> Result<SemanticPlan, DbError> {
...
pub static DYNAMODB_METADATA: LazyLock<DriverMetadata> = LazyLock::new(|| DriverMetadata {
    ...
    query: Some(QueryCapabilities {
        ...
        order_by_mode: dory_core::OrderByMode::SortKeyOnly,
...
impl Connection for DynamoConnection {
        ...
        let mut result = match envelope {
            ...
            DynamoCommandEnvelope::Update {
                ...
                let update_request = DocumentUpdate {
                    ...
                    filter: dory_core::DocumentFilter { filter: key },
                    update,
                    many,
                    upsert,
                };
    ...
    fn insert_document(&self, insert: &DocumentInsert) -> Result<dory_core::CrudResult, DbError> {
    ...
    fn update_document(&self, update: &DocumentUpdate) -> Result<dory_core::CrudResult, DbError> {
    ...
    fn delete_document(&self, delete: &DocumentDelete) -> Result<dory_core::CrudResult, DbError> {
        let config = DynamoProfileConfig {
            region: self.default_region.clone(),
            profile: None,
            endpoint: None,
...
fn crud_result_with_affected_rows(affected_rows: usize) -> dory_core::CrudResult {
    dory_core::CrudResult::new(affected_rows as u64, None)
}
...
fn crud_result_to_query_result(result: dory_core::CrudResult) -> QueryResult {
...
mod tests {
    ...
    fn connect_and_test_connection_succeed_against_local_endpoint() {
        let endpoint = match std::env::var("DORY_DYNAMODB_TEST_ENDPOINT") {
    ...
    fn attribute_value_conversion_round_trips_nested_json_shapes() {
        ...
        match converted_map {
            dory_core::Value::Document(map) => {
                assert_eq!(
                    map.get("pk"),
                    Some(&dory_core::Value::Text("USER#1".to_string()))
                );
                assert_eq!(map.get("count"), Some(&dory_core::Value::Int(42)));
            }
            other => panic!("expected document value, got {other:?}"),
        }
    }
    ...
    fn semantic_planner_rejects_aggregate_requests_explicitly() {
        let error = plan_dynamo_semantic_request(&SemanticRequest::Aggregate(
            dory_core::AggregateRequest::new(dory_core::TableRef::new("users")),
        ))
    ...
    fn dynamodb_export_hint_profile_is_required_on_import() {
        ...
        assert_eq!(
            ...
            dory_core::ExportFieldHint::RequiredOnImport,
