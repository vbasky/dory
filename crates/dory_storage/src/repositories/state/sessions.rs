//! Repository for session and tab metadata in dory.db.
//!
//! Session metadata (which tabs are open, their kind, paths, positions, active index)
//! lives in `dory.db`. Actual file content (scratch files, shadow files) stays on disk
//! in `~/.local/share/dory/st_sessions/` via `ArtifactStore`.
//!
...
/// Session repository — manages session and tab metadata in dory.db.
            ...
            tabs: session
                ...
                .map(|tab| {
                    let exec_ctx_json = tab
                        ...
                        .clone()
                        .filter(|json| {
                            serde_json::from_str::<dory_core::ExecutionContext>(json).is_ok()
                        })
                        .unwrap_or_else(|| {
                            let exec_ctx = dory_core::ExecutionContext {
                                connection_id: tab
                                    .exec_ctx_connection_id
                                    .as_ref()
...
/// A session manifest restored from dory.db.
///
...
///
/// This mirrors the old `SessionManifest` type from `dory_core::storage::session`.
#[derive(Debug, Clone, Serialize, Deserialize)]
...
pub struct WorkspaceTab {
    ...
    pub tab_kind: String,
    pub language: String,
    pub exec_ctx: dory_core::ExecutionContext,
    pub scratch_path: Option<std::path::PathBuf>,
    pub shadow_path: Option<std::path::PathBuf>,
    pub file_path: Option<std::path::PathBuf>,
    pub title: String,
...
mod tests {
    ...
    fn save_and_restore_workspace_session() {
        ...
        let manifest = WorkspaceSessionManifest {
            ...
            tabs: vec![WorkspaceTab {
                ...
                tab_kind: "Scratch".to_string(),
                language: "sql".to_string(),
                exec_ctx: dory_core::ExecutionContext::default(),
    ...
    fn save_and_restore_file_backed_tab() {
        ...
        let exec_ctx_restored: dory_core::ExecutionContext =
    ...
    fn exec_context_roundtrip_in_restore_payload() {
        ...
        let restored_ctx: dory_core::ExecutionContext =
    ...
    fn cloudwatch_exec_context_roundtrips_per_document() {
        ...
        let manifest = WorkspaceSessionManifest {
            ...
            tabs: vec![
                WorkspaceTab {
                    ...
                    tab_kind: "Scratch".to_string(),
                    language: "sql".to_string(),
                    exec_ctx: dory_core::ExecutionContext {
                        connection_id: Some(profile_id),
                        database: Some("logs".into()),
                        schema: None,
                        container: None,
                        source: Some(dory_core::ExecutionSourceContext::CollectionWindow {
        ...
        let artifact_root = std::env::temp_dir().join(format!(
            "dory_test_cloudwatch_exec_ctx_{}_{}",
            std::process::id(),
            Uuid::new_v4()
        ));
        let store = ArtifactStore::for_root(artifact_root.clone()).expect("store");
        ...
        let restored_contexts = restored
            ...
            .iter()
            .map(|tab| {
                serde_json::from_str::<dory_core::ExecutionContext>(&tab.exec_ctx_json)
                    .expect("exec ctx json")
            })
            .collect::<Vec<_>>();

        match &restored_contexts[0].source {
            Some(dory_core::ExecutionSourceContext::CollectionWindow {
                targets,
                start_ms,
                end_ms,
                query_mode,
    ...
    fn restore_prefers_exec_ctx_json_over_legacy_columns() {
        ...
        let exec_ctx_json = serde_json::to_string(&dory_core::ExecutionContext {
            connection_id: Some(profile_id),
            database: Some("logs".into()),
            schema: None,
            container: None,
        ...
        let artifact_root = std::env::temp_dir().join(format!(
            "dory_test_cloudwatch_json_restore_{}_{}",
            std::process::id(),
            Uuid::new_v4()
        ));
        let store = ArtifactStore::for_root(artifact_root.clone()).expect("store");
