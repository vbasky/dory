...
impl AuditRepository {
    ...
    /// same `dory.db` connection pool, so no additional connection is needed.
    ...
    pub fn try_record_panic(
        ...
        match self.conn.try_lock() {
            ...
            Err(TryLockError::Poisoned(_)) => {
                ...
                eprintln!(
                    "[dory_audit] panic event (lock poisoned): {}",
                    panic_info_str
                );
                Ok(None)
            }
...
#[cfg(test)]
mod tests {
    use dory_core::observability::actions::{CONFIG_CHANGE, QUERY_EXECUTE};
    ...
    fn temp_db(name: &str) -> std::path::PathBuf {
        let path =
            std::env::temp_dir().join(format!("dory_repo_audit_{}_{}", name, std::process::id()));
