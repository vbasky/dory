//! Repository for SSH tunnel profiles in dory.db.
//!
...
mod tests {
    ...
    fn temp_db(name: &str) -> std::path::PathBuf {
        std::env::temp_dir().join(format!("dory_repo_ssh_{}_{}", name, std::process::id()))
    }
    ...
    fn ssh_insert_with_key_auth() {
        ...
        let auth_dto = SshTunnelAuthDto {
            ...
            password_secret_ref: None,
            passphrase_secret_ref: Some("dory:secret:ssh:passphrase:test".to_string()),
        };
    ...
    fn ssh_upsert_updates_auth() {
        ...
        let updated_auth = SshTunnelAuthDto {
            ssh_tunnel_profile_id: dto.id.clone(),
            key_path: None,
            password_secret_ref: Some("dory:secret:ssh:password:test".to_string()),
            passphrase_secret_ref: None,
        };
