...
impl From<StorageError> for dory_core::DbError {
    fn from(err: StorageError) -> Self {
        match err {
            StorageError::Sqlite { source, .. } => {
                dory_core::DbError::query_failed(source.to_string())
            }
            StorageError::Io { source, .. } => dory_core::DbError::IoError(source),
            StorageError::DataDirUnavailable => {
                dory_core::DbError::InvalidProfile("data directory not available".to_string())
            }
            StorageError::Data(msg) => dory_core::DbError::InvalidProfile(msg),
            StorageError::Migration { kind, details } => dory_core::DbError::InvalidProfile(
