...
//! crash dumps) lives on disk in `~/.local/share/dory/` subdirectories.
...
/// Manages filesystem artifact paths for Dory session content.
///
...
pub struct ArtifactStore {
    /// Root directory for session artifacts (e.g. `~/.local/share/dory/sessions/`).
    root: PathBuf,
}

impl ArtifactStore {
    /// Creates an `ArtifactStore` for the default Dory data directory.
    ///
    /// Calls `paths::data_dir()` which resolves `~/.local/share/dory/`.
