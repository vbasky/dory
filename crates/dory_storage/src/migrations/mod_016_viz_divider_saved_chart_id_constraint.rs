//! Migration 016: Tighten the divider-panel CHECK constraint.
//!
//! Migration 014 allowed any `saved_chart_id` value for divider rows. A
//! divider with a non-empty `saved_chart_id` could collide with chart dedup
//! in `reconcile_panels_from_manager`. This migration rebuilds
//! `viz_dashboard_panels` with the divider arm extended to also reject
//! non-empty `saved_chart_id` values.
//!
//! While rebuilding, the inspector arm is also cleaned up: the unreachable
//! `OR saved_chart_id IS NULL` branch from migration 015 is removed since
//! `saved_chart_id TEXT NOT NULL` makes NULL impossible.
...
CREATE TABLE viz_dashboard_panels__016 (
    ...
    CHECK (
        ...
        (panel_kind = 'divider'   AND saved_chart_id = ''   AND divider_markdown IS NOT NULL AND inspector_metric_id IS NULL) OR
        (panel_kind = 'inspector' AND saved_chart_id = ''   AND divider_markdown IS NULL AND inspector_metric_id IS NOT NULL)
    )
);

INSERT INTO viz_dashboard_panels__016 (
...
ALTER TABLE viz_dashboard_panels__016 RENAME TO viz_dashboard_panels;
...
mod tests {
    ...
    /// L12: Divider rows with a non-empty saved_chart_id must be rejected.
    #[test]
    fn divider_panel_with_saved_chart_id_rejected() {
        let conn = setup();
        ...
        let result = conn.execute(
            "INSERT INTO viz_dashboard_panels \
             (dashboard_id, panel_index, panel_kind, saved_chart_id, divider_markdown, \
              grid_row, grid_column, grid_width, grid_height) \
             VALUES (?1, 0, 'divider', 'non-empty-chart-id', '## Section', 0, 0, 12, 1)",
            rusqlite::params![dashboard_id],
        );

        assert!(
            result.is_err(),
            "divider row with non-empty saved_chart_id must be rejected by CHECK constraint"
        );
    }

    /// L12: Divider rows with an empty saved_chart_id must be accepted.
    #[test]
    fn divider_panel_with_empty_saved_chart_id_accepted() {
        let conn = setup();
        let profile_id = insert_profile(&conn);
        let dashboard_id = insert_dashboard(&conn, &profile_id);
        ...
        conn.execute(
            "INSERT INTO viz_dashboard_panels \
             (dashboard_id, panel_index, panel_kind, saved_chart_id, divider_markdown, \
              grid_row, grid_column, grid_width, grid_height) \
             VALUES (?1, 0, 'divider', '', '## Section', 0, 0, 12, 1)",
            rusqlite::params![dashboard_id],
        )
        .expect("divider row with empty saved_chart_id must be accepted");
    }

    /// L13: Inspector rows with a non-empty saved_chart_id must be rejected
    /// (clean CHECK without the dead IS NULL branch).
    #[test]
        ...
        let result = conn.execute(
            "INSERT INTO viz_dashboard_panels \
             (dashboard_id, panel_index, panel_kind, saved_chart_id, inspector_metric_id, \
              grid_row, grid_column, grid_width, grid_height) \
             VALUES (?1, 0, 'inspector', 'some-chart-id', 'some.metric', 0, 0, 12, 2)",
            rusqlite::params![dashboard_id],
        );
    ...
    /// Inspector rows with an empty saved_chart_id must be accepted.
    #[test]
    fn inspector_panel_with_empty_saved_chart_id_accepted() {
        ...
        conn.execute(
            "INSERT INTO viz_dashboard_panels \
             (dashboard_id, panel_index, panel_kind, saved_chart_id, inspector_metric_id, \
              grid_row, grid_column, grid_width, grid_height) \
             VALUES (?1, 0, 'inspector', '', 'some.metric', 0, 0, 12, 2)",
            rusqlite::params![dashboard_id],
        )
