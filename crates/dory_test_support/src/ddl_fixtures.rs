use dory_core::{DbError, QueryRequest};
...
pub fn seed_table<C: dory_core::Connection + ?Sized>(
    conn: &C,
    table_name: &str,
    rows: Vec<HashMap<String, dory_core::Value>>,
) -> Result<(), DbError> {
    for row in rows {
        let columns: Vec<String> = row.keys().cloned().collect();
        let values: Vec<dory_core::Value> = columns.iter().map(|k| row[k].clone()).collect();
...
pub fn cleanup_table<C: dory_core::Connection + ?Sized>(
    conn: &C,
    table_name: &str,
) -> Result<(), DbError> {
...
pub fn cleanup_view<C: dory_core::Connection + ?Sized>(
    conn: &C,
    view_name: &str,
) -> Result<(), DbError> {
...
pub fn cleanup_index<C: dory_core::Connection + ?Sized>(
    conn: &C,
    index_name: &str,
    dialect: &str,
) -> Result<(), DbError> {
