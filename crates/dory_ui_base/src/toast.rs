...
use dory_components::primitives::{Icon, IconButton, Text};
...
pub fn copy_action(payload: impl Into<String>) -> ToastAction {
    ...
    ToastAction::new("copy-error", dory_i18n::t!("toast.action.copy")).on_click(
        move |cx: &mut App| {
...
mod i18n_tests {
    ...
    fn toast_copy_label_differs_between_locales() {
        let english = dory_i18n::t!("toast.action.copy", locale = "en");
        let spanish = dory_i18n::t!("toast.action.copy", locale = "es");
    ...
    fn render_toast(
        ...
        if can_collapse {
            let label: String = if is_collapsed {
                dory_i18n::t!("toast.action.show_details")
            } else {
                dory_i18n::t!("toast.action.hide_details")
            };
            let toggle = gpui::div()
