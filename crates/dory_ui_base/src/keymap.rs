//! Keymap helpers that depend on both GPUI and `dory_app::keymap`.
//!
//! These helpers live in `dory_ui_base` rather than `dory_components`
//! because they require `dory_app::keymap` types, which `dory_components`
//! intentionally does not depend on.

use dory_app::keymap::{Command, ContextId, KeymapLayer};
use dory_app::keymap::{KeyChord, KeymapStack, Modifiers};
use gpui::Keystroke;
...
fn global_layer() -> KeymapLayer {
    ...
    // Settings — Cmd+, on macOS, Ctrl+, elsewhere (Zed / VS Code).
    layer.bind(
...
fn results_layer() -> KeymapLayer {
    ...
    layer.bind(KeyChord::new("/", Modifiers::none()), Command::FocusSearch);
    layer.bind(
        KeyChord::new("f", Modifiers::primary()),
        Command::FocusSearch,
    );
...
mod tests {
    ...
    fn test_default_keymap_resolves_global() {
        ...
        let settings = KeyChord::new(",", Modifiers::primary());
        assert_eq!(
            keymap.resolve(ContextId::Global, &settings),
