...
#[cfg(target_os = "linux")]
use dory_components::tokens::ChromeColors;
#[cfg(target_os = "linux")]
...
/// after the app name: `Dory  ›  {crumb1}  ›  {crumb2}`.
...
/// Backward-compatible alias: renders the title bar with a fixed "Dory" title.
...
pub fn render_linux_title_bar(window: &mut Window, cx: &mut App) -> impl IntoElement + 'static {
    match render_csd_title_bar(window, cx, "Dory") {
...
/// Applies standard Dory window options for secondary windows (Settings, Connection
