...
use dory_components::composites::ModalFrame as ComponentModalFrame;
...
///
/// Keep the existing `dory_ui` builder surface during migration, but forward
/// all shared modal chrome to `dory_components` so overlays stop owning a
...
mod tests {
    ...
    const UI_DIR: &str = concat!(env!("CARGO_MANIFEST_DIR"), "/../dory_ui/src/ui");
    ...
    fn modal_frame_delegates_to_component_owned_implementation() {
        ...
        assert!(
            ...
            "modal_frame.rs must delegate to dory_components ComponentModalFrame"
        );
    }
    ...
    fn overlay_call_sites_import_modal_frame_directly_from_ui_base() {
        for path in ["overlays/login_modal.rs"] {
            ...
            assert!(
                source.contains("dory_ui_base::modal_frame::ModalFrame"),
                "{path} must import ModalFrame directly from dory_ui_base"
            );
            assert!(
