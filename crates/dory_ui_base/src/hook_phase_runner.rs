...
use dory_core::observability::actions::{HOOK_EXECUTE, HOOK_EXECUTE_FAILED};
use dory_core::{
...
impl HookPhasePolicy {
    ...
    fn record(
        &mut self,
        on_failure: dory_core::HookFailureMode,
        succeeded: bool,
        failure_message: &str,
        ...
        match on_failure {
            dory_core::HookFailureMode::Disconnect => Some(HookPhaseState::Aborted {
                error: failure_message.to_string(),
            }),
            dory_core::HookFailureMode::Warn => {
                self.warnings.push(failure_message.to_string());
                None
            }
            dory_core::HookFailureMode::Ignore => {
    ...
    E: HookExecutor + Clone + 'static,
{
    ...
    for hook in hooks {
        ...
        if phase == HookPhase::PreConnect && hook.is_detached() && hook.ready_signal.is_none() {
            let error = "Detached pre-connect hooks must set a ready signal before Dory can continue connecting"
                .to_string();
        ...
        let hook_start_ms = dory_core::chrono::Utc::now().timestamp_millis();
        ...
        if let Err(error) = cx.update(|cx| {
            if let Err(error) = app_state.read(cx).audit_service().record(
                dory_core::observability::EventRecord::new(
                    ...
                    dory_core::observability::EventCategory::Hook,
                ...
                .with_origin(dory_core::observability::EventOrigin::hook())
                .with_actor_id("hook")
        ...
        let hook_end_ms = dory_core::chrono::Utc::now().timestamp_millis();
        ...
        let (hook_complete_action, hook_complete_outcome, hook_complete_severity) = if cancelled {
            (
                HOOK_EXECUTE_FAILED,
                dory_core::observability::EventOutcome::Cancelled,
        ...
        if let Err(error) = cx.update(|cx| {
            ...
            event = event.with_origin(dory_core::observability::EventOrigin::hook());
