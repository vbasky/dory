...
use dory_components::controls::{Button, GpuiInput as Input, InputEvent, InputState};
...
pub fn submit_label_for(tab: AddPanelTab, saved_selection_count: usize) -> String {
    match tab {
        AddPanelTab::Saved => match saved_selection_count {
            0 => dory_i18n::t!("modals.add_panel_picker.submit.zero"),
            1 => dory_i18n::t!("modals.add_panel_picker.submit.one"),
            n => dory_i18n::t!("modals.add_panel_picker.submit.many", count = n),
        },
        AddPanelTab::Query | AddPanelTab::Metric => {
            dory_i18n::t!("modals.add_panel_picker.submit.create")
        }
    }
}
...
fn chart_kind_label(kind: ChartKind) -> String {
    dory_i18n::t!(chart_kind_label_key(kind))
}
...
impl ModalAddPanelPicker {
    pub fn new(window: &mut Window, cx: &mut Context<Self>) -> Self {
        ...
        let query_name_input = cx.new(|cx| {
            InputState::new(window, cx)
                .placeholder(dory_i18n::t!("modals.add_panel_picker.name_placeholder"))
        });
        let query_input = cx.new(|cx| {
            InputState::new(window, cx)
                ...
                .line_number(true)
                .soft_wrap(true)
                .placeholder(dory_i18n::t!("modals.add_panel_picker.query.placeholder"))
        });
        ...
        let metric_period_input = cx.new(|cx| {
            let mut s = InputState::new(window, cx)
                .placeholder(dory_i18n::t!("modals.add_panel_picker.metric.period_label"));
    ...
    fn render_chart_row(
        ...
        if is_selected {
            checkbox = checkbox.child(
                dory_components::primitives::Text::caption("✓").color(theme.primary_foreground),
            );
        }
        ...
        div()
            ...
            .px(Spacing::SM)
            .py(Spacing::XS)
            .rounded(dory_components::tokens::Radii::SM)
            .bg(row_bg)
    ...
    fn render_tab_strip(&self, cx: &mut Context<Self>) -> AnyElement {
        ...
        let buttons: Vec<AnyElement> = visible
            ...
            .map(|tab| {
                let label = match tab {
                    AddPanelTab::Saved => dory_i18n::t!("modals.add_panel_picker.tab.saved"),
                    AddPanelTab::Query => dory_i18n::t!("modals.add_panel_picker.tab.query"),
                    AddPanelTab::Metric => dory_i18n::t!("modals.add_panel_picker.tab.metric"),
                };
                let id = match tab {
    ...
    fn render_saved_tab(&self, candidates: &[SavedChart], cx: &mut Context<Self>) -> AnyElement {
        if candidates.is_empty() {
            return Text::body(dory_i18n::t!("modals.add_panel_picker.saved.empty"))
                .into_any_element();
        }
        ...
        let search_row = div()
            ...
            .gap(Spacing::XS)
            .child(
                Text::subsection_label(dory_i18n::t!("modals.add_panel_picker.saved.search_label"))
                    .into_any_element(),
            )
    ...
    fn render_query_tab(&self, cx: &mut Context<Self>) -> AnyElement {
        let name_row = div()
            ...
            .gap(Spacing::XS)
            .child(
                Text::subsection_label(dory_i18n::t!("modals.add_panel_picker.name_label"))
                    .into_any_element(),
            )
        ...
        let query_row = div()
            ...
            .gap(Spacing::XS)
            .child(
                Text::subsection_label(dory_i18n::t!("modals.add_panel_picker.query.label"))
                    .into_any_element(),
            )
            .child(
                div()
        ...
        let kind_row = div()
            ...
            .gap(Spacing::XS)
            .child(
                Text::subsection_label(dory_i18n::t!("modals.add_panel_picker.chart_kind.label"))
                    .into_any_element(),
            )
    ...
    fn render_metric_list(&self, cx: &mut Context<Self>) -> AnyElement {
        ...
        let Some(metrics) = self.metric_metrics_for_namespace.get(&namespace) else {
            return div()
                ...
                .p(Spacing::SM)
                .child(
                    Text::body(dory_i18n::t!("modals.add_panel_picker.metric.loading"))
                        .into_any_element(),
                )
                .into_any_element();
        };
        ...
        if metrics.is_empty() {
            return div()
                ...
                .p(Spacing::SM)
                .child(
                    Text::body(dory_i18n::t!("modals.add_panel_picker.metric.no_metrics"))
                        .into_any_element(),
                )
                .into_any_element();
        }
    ...
    fn render_metric_tab(&self, cx: &mut Context<Self>) -> AnyElement {
        ...
        let namespace_column = div()
            ...
            .gap(Spacing::XS)
            .child(
                Text::subsection_label(dory_i18n::t!(
    ...
    fn render(&mut self, _window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        let footer = div()
            ...
            .child(
                Button::new(
                    "add-panel-cancel",
                    dory_i18n::t!("modals.add_panel_picker.cancel"),
                )
                .on_click(on_cancel),
            )
            .child(
        ...
        ModalShell::new(
            dory_i18n::t!("modals.add_panel_picker.title"),
            body,
...
mod tests {
    use super::*;
    use dory_components::chart::ChartSpec;
    use dory_components::saved_chart::{SavedChart, SavedChartRefreshPolicy, SavedChartSource};
    ...
    fn submit_label_updates_live_based_on_selection_count() {
        ...
        assert_eq!(
            ...
            dory_i18n::t!("modals.add_panel_picker.submit.zero")
        );
        assert_eq!(
            submit_label_for(AddPanelTab::Saved, ids_1.len()),
            dory_i18n::t!("modals.add_panel_picker.submit.one")
        );
        assert_eq!(
            submit_label_for(AddPanelTab::Saved, ids_3.len()),
            dory_i18n::t!("modals.add_panel_picker.submit.many", count = ids_3.len())
        );
    }
    ...
    fn submit_label_per_tab_pure() {
        let zero = dory_i18n::t!("modals.add_panel_picker.submit.zero");
        let one = dory_i18n::t!("modals.add_panel_picker.submit.one");
        let many_5 = dory_i18n::t!("modals.add_panel_picker.submit.many", count = 5);
        let create = dory_i18n::t!("modals.add_panel_picker.submit.create");
    ...
    fn add_panel_picker_title_differs_between_locales() {
        let en = dory_i18n::t!("modals.add_panel_picker.title");
        let es = dory_i18n::t!("modals.add_panel_picker.title", locale = "es");
        assert_ne!(en, es);
    }
    ...
    fn submit_label_for_many_interpolates_count() {
        ...
        assert_eq!(
            label,
            dory_i18n::t!("modals.add_panel_picker.submit.many", count = 5)
        );
