use dory_components::semantic::FontSettingGlobal;
use dory_components::typography::AppFonts;
use dory_core::{AppStyle, FontSetting, ThemeSetting};
use dory_ui::theme;
...
fn assert_centralized_fonts(theme: &Theme) {
    assert_eq!(theme.font_family, SharedString::from(AppFonts::SYSTEM));
    assert_eq!(theme.mono_font_family, SharedString::from(AppFonts::MONO));
    assert_eq!(
        theme.dark_theme.font_family,
        Some(SharedString::from(AppFonts::SYSTEM))
    );
    assert_eq!(
...
fn theme_init_and_apply_theme_keep_centralized_fonts_without_changing_base_tokens(
    ...
    cx.update(|cx| {
        FontSettingGlobal::set(cx, FontSetting::system());
    });
    ...
    cx.update(|cx| {
        ...
        assert_eq!(theme.border, rgb_to_hsla(0x2B2B2B));
        assert_eq!(theme.popover, rgb_to_hsla(0x222222));
    });
...
fn ghost_border_contract_moves_out_of_theme_module() {
    ...
    assert_eq!(
        dory_components::tokens::ChromeColors::ghost_border(),
        expected
    );
}
