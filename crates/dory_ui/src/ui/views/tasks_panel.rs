...
use dory_core::{TaskId, TaskKind, TaskSnapshot, TaskStatus};
        ...
        match task_kind {
            TaskKind::Query => {
                ...
                if let Some(target) = task_target {
                    ...
                    let fallback_target = dory_core::TaskTarget {
                        profile_id,
                        database: None,
                    };
    ...
    fn render_task_row(&mut self, task: &TaskSnapshot, cx: &mut Context<Self>) -> Div {
        ...
        div()
            ...
            .when(has_details && is_expanded, |el| {
                ...
                if lines.len() > 40 {
                    lines.truncate(40);
                    lines.push(dory_i18n::t!("tasks_panel.output_truncated"));
                }
            ...
            .when(all_tasks.is_empty(), |el: Div| {
                el.child(
                    div()
                        ...
                        .justify_center()
                        .py_4()
                        .child(Text::muted(dory_i18n::t!("tasks_panel.empty"))),
                )
            })
            .children(task_rows)
    }
