...
use dory_components;
use dory_core::observability::actions::CONFIG_CHANGE;
use dory_ui_base::modals::{
...
use dory_ui_base::{AppStateGlobal, OpenAuditRequested, drain_hook_load_diagnostics, report_error};
...
#[cfg(test)]
use dory_core::{CollectionRef, TableRef};
use dory_core::{ExecutionContext, QueryLanguage};
use dory_ui_base::toast::{Toast, ToastGlobal, ToastHost, copy_action, now_hms};
use dory_ui_sidebar::{Sidebar, SidebarEvent, SidebarTab};
use dory_ui_windows::connection_manager::ConnectionManagerWindow;
...
pub(super) fn build_resource_items_from_schema(
    ...
    profile_name: &str,
    structure: &dory_core::DataStructure,
    items: &mut Vec<PaletteItem>,
) {
    match structure {
        dory_core::DataStructure::Relational(rel) => {
        ...
        dory_core::DataStructure::Document(doc) => {
            let default_db = doc
                .current_database
                .clone()
        ...
        dory_core::DataStructure::KeyValue(kv) => {
...
pub struct Workspace {
    ...
    export_modal: Entity<dory_ui_windows::connection_manager::ExportBundleModal>,
    /// Import wizard (folder bundle -> tables), targeting the connection it
    /// was opened from.
    import_wizard: Entity<dory_ui_document::import_wizard::ImportWizard>,
    /// Migrate wizard (table -> table, cross-connection), pre-populated from
    /// the sidebar's multi-select Migrate action.
    migrate_wizard: Entity<dory_ui_document::migrate_wizard::MigrateWizard>,
    /// Export wizard (table -> file bundle), pre-populated from the
    /// sidebar's multi-select Export action.
    export_wizard: Entity<dory_ui_document::export_wizard::ExportWizard>,
...
impl Workspace {
    fn sync_theme_with_appearance(&mut self, window: &mut Window, cx: &mut Context<Self>) {
        let settings = self.app_state.read(cx).general_settings().clone();
        if settings.theme_mode != dory_core::ThemeModeSetting::System {
            return;
        }
        let resolved = settings.resolved_theme(dory_components::theme::appearance_is_dark(
            window.appearance(),
        ));
        if resolved == dory_components::semantic::ThemeSettingGlobal::get(cx) {
            return;
        }
        dory_components::semantic::ThemeSettingGlobal::set(cx, resolved);
        dory_components::theme::apply_theme(resolved, settings.style, Some(window), cx);
        cx.notify();
    }
    ...
    pub fn new(
...

        {
            ...
            dory_components::theme::apply_resolved(
                &settings,
                dory_components::theme::appearance_is_dark(window.appearance()),
                Some(window),
                cx,
            );
        }
        ...
        let export_modal = cx.new(|cx| {
            dory_ui_windows::connection_manager::ExportBundleModal::new(
                app_state.clone(),
                window,
                cx,
            )
        ...
        let import_wizard =
            cx.new(|cx| dory_ui_document::import_wizard::ImportWizard::new(app_state.clone(), cx));
        let migrate_wizard = cx
            .new(|cx| dory_ui_document::migrate_wizard::MigrateWizard::new(app_state.clone(), cx));
        let export_wizard = cx.new(|cx| {
            dory_ui_document::export_wizard::ExportWizard::new(app_state.clone(), window, cx)
        });
        ...
        cx.subscribe_in(
            ...
            |this, _, event: &SidebarEvent, window, cx| match event {
                ...
                SidebarEvent::OpenScript { path } => {
                    if dory_core::is_openable_script(path) {
                        this.open_script_from_path(path.clone(), cx);
                    } else {
        ...
        cx.subscribe_in(
            ...
            |this, _, event: &crate::ui::document::TabManagerEvent, window, cx| {
                ...
                match event {
                    ...
                    TabManagerEvent::DocumentUpdated => {}
                }
            },
        )
        .detach();
        ...
        let appearance_subscription = cx.observe_window_appearance(window, |this, window, cx| {
            this.sync_theme_with_appearance(window, cx);
        });
...

        {
            ...
            match settings.default_focus_on_startup {
                dory_core::StartupFocus::Sidebar => {
                    ...
                    loop {
                        ...
                        match purge_result {
                            Some(Ok(stats)) => {
                                ...
                                let event = dory_core::observability::EventRecord::new(
                                    now_ms,
                                    dory_core::observability::EventSeverity::Info,
                                    dory_core::observability::EventCategory::System,
                                    dory_core::observability::EventOutcome::Success,
                                )
                            ...
                            Some(Err(e)) => {
                                ...
                                let event = dory_core::observability::EventRecord::new(
                                    now_ms,
                                    dory_core::observability::EventSeverity::Error,
                                    dory_core::observability::EventCategory::System,
                                    dory_core::observability::EventOutcome::Failure,
                                )
                                .with_typed_action(CONFIG_CHANGE)
                                .with_summary(format!(
    ...
    fn default_commands() -> Vec<PaletteCommand> {
        ...
        vec![
            ...
            PaletteCommand::new(
                "new_query_tab",
                dory_i18n::t!("palette.command.new_query_tab.name"),
                dory_i18n::t!("palette.category.editor"),
            )
            .with_shortcut(SC.new_query_tab),
            PaletteCommand::new(
                "run_query",
                dory_i18n::t!("palette.command.run_query.name"),
                dory_i18n::t!("palette.category.editor"),
            )
            .with_shortcut(SC.run_query),
            PaletteCommand::new(
                "run_query_in_new_tab",
                dory_i18n::t!("palette.command.run_query_in_new_tab.name"),
                dory_i18n::t!("palette.category.editor"),
            )
            .with_shortcut(SC.run_query_in_new_tab),
            PaletteCommand::new(
                "save_query",
                dory_i18n::t!("palette.command.save_query.name"),
                dory_i18n::t!("palette.category.editor"),
            )
            .with_shortcut(SC.save_query),
            PaletteCommand::new(
                "save_file_as",
                dory_i18n::t!("palette.command.save_file_as.name"),
                dory_i18n::t!("palette.category.editor"),
            )
            .with_shortcut(SC.save_file_as),
            PaletteCommand::new(
                "open_script_file",
                dory_i18n::t!("palette.command.open_script_file.name"),
                dory_i18n::t!("palette.category.editor"),
            )
            .with_shortcut(SC.open_script_file),
            PaletteCommand::new(
                "open_history",
                dory_i18n::t!("palette.command.open_history.name"),
                dory_i18n::t!("palette.category.editor"),
            )
            .with_shortcut(SC.open_history),
            PaletteCommand::new(
                "cancel_query",
                dory_i18n::t!("palette.command.cancel_query.name"),
                dory_i18n::t!("palette.category.editor"),
            )
            .with_shortcut("esc"),
            ...
            PaletteCommand::new(
                "close_tab",
                dory_i18n::t!("palette.command.close_tab.name"),
                dory_i18n::t!("palette.category.tabs"),
            )
            .with_shortcut(SC.close_tab),
            PaletteCommand::new(
                "next_tab",
                dory_i18n::t!("palette.command.next_tab.name"),
                dory_i18n::t!("palette.category.tabs"),
            )
            .with_shortcut("ctrl-tab"),
            PaletteCommand::new(
                "prev_tab",
                dory_i18n::t!("palette.command.prev_tab.name"),
                dory_i18n::t!("palette.category.tabs"),
            )
            ...
            PaletteCommand::new(
                "export_results",
                dory_i18n::t!("palette.command.export_results.name"),
                dory_i18n::t!("palette.category.results"),
            )
            .with_shortcut(SC.export_results),
            // Connections
            PaletteCommand::new(
                "open_connection_manager",
                dory_i18n::t!("palette.command.open_connection_manager.name"),
                dory_i18n::t!("palette.category.connections"),
            ),
            PaletteCommand::new(
                "disconnect",
                dory_i18n::t!("palette.command.disconnect.name"),
                dory_i18n::t!("palette.category.connections"),
            ),
            PaletteCommand::new(
                "refresh_schema",
                dory_i18n::t!("palette.command.refresh_schema.name"),
                dory_i18n::t!("palette.category.connections"),
            ),
            ...
            PaletteCommand::new(
                "focus_sidebar",
                dory_i18n::t!("palette.command.focus_sidebar.name"),
                dory_i18n::t!("palette.category.focus"),
            )
            .with_shortcut("ctrl-shift-1"),
            PaletteCommand::new(
                "focus_editor",
                dory_i18n::t!("palette.command.focus_editor.name"),
                dory_i18n::t!("palette.category.focus"),
            )
            .with_shortcut("ctrl-shift-2"),
            PaletteCommand::new(
                "focus_results",
                dory_i18n::t!("palette.command.focus_results.name"),
                dory_i18n::t!("palette.category.focus"),
            )
            .with_shortcut("ctrl-shift-3"),
            PaletteCommand::new(
                "focus_tasks",
                dory_i18n::t!("palette.command.focus_tasks.name"),
                dory_i18n::t!("palette.category.focus"),
            )
            ...
            PaletteCommand::new(
                "toggle_sidebar",
                dory_i18n::t!("palette.command.toggle_sidebar.name"),
                dory_i18n::t!("palette.category.view"),
            )
            .with_shortcut(SC.toggle_sidebar),
            PaletteCommand::new(
                "toggle_editor",
                dory_i18n::t!("palette.command.toggle_editor.name"),
                dory_i18n::t!("palette.category.view"),
            ),
            PaletteCommand::new(
                "toggle_results",
                dory_i18n::t!("palette.command.toggle_results.name"),
                dory_i18n::t!("palette.category.view"),
            ),
            PaletteCommand::new(
                "toggle_tasks",
                dory_i18n::t!("palette.command.toggle_tasks.name"),
                dory_i18n::t!("palette.category.view"),
            ),
            PaletteCommand::new(
                "open_settings",
                dory_i18n::t!("palette.command.open_settings.name"),
                dory_i18n::t!("palette.category.view"),
            ),
            PaletteCommand::new(
                "open_login_modal",
                dory_i18n::t!("palette.command.open_login_modal.name"),
                dory_i18n::t!("palette.category.view"),
            ),
            PaletteCommand::new(
                "open_sso_wizard",
                dory_i18n::t!("palette.command.open_sso_wizard.name"),
                dory_i18n::t!("palette.category.view"),
            ),
            #[cfg(feature = "mcp")]
            PaletteCommand::new(
                "open_mcp_approvals",
                dory_i18n::t!("palette.command.open_mcp_approvals.name"),
                dory_i18n::t!("palette.category.view"),
            ),
            #[cfg(feature = "mcp")]
            PaletteCommand::new(
                "refresh_mcp_governance",
                dory_i18n::t!("palette.command.refresh_mcp_governance.name"),
                dory_i18n::t!("palette.category.view"),
            ),
            PaletteCommand::new(
                "open_audit_viewer",
                dory_i18n::t!("palette.command.open_audit_viewer.name"),
                dory_i18n::t!("palette.category.view"),
            )
            ...
            PaletteCommand::new(
                "open_saved_chart",
                dory_i18n::t!("palette.command.open_saved_chart.name"),
                dory_i18n::t!("palette.category.charts"),
            ),
            PaletteCommand::new(
                "new_dashboard",
                dory_i18n::t!("palette.command.new_dashboard.name"),
                dory_i18n::t!("palette.category.dashboards"),
            ),
        ]
    }
    ...
    fn build_palette_items(&self, cx: &Context<Self>) -> Vec<PaletteItem> {
        ...
        if app_state.active_connection().is_some_and(|a| {
            a.connection
                .metadata()
                .capabilities
                .contains(dory_core::DriverCapabilities::DASHBOARD_IMPORT)
        }) {
    ...
    fn flatten_script_entries(
        entries: &[dory_core::ScriptEntry],
        scripts_root: &std::path::Path,
        items: &mut Vec<PaletteItem>,
    ) {
        use dory_core::ScriptEntry;
    ...
    fn start_pipeline_progress(
        &mut self,
        profile_name: String,
        watcher: dory_core::StateWatcher,
        window: &mut Window,
        cx: &mut Context<Self>,
    ) {
            ...
            move |this, _, event: &pipeline::PipelineProgressEvent, window, cx| {
                match event {
                    ...
                    pipeline::PipelineProgressEvent::WatchClosed { last_state } => {
                        if matches!(last_state, dory_core::PipelineState::Connected) {
    ...
    fn persist_inspector_width(&mut self, width: Pixels, cx: &mut Context<Self>) {
        ...
        if let Err(e) = dory_app::config_loader::save_general_settings(runtime, &settings) {
