...
//!
//! The actual implementation lives in `dory_components::composites::menu_popup`
//! so that document and sidebar crates (which cannot depend on `dory_ui`) can
//! render the same chrome. This module preserves the historical `dory_ui`
//! call sites by re-exporting under the original names.

pub use dory_components::composites::MenuItem;
use dory_components::composites::render_menu_items;
pub use dory_components::composites::render_menu_overlay;
use gpui::*;
...
/// `dory_components::composites::render_menu_items` directly.
