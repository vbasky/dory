...
const UI_BASE_SRC: &str = concat!(env!("CARGO_MANIFEST_DIR"), "/../dory_ui_base/src");
const UI_DOCUMENT_SRC: &str = concat!(env!("CARGO_MANIFEST_DIR"), "/../dory_ui_document/src");
const UI_WINDOWS_SRC: &str = concat!(env!("CARGO_MANIFEST_DIR"), "/../dory_ui_windows/src");
...
fn representative_overlays_reject_raw_scrim_regressions() {
    // history_modal.rs now lives in dory_ui_document (moved in Step 3b)
...
fn ui_keeps_shared_contract_helpers_out_of_feature_modules() {
    ...
    for forbidden_pattern in ["pub(super) fn section_header(", "fn section_header("] {
        assert!(
            ...
            "settings/layout.rs reintroduced shared contract helper `{forbidden_pattern}` outside dory_components"
        );
    }
    ...
    for forbidden_pattern in [
        ...
        assert!(
            ...
            "views/workspace/render.rs reintroduced shared contract helper `{forbidden_pattern}` outside dory_components"
        );
    }
}
