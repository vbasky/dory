...
#[cfg(test)]
use dory_components::helpers::text_color_for_selected;
use dory_components::primitives::{Chord, Icon, overlay_bg, surface_modal_container};
use dory_components::semantic::BannerColors as SemBannerColors;
use dory_components::tokens::{Radii, Spacing};
use dory_components::typography::{Body, MonoCaption, MonoLabel};
...
impl PaletteItem {
    ...
    pub fn search_text(&self) -> String {
        match self {
            ...
            Self::Connection { name, .. } => {
                format!(
                    "Connection {} {}",
                    dory_i18n::t!("palette.kind.connection"),
                    name
                )
            }
            Self::SavedChart {
                ...
                "Chart {} {} {}",
                dory_i18n::t!("palette.kind.chart"),
                name,
                profile_name
            ),
            Self::Resource(r) => match r {
                ResourceItem::Table {
                    ...
                    let mut parts = format!(
                        "Table {} {} {}",
                        dory_i18n::t!("palette.kind.table"),
                        profile_name,
                        name
                    );
                ...
                ResourceItem::Collection {
                    ...
                    "Collection {} {} {} {}",
                    dory_i18n::t!("palette.kind.collection"),
                    profile_name,
                    name,
                    database
                ),
                ...
                ResourceItem::KeyValueDb {
                    ...
                    "Keyspace {} {} {}",
                    dory_i18n::t!("palette.kind.keyspace"),
                    profile_name,
                    database
                ),
            },
            Self::Script {
                ...
                format!(
                    "Script {} {} {}",
                    dory_i18n::t!("palette.kind.script"),
                    name,
                    relative_path
                )
            }
            Self::ImportDashboard => {
                format!("Charts {}", dory_i18n::t!("palette.import_dashboard.name"))
            }
        }
    }
    ...
    pub fn display_label(&self) -> (String, String) {
        match self {
            ...
            Self::Connection { name, .. } => {
                (dory_i18n::t!("palette.kind.connection"), name.clone())
            }
            Self::SavedChart {
                name,
                ...
                let display = if *is_collection_source {
                    format!("{} {}", name, dory_i18n::t!("palette.chart.browse_suffix"))
                } else {
                    name.clone()
                };
                (dory_i18n::t!("palette.kind.chart"), display)
            }
            Self::Resource(r) => match r {
                ResourceItem::Table { name, .. } => {
                    (dory_i18n::t!("palette.kind.table"), name.clone())
                }
                ResourceItem::Collection { name, .. } => {
                    (dory_i18n::t!("palette.kind.collection"), name.clone())
                }
                ResourceItem::View { name, .. } => {
                    (dory_i18n::t!("palette.kind.view"), name.clone())
                }
                ResourceItem::KeyValueDb { database, .. } => {
                    (dory_i18n::t!("palette.kind.keyspace"), database.clone())
                }
            },
            Self::Script { name, .. } => (dory_i18n::t!("palette.kind.script"), name.clone()),
            Self::ImportDashboard => (
                dory_i18n::t!("palette.section.charts"),
                dory_i18n::t!("palette.import_dashboard.name"),
            ),
        }
    }
...
impl PaletteSection {
    fn label(self) -> String {
        match self {
            Self::Connections => dory_i18n::t!("palette.section.connections"),
            Self::Commands => dory_i18n::t!("palette.section.commands"),
            Self::Charts => dory_i18n::t!("palette.section.charts"),
            Self::Tables => dory_i18n::t!("palette.section.tables"),
            Self::Scripts => dory_i18n::t!("palette.section.scripts"),
        }
    }
...
impl CommandPalette {
    pub fn new(window: &mut Window, cx: &mut Context<Self>) -> Self {
        let input_state = cx.new(|cx| {
            InputState::new(window, cx).placeholder(dory_i18n::t!("palette.search.placeholder"))
        });
                            ...
                            .when(self.filtered.is_empty(), |d| {
                                d.child(
                                    div()
                                        ...
                                        .flex()
                                        .justify_center()
                                        .child(Body::new(dory_i18n::t!("palette.empty")).muted(cx)),
                                )
                            }),
                    )
                    .child(
...
mod tests {
    ...
    #[gpui::test]
    fn action_shortcuts_use_ui_family_caption(cx: &mut TestAppContext) {
    ...
    #[gpui::test]
    fn qualifiers_use_ui_family_caption_role(cx: &mut TestAppContext) {
        cx.update(theme::init);
    ...
    fn command_palette_includes_new_dashboard_entry() {
        ...
        let commands = Workspace::palette_commands_for_test();
        let expected_name = dory_i18n::t!("palette.command.new_dashboard.name");
        let expected_category = dory_i18n::t!("palette.category.dashboards");
        let found = commands
            .iter()
    ...
    fn palette_section_label_differs_between_locales() {
        let english = dory_i18n::t!("palette.section.connections", locale = "en");
        let spanish = dory_i18n::t!("palette.section.connections", locale = "es");
    ...
    fn palette_command_name_differs_between_locales() {
        let english = dory_i18n::t!("palette.command.run_query.name", locale = "en");
        let spanish = dory_i18n::t!("palette.command.run_query.name", locale = "es");
