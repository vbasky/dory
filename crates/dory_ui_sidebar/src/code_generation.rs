use super::*;
use dory_core::MutationCategory;
use dory_ui_base::sql_preview_modal::SqlGenerationType;
        ...
        let index_info = if is_schema_level {
            ...
            conn.table_details
                ...
                .and_then(|index_data| match index_data {
                    dory_core::IndexData::Relational(indexes) => indexes
                        .iter()
                        .find(|idx| idx.name == index_name)
                        .map(|idx| (table_name.clone(), idx.columns.clone(), idx.is_unique)),
                    dory_core::IndexData::Document(indexes) => indexes
                        .iter()
                        .find(|idx| idx.name == index_name)
                        .map(|idx| {
