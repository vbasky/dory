...
use dory_app::AppState;
use dory_components::components::tree_nav::{self, GutterInfo};
...
use dory_core::ConnectedProfile;
use dory_core::{
...
use dory_ui_base::app_state_entity::{AppStateChanged, AppStateEntity};
...
pub enum SidebarEvent {
    ...
    RequestSqlPreview {
        ...
        generation_type: dory_ui_base::sql_preview_modal::SqlGenerationType,
    },
    RequestQueryPreview {
    ...
    RequestDropTable {
        ...
        dependents: Vec<dory_core::RelationRef>,
    },
    ...
    ///
    /// The integrator (`dory_ui` Workspace) owns window creation; the sidebar
    /// emits this event rather than calling `open_or_focus_settings` directly,
    /// keeping `dory_ui_sidebar` free of a dependency on `dory_ui_windows`.
    RequestOpenSettings,
    ...
    /// database). The wizard itself lives in `dory_ui_document`; the
    /// integrator (`dory_ui` Workspace) owns opening it, mirroring
    ...
    /// connection via its own picker. The wizard lives in `dory_ui_document`;
    /// the integrator (`dory_ui` Workspace) owns opening it, mirroring
    ...
    /// lives in `dory_ui_document`; the integrator (`dory_ui` Workspace)
...
impl ContextMenuItem {
    ...
    pub fn to_menu_items(items: &[ContextMenuItem]) -> Vec<dory_components::composites::MenuItem> {
        items
            .iter()
            .map(|item| {
                if item.is_separator {
                    return dory_components::composites::MenuItem::separator();
                }

                let mut mi = dory_components::composites::MenuItem::new(item.label.clone());
...
impl Sidebar {
    pub fn new(
        ...
        let connections_search_input = cx.new(|cx| {
            InputState::new(window, cx)
                .placeholder(dory_i18n::t!("sidebar.filter.connections_placeholder"))
        });
        ...
        let scripts_search_input = cx.new(|cx| {
            InputState::new(window, cx)
                .placeholder(dory_i18n::t!("sidebar.filter.scripts_placeholder"))
        });
    ...
    fn refresh_scripts_tree(&mut self, cx: &mut Context<Self>) {
        ...
        let entries = match state.scripts_directory() {
            Some(dir) => dory_core::filter_entries(dir.entries(), &self.scripts_search_query),
            None => Vec::new(),
        };
    ...
    fn show_edit_reconnect_toast(
        ...
        let profile_name = app_state
            ...
            .unwrap_or_else(|| dory_i18n::t!("sidebar.status.profile_fallback_name"));

        let app_state_for_action = app_state.clone();
        let reconnect_action = dory_ui_base::toast::ToastAction::new(
            "edit-reconnect-now",
            dory_i18n::t!("sidebar.toast.edit_reconnect_now"),
        )
        .primary()
        .on_click(move |cx| {
        ...
        let later_action = dory_ui_base::toast::ToastAction::new(
            "edit-reconnect-later",
            dory_i18n::t!("sidebar.toast.edit_reconnect_later"),
        );

        dory_ui_base::toast::Toast::info(labels::profile_updated_label(&profile_name))
            .body(dory_i18n::t!("sidebar.toast.edit_reconnect_body"))
...
mod tests {
    ...
    use dory_app::{ExternalDriverDiagnostic, ExternalDriverStage};
    use dory_core::PrepareConnectError;
    use dory_core::{SchemaNodeId, SchemaNodeKind};
    ...
    fn to_menu_items_uses_explicit_icon_override() {
        use super::{ContextMenuAction, ContextMenuItem};
        use dory_components::icon::IconSource;
        use dory_components::icons::AppIcon;
    ...
    fn menu_labels_for(item_id: &str) -> Vec<String> {
        ...
        match kind {
            SchemaNodeKind::DashboardsFolder => {
                vec![dory_i18n::t!("sidebar.menu.new_dashboard")]
            }
            SchemaNodeKind::SavedChartsFolder => {
                vec![dory_i18n::t!("sidebar.menu.new_saved_chart")]
            }
