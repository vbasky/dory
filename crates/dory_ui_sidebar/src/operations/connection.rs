...
use dory_core::{DatabaseConnection, DbSchemaInfo, HookPhase, PrepareConnectError, TaskKind};
use dory_ssh::is_passphrase_required_error_str;
...
pub(crate) fn try_close_held_database_connection(
    ...
    let Some(connection) = Arc::get_mut(&mut held_connection.connection.connection) else {
        return Err(format!(
            "Cannot drop database '{}' while Dory still has active references to its connection",
            held_connection.database
        ));
    };
    ...
    connection.close().map_err(|error| {
        format!(
            "Failed to release Dory connection for database '{}': {}",
...
async fn wait_for_connection_teardown(
    ...
    cancel_token: &dory_core::CancelToken,
    cx: &gpui::AsyncApp,
) -> Option<String> {
...
impl Sidebar {
    ...
    fn connect_to_profile_inner(
        ...
        cx.spawn(async move |_this, cx| {
            ...
            let result = cx
                .background_executor()
                .spawn(async move { params.execute(Some(dory_app::proxy::create_proxy_tunnel)) })
                .await;
    ...
    pub fn disconnect_profile(&mut self, profile_id: Uuid, cx: &mut Context<Self>) {
        ...
        cx.spawn(async move |_this, cx| {
            ...
            let disconnect_now_ms = dory_core::chrono::Utc::now().timestamp_millis();
            let _ = cx.update(|cx| {
...
mod tests {
    ...
    fn connected_toast_differs_between_locales() {
        ...
        let spanish_key = dory_i18n::t!("sidebar.toast.connected.plain", locale = "es");
