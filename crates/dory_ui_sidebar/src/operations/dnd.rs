...
use crate::*;
use dory_core::{Connection, SchemaDropTarget, SchemaObjectKind, TaskKind, TaskTarget};
...
impl Sidebar {
    ...
    fn prepare_database_drop_release(
        ...
        let Some(connected) = state.connections_mut().get_mut(&profile_id) else {
            return Err(format!(
                "No active Dory connection found for database '{}'",
                database
            ));
        };
        ...
        {
            return Err(format!(
                "Cannot drop database '{}' while Dory is still connected to it as the current session. Open another database first.",
                database
            ));
        }
