use crate::*;
use dory_core::observability::actions::{CONNECTION_CONNECT, CONNECTION_CONNECT_FAILED};
use dory_core::{CancelToken, HookContext, HookPhase, PipelineState, TaskId, TaskKind};
...
impl Sidebar {
    ...
    pub(super) fn connect_via_pipeline(&mut self, profile_id: Uuid, cx: &mut Context<Self>) {
        ...
        let (state_tx, state_rx) = dory_core::pipeline_state_channel();
        ...
        cx.spawn(async move |_this, cx| {
            ...
            .await
            {
                ...
                HookPhaseState::Aborted { error } => {
                    let _ = state_tx.send(dory_core::PipelineState::Failed {
                ...
                HookPhaseState::Cancelled => {
                    let _ = state_tx.send(dory_core::PipelineState::Cancelled);
            ...
            let pipeline_result = cx
                .background_executor()
                .spawn(
                    async move { dory_core::run_pipeline(input, &state_tx_for_pipeline).await },
                )
                .await;
            ...
            let output = match pipeline_result {
                ...
                Err(pipeline_error) => {
                    ...
                    let pipeline_fail_now_ms = dory_core::chrono::Utc::now().timestamp_millis();
                    let pipeline_fail_driver_id = driver.display_name().to_string();
                    let _ = cx.update(|cx| {
                        let audit_service = app_state.read(cx).audit_service().clone();
                        let mut event = dory_core::observability::EventRecord::new(
                            pipeline_fail_now_ms,
                            dory_core::observability::EventSeverity::Error,
                            dory_core::observability::EventCategory::Connection,
                            dory_core::observability::EventOutcome::Failure,
                        );
                        event.action = CONNECTION_CONNECT_FAILED.as_str().to_string();
                        event.actor_type = dory_core::observability::EventActorType::User;
                        event.source_id = dory_core::observability::EventSourceId::Local;
            ...
            let overrides = dory_core::ConnectionOverrides::new(effective_password);
            ...
            let connect_result = cx
                .background_executor()
                .spawn(async move {
                    let _ = state_tx_for_connect.send(dory_core::PipelineState::Connecting {
                    ...
                    let _ = state_tx_for_connect.send(dory_core::PipelineState::FetchingSchema);
            ...
            let (profile, connection, schema, tunnel_handle) = match connect_result {
                ...
                Err(error) => {
                    ...
                    let driver_fail_now_ms = dory_core::chrono::Utc::now().timestamp_millis();
            ...
            let _ = state_tx.send(dory_core::PipelineState::Connected);
            ...
            let connect_success_now_ms = dory_core::chrono::Utc::now().timestamp_millis();
            let _ = cx.update(|cx| {
                let audit_service = app_state.read(cx).audit_service().clone();
                let mut event = dory_core::observability::EventRecord::new(
            ...
            let capture_tables: Vec<dory_core::TableInfo> = schema
                .as_ref()
                .map(|s| {
                    let mut tables = s.tables().to_vec();
                    if tables.is_empty() {
                        tables.extend(s.schemas().iter().flat_map(|schema| schema.tables.clone()));
                    }
                    tables
                })
                .unwrap_or_default();
            ...
            let capture_ctx = if capture_category == dory_core::DatabaseCategory::Relational {
                cx.update(|cx| {
            ...
            if let Some((capture_repo, capture_retention)) = capture_ctx {
                ...
                let (capture_result, hydration) = cx
                    .background_executor()
                    .spawn(async move {
                        let mut manager = dory_ui_base::SchemaSnapshotManager::new(capture_repo);

                        let capture_result = manager.capture(
                            &profile_id_string,
                            capture_database.as_deref(),
                            &capture_tables,
                            dory_core::SnapshotDepth::Shallow,
                            capture_retention,
                        );
