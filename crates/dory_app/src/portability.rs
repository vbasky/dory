//! Connection-export/import bridge between `dory_portability` and `AppState`.
//!
...
//! types appear here, which is what allows the `dory_app` test binary to
//! compile and run these tests (unlike `dory_ui_windows`, whose GPUI proc-macro
...
/// Run the pure `dory_portability::apply()` and persist the resulting
...
) -> Result<ImportOutcome, dory_portability::PortabilityError> {
    let actions = dory_portability::import::apply(parsed, plan, choices)?;
...
pub fn persist_import_actions(
    ...
    // Auth secret refs use the format `dory:auth:<uuid>:<field>`.
    for (ref_str, secret) in &actions.secret_writes {
        if let Some(stripped) = ref_str.strip_prefix("dory:auth:")
...
pub fn mapto_candidates(
    kind: dory_portability::ConflictKind,
    dest: &OwnedDestSnapshot,
) -> Vec<(uuid::Uuid, String)> {
    match kind {
        dory_portability::ConflictKind::AuthProfile => dest
            .auth_profiles
            .iter()
            .map(|a| (a.id, a.name.clone()))
            .collect(),
        dory_portability::ConflictKind::SshTunnel => dest
            .ssh_tunnels
            .iter()
            .map(|s| (s.id, s.name.clone()))
            .collect(),
        dory_portability::ConflictKind::Proxy => dest
            .proxies
            .iter()
            .map(|p| (p.id, p.name.clone()))
            .collect(),
        dory_portability::ConflictKind::Connection => dest
            .connections
            .iter()
...
pub fn confirm_summary(
    parsed: &dory_portability::ParsedBundle,
    plan: &dory_portability::ImportPlan,
...
mod tests {
    ...
    use dory_portability::{AwsRef, FieldHintResolver, SecretReader};
    use uuid::Uuid;
    ...
    impl dory_core::DbDriver for FakeDriver {
        fn kind(&self) -> DbKind {
            DbKind::SQLite
        }
    ...
    fn make_ssh_tunnel(id: Uuid) -> SshTunnelProfile {
        let mut s = SshTunnelProfile::new(
            "test-ssh",
            dory_core::SshTunnelConfig {
                host: "bastion.example.com".to_string(),
                port: 22,
                user: "ec2-user".to_string(),
                auth_method: dory_core::SshAuthMethod::Password,
            },
        );
        s.id = id;
        s
    ...
    fn make_proxy(id: Uuid) -> ProxyProfile {
        ProxyProfile {
            ...
            kind: dory_core::ProxyKind::Socks5,
            host: "proxy.example.com".to_string(),
            port: 1080,
            auth: dory_core::ProxyAuth::None,
            no_proxy: None,
            enabled: true,
            save_secret: false,
        }
    ...
    fn hint_resolver_delegates_to_driver_without_branching_on_id() {
        let mut drivers: HashMap<String, Arc<dyn dory_core::DbDriver>> = HashMap::new();
        drivers.insert(
            "fake".to_string(),
    ...
    fn secret_reader_returns_value_for_known_ref() {
        ...
        let store = FakeSecretStore::with(&[("dory/conn/profile-1/password", "s3cr3t")]);
        let reader = AppSecretReader::new(make_store_arc(store));

        let result = reader.read("dory/conn/profile-1/password");
    ...
    fn secret_reader_returns_none_for_missing_ref() {
        ...
        let result = reader.read("dory/conn/nonexistent/password");
    ...
    use dory_portability::ImportActions;
    ...
    fn make_conn_profile(name: &str, driver_id: &str) -> ConnectionProfile {
        let mut p = ConnectionProfile::new(
            name,
            dory_core::DbConfig::External {
                kind: dory_core::DbKind::SQLite,
                values: FormValues::default(),
            },
        );
        p.driver_id = Some(driver_id.to_string());
    ...
    fn persist_secret_write_failure_is_recorded_not_silently_lost() {
        let mut deps =
            FakePersistence::all_drivers().with_keyring_failure("dory:conn:aaaa-bbbb:password");
        ...
        let actions = ImportActions {
            ...
            proxies: vec![],
            secret_writes: vec![(
                "dory:conn:aaaa-bbbb:password".to_string(),
        ...
        assert_eq!(outcome.secret_failures[0].1, "dory:conn:aaaa-bbbb:password");
    }
    ...
    fn persist_successful_secret_write_does_not_appear_in_failures() {
        ...
        let actions = ImportActions {
            ...
            proxies: vec![],
            secret_writes: vec![(
                "dory:conn:1111-2222:password".to_string(),
    ...
    fn persist_connection_driver_id_is_preserved_through_seam() {
        ...
        impl ImportPersistence for DriverIdRecorder {
            ...
            fn add_connection(&mut self, profile: ConnectionProfile) -> ConnectionInsertResult {
                ...
                let is_external = matches!(profile.config, dory_core::DbConfig::External { .. });
    ...
    fn persist_skipped_connection_secrets_are_not_written() {
        use dory_core::connection_secret_ref;
    ...
    fn export_report_skipped_connections_can_be_populated() {
        let mut report = dory_portability::ExportReport::default();
    ...
    fn mapto_candidates_auth_profile_returns_all_dest_auth_profiles() {
        ...
        let candidates =
            super::mapto_candidates(dory_portability::ConflictKind::AuthProfile, &dest);
    ...
    fn mapto_candidates_ssh_tunnel_returns_all_dest_tunnels() {
        ...
        let candidates = super::mapto_candidates(dory_portability::ConflictKind::SshTunnel, &dest);
    ...
    fn confirm_summary_counts_bundle_entities() {
        use dory_portability::ParsedBundle;
        use dory_portability::bundle::{
        ...
        let plan = dory_portability::ImportPlan {
            conflicts: vec![],
    ...
    fn hydrate_auth_secret_fields_is_called_for_new_auth_profiles() {
        ...
        let auth_id = Uuid::new_v4();
        let auth_secret_ref = dory_core::auth_field_secret_ref(&auth_id, "token");
