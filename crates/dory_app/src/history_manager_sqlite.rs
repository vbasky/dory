//! Repository-backed history manager using `dory.db`.
//!
...
use dory_core::{HistoryEntry, SavedQuery};
use dory_storage::bootstrap::StorageRuntime;
use dory_storage::repositories::state::query_history::QueryHistoryRepository;
use dory_storage::repositories::state::recent_items::RecentItemDto;
use dory_storage::repositories::state::recent_items::RecentItemsRepository;
use dory_storage::repositories::state::saved_queries::SavedQueriesRepository;
use log::{error, info};
use std::time::Duration;
use uuid::Uuid;

/// Re-export for backwards compatibility.
pub type RecentFile = dory_core::RecentFile;
...
impl HistoryManager {
    /// Creates a manager backed by `dory.db` repositories.
    ///
    ...
    pub fn new(runtime: &StorageRuntime) -> Self {
        ...
        info!(
            "Loaded {} history entries, {} saved queries, {} recent files from dory.db",
            history_entries.len(),
            saved_queries.len(),
            recent_files.len()
        );
    ...
    fn load_history(repo: &QueryHistoryRepository) -> Vec<HistoryEntry> {
        match repo.recent(500) {
            Ok(entries) => entries
                ...
                .collect(),
            Err(e) => {
                log::warn!("Failed to load history from dory.db: {}", e);
                Vec::new()
            }
        }
    }
    ...
    fn load_saved_queries(repo: &SavedQueriesRepository) -> Vec<SavedQuery> {
        match repo.all() {
            Ok(entries) => entries
                ...
                .collect(),
            Err(e) => {
                log::warn!("Failed to load saved queries from dory.db: {}", e);
                Vec::new()
            }
        }
    }
    ...
    fn load_recent_files(repo: &RecentItemsRepository) -> Vec<RecentFile> {
        match repo.all() {
            Ok(entries) => entries
                ...
                .collect(),
            Err(e) => {
                log::warn!("Failed to load recent files from dory.db: {}", e);
                Vec::new()
            }
        }
    }
...
fn parse_rfc3339(rfc3339: &str) -> i64 {
    ...
    dory_core::chrono::DateTime::parse_from_rfc3339(rfc3339)
...
fn chrono_utc_to_rfc3339(timestamp: i64) -> String {
    ...
    dory_core::chrono::DateTime::<Utc>::from(duration).to_rfc3339()
}

fn query_history_dto_from_entry(
    entry: &HistoryEntry,
) -> dory_storage::repositories::state::query_history::QueryHistoryDto {
    dory_storage::repositories::state::query_history::QueryHistoryDto {
...
fn saved_query_dto_from_query(
    query: &SavedQuery,
) -> dory_storage::repositories::state::saved_queries::SavedQueryDto {
    dory_storage::repositories::state::saved_queries::SavedQueryDto {
