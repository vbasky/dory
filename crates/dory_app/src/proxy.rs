...
use dory_proxy::{ProxyTunnel, ProxyTunnelConfig};

/// Creates a proxy tunnel and returns a type-erased handle plus the local port.
///
/// This is the bridge between `dory_core` (which cannot depend on
/// `dory_proxy`) and the actual tunnel implementation.
