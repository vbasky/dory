...
use dory_core::ResolvedProxy;
use dory_core::SshTunnelConfig;
...
impl AppAccessManager {
    ...
    fn open_ssh(
        ...
        let session = dory_ssh::establish_session(
            &resolved.config,
            resolved
                .secret
                .as_ref()
        ...
        let tunnel = dory_ssh::SshTunnel::start(session, remote_host.to_string(), remote_port)?;
    ...
    fn open_proxy(
        ...
        if let Some(patterns) = &resolved.profile.no_proxy
            && dory_core::host_matches_no_proxy(remote_host, patterns)
        {
            log::info!("Bypassing proxy for '{}' (no_proxy match)", remote_host);
            return Ok(AccessHandle::direct());
        }

        let config = dory_proxy::ProxyTunnelConfig::from_profile(
            &resolved.profile,
            resolved.secret.as_ref().map(|s| s.expose_secret()),
        );

        let tunnel = dory_proxy::ProxyTunnel::start(config, remote_host.to_string(), remote_port)?;
        let local_port = tunnel.local_port();
...
mod tests {
    ...
    use dory_core::DbError;
    use dory_core::access::{AccessKind, AccessManager};
    use dory_core::{ProxyAuth, ProxyKind, ProxyProfile, ResolvedProxy};
    use uuid::Uuid;
