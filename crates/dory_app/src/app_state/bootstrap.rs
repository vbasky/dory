...
use dory_driver_dynamodb::DynamoDriver;

#[cfg(feature = "cloudwatch")]
use dory_driver_cloudwatch::CloudWatchDriver;
...
#[cfg(test)]
use dory_driver_ipc::driver::IpcDriverLaunchConfig;
...
impl AppState {
    pub fn new() -> Result<Self, dory_storage::error::StorageError> {
    ...
    ) -> Result<Self, dory_storage::error::StorageError> {
        let (built, storage_runtime, profiles, auth_profiles, proxies, ssh_tunnels) =
    ...
    pub(super) fn new_with_drivers_and_settings(
        ...
        storage_runtime: dory_storage::bootstrap::StorageRuntime,
        profiles: Vec<ConnectionProfile>,
        auth_profiles: Vec<dory_core::AuthProfile>,
        proxies: Vec<dory_core::ProxyProfile>,
        ...
        let (audit_service, audit_degraded, audit_emitter) =
            Self::init_audit_backend(storage_runtime.dory_db_path());
        ...
        let mut state = Self {
            ...
            audit_service,
            audit_degraded,
            session_passphrase_vault: Arc::new(
                RwLock::new(dory_ssh::SessionPassphraseVault::new()),
            ),
    ...
    fn init_audit_backend(
        ...
        dory_audit::AuditService,
        bool,
        Arc<dyn dory_ipc::ExternalAuditEmitter>,
    ) {
        let (audit_service, audit_degraded) = match dory_audit::AuditService::new_sqlite(db_path) {
            Ok(service) => (service, false),
            Err(e) => {
                log::error!("Failed to initialize audit service at {:?}: {}", db_path, e);
                let store = dory_audit::store::sqlite::SqliteAuditStore::new(":memory:")
                    .expect("in-memory audit store must work: rusqlite :memory: unavailable");
                let svc = dory_audit::AuditService::new(store);
        ...
        let audit_emitter: Arc<dyn dory_ipc::ExternalAuditEmitter> =
    ...
    fn build_auth_provider_registry(
        services: Vec<ServiceConfig>,
        audit_emitter: Arc<dyn dory_ipc::ExternalAuditEmitter>,
    ) -> AuthProviderRegistry {
        let mut auth_provider_registry = AuthProviderRegistry::new();
        #[cfg(feature = "aws")]
        {
            auth_provider_registry.register(Arc::new(dory_aws::AwsSsoSessionAuthProvider::new()));
            auth_provider_registry.register(Arc::new(dory_aws::AwsSsoAuthProvider::new()));
            auth_provider_registry
                .register(Arc::new(dory_aws::AwsSharedCredentialsAuthProvider::new()));
        }
    ...
    fn build_session_facade(
        ...
        let ssh_manager =
            dory_core::SshTunnelManager::with_items(ssh_tunnels, None, "SSH tunnel profiles");
        let proxy_manager = dory_core::ProxyManager::with_items(proxies, None, "proxy profiles");
        let auth_manager =
            dory_core::AuthProfileManager::with_items(auth_profiles, None, "auth profiles");

        let tree_store: Box<dyn dory_core::TreeStore> =
            Box::new(dory_storage::sqlite_tree_store::SqliteTreeStore::new(
                storage_runtime.dory_db_path().to_path_buf(),
            ));
    ...
    fn init_mcp_runtime(
        ...
        let approval_store: Box<dyn dory_approval::PendingExecutionStore> =
    ...
    fn build_viz_repositories(
        ...
        dory_storage::error::StorageError,
    > {
        ...
        for idx in 0..count {
            ...
            let secret_field_ids: Vec<String> = provider
                ...
                .filter(|field| {
                    matches!(
                        field.kind,
                        dory_core::FormFieldKind::Password | dory_core::FormFieldKind::WriteOnly
                    )
                })
            ...
            for field_id in secret_field_ids {
                let secret_ref = dory_core::auth_field_secret_ref(&profile_id, &field_id);
                ...
                let secret = dory_core::secrecy::SecretString::from(plaintext);
    ...
    /// depends on `CachedAwsConfig` from `dory_aws`.
    ///
    ...
    fn build_default_drivers() -> Result<DefaultDriverBuild, dory_storage::error::StorageError> {
        let runtime = dory_storage::bootstrap::initialize()
    ...
    ) -> Result<DefaultDriverBuild, dory_storage::error::StorageError> {
    ...
    fn load_app_config_from_runtime(
        runtime: dory_storage::bootstrap::StorageRuntime,
    ) -> Result<
        (
            crate::config_loader::LoadedConfig,
            dory_storage::bootstrap::StorageRuntime,
        ),
        dory_storage::error::StorageError,
    > {
    ...
    fn launch_rpc_services(
        ...
        services: Vec<ServiceConfig>,
        audit_emitter: Option<Arc<dyn dory_ipc::ExternalAuditEmitter>>,
    ) {
    ...
    pub(super) fn launch_rpc_services_with<Probe, Build>(
        ...
        ) -> Result<DriverProbe, Box<dory_core::DbError>>,
        Build:
    ...
    pub(super) fn launch_rpc_auth_providers_with<Probe>(
        ...
        Probe: FnMut(
            &str,
            Option<&dory_ipc::IpcServiceLaunchConfig>,
        )
            -> Result<Arc<dyn dory_core::auth::DynAuthProvider>, Box<dory_core::DbError>>,
    {
        for discovery in discover_services(services) {
            let descriptor = match discovery {
                RpcServiceDiscovery::Descriptor(descriptor) => descriptor,
