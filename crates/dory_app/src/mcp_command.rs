...
pub fn run_mcp_command(args: &[String]) -> i32 {
    match parse_mcp_args(args) {
        Ok(mcp_args) => {
            ...
            // Run async server
            match rt.block_on(dory_mcp_server::run_mcp_server(mcp_args)) {
                Ok(_) => 0,
                Err(e) => {
...
#[cfg(feature = "mcp")]
fn parse_mcp_args(args: &[String]) -> Result<dory_mcp_server::McpServerArgs, String> {
    ...
    Ok(dory_mcp_server::McpServerArgs {
        client_id,
        config_dir,
    })
}
...
#[cfg(feature = "mcp")]
fn print_mcp_help() {
    eprintln!("Usage: dory mcp --client-id <id> [options]");
    eprintln!();
    eprintln!("Run Dory as an MCP (Model Context Protocol) server for AI clients.");
    eprintln!();
    eprintln!("Options:");
    eprintln!("  --client-id <id>      Identifier for this AI client (required)");
    eprintln!("                        Must match a trusted client in MCP settings");
    eprintln!("  --config-dir <path>   Override config directory (default: ~/.local/share/dory)");
    eprintln!("  --help, -h            Show this help message");
    eprintln!();
    eprintln!("Example:");
    eprintln!("  dory mcp --client-id claude-desktop");
    eprintln!();
