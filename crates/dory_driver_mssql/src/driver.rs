...
fn password_fingerprint(password: &str) -> String {
    ...
    hasher.update(b"dory-mssql-password-fingerprint:v1");
...
fn tiberius_column_to_kind(column_type: tiberius::ColumnType) -> dory_core::ColumnKind {
...
impl Connection for MssqlConnection {
    ...
    ///
    /// Overridden because the default impl in `dory_core` (a) emits the
...
impl MssqlConnection {
    ...
    fn fetch_custom_types(
        ...
        // User-defined alias types map most cleanly to Dory's Domain kind
...
// Local indexmap usage so we don't depend on dory_core's indexmap re-export.
...
fn extract_location_from_message(message: &str) -> dory_core::ErrorLocation {
    let mut location = dory_core::ErrorLocation::new();
