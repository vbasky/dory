...
//! Run with: `cargo nextest run -p dory_driver_postgres --run-ignored all`
#![allow(
    clippy::unwrap_used,
    clippy::expect_used,
    clippy::panic,
...
use dory_core::{ConnectionProfile, DbConfig, DbDriver, DbError, QueryRequest, Value};
use dory_driver_postgres::PostgresDriver;
use dory_test_support::containers;
use std::time::Duration;

fn connect_postgres(uri: String) -> Result<Box<dyn dory_core::Connection>, DbError> {
...
fn setup_schema(connection: &dyn dory_core::Connection) -> Result<(), DbError> {
...
fn pg_relational_filter_single_hop() -> Result<(), DbError> {
    containers::with_postgres_url(|uri| {
        ...
        let lowering = dory_core::parse_and_resolve(
        ...
        let select = dory_core::select_query_from_spec(&lowering.spec, dialect).expect("build SQL");
...
fn pg_count_parity() -> Result<(), DbError> {
    containers::with_postgres_url(|uri| {
        ...
        let select =
            dory_core::select_query_from_spec(&lowering.spec, dialect).expect("build data SQL");
        let count = dory_core::count_query_from_spec(&lowering.spec, dialect);
