...
fn static_catalog_shape_matches_expected_metrics() {
    let metrics = dory_driver_postgres::instance_catalog::PgInstanceCatalog::static_metrics();
    ...
    containers::with_postgres_url(|uri| -> Result<(), DbError> {
        ...
        for metric_id in numeric_metrics {
            ...
            if metric_id == "pg.cache_hit_ratio"
                && let Some(dory_core::Value::Float(ratio)) = result.rows[0].get(1)
            {
                assert!(
...
fn pg_stat_statements_gating_logic() {
    let metrics_without =
        dory_driver_postgres::instance_catalog::PgInstanceCatalog::metrics_with_probe(false);
    let metrics_with =
        dory_driver_postgres::instance_catalog::PgInstanceCatalog::metrics_with_probe(true);
