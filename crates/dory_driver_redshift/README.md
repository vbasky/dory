...
Amazon Redshift driver for Dory (read-only v1), built directly on the [`postgres`](https://crates.io/crates/postgres) wire client rather than on `dory_driver_postgres`.
...
- The extended-type OID values used for `SUPER`/`VARBYTE`/`GEOMETRY`/`GEOGRAPHY`/`HLLSKETCH`, and the exact `SVV_TABLE_INFO`/`PG_TABLE_DEF` query shapes used for storage hints, are validated only by `#[ignore]`d live integration tests (`crates/dory_driver_redshift/tests/live_integration.rs`), since no local or Docker-based Redshift engine exists. Run them explicitly against a real cluster with `cargo nextest run -p dory_driver_redshift --run-ignored all`.
