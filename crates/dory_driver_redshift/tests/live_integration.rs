...
//!
//! ```text
//! DORY_TEST_REDSHIFT_HOST=cluster.abc123.us-east-1.redshift.amazonaws.com \
//! DORY_TEST_REDSHIFT_USER=awsuser \
//! DORY_TEST_REDSHIFT_PASSWORD=... \
//! DORY_TEST_REDSHIFT_DATABASE=dev \
//! cargo nextest run -p dory_driver_redshift --run-ignored all
//! ```
//!
...
//!
//! - `DORY_TEST_REDSHIFT_SCHEMA` (defaults to `public`)
//! - `DORY_TEST_REDSHIFT_EMPTY_SCHEMA` — a schema with zero tables/views
//! - `DORY_TEST_REDSHIFT_TABLE` — a table with a declared DISTKEY and SORTKEY
//! - `DORY_TEST_REDSHIFT_PK_TABLE` — a table with a declared (informational) PRIMARY KEY
//! - `DORY_TEST_REDSHIFT_SUPER_TABLE` / `DORY_TEST_REDSHIFT_SUPER_COLUMN` — a table/column of type SUPER or VARBYTE

use dory_core::secrecy::SecretString;
use dory_core::{ConnectionProfile, DbConfig, DbDriver, DbError, QueryRequest};
use dory_driver_redshift::RedshiftDriver;
...
impl LiveRedshiftEnv {
    fn from_env() -> Self {
        let host = std::env::var("DORY_TEST_REDSHIFT_HOST").expect(
            "DORY_TEST_REDSHIFT_HOST must be set to run Redshift live tests (see module docs)",
        );
        let port = std::env::var("DORY_TEST_REDSHIFT_PORT")
            .ok()
            .and_then(|value| value.parse().ok())
            .unwrap_or(5439);
        let user =
            std::env::var("DORY_TEST_REDSHIFT_USER").unwrap_or_else(|_| "awsuser".to_string());
        let password = std::env::var("DORY_TEST_REDSHIFT_PASSWORD")
            .expect("DORY_TEST_REDSHIFT_PASSWORD must be set to run Redshift live tests");
        let database =
            std::env::var("DORY_TEST_REDSHIFT_DATABASE").unwrap_or_else(|_| "dev".to_string());
...
#[test]
#[ignore = "requires a real Amazon Redshift cluster with an empty schema; see DORY_TEST_REDSHIFT_EMPTY_SCHEMA"]
fn redshift_live_empty_schema_lists_zero_tables_without_error() -> Result<(), DbError> {
    let env = LiveRedshiftEnv::from_env();
    let empty_schema = std::env::var("DORY_TEST_REDSHIFT_EMPTY_SCHEMA")
        .expect("DORY_TEST_REDSHIFT_EMPTY_SCHEMA must name a schema with zero tables/views");
...
#[test]
#[ignore = "requires a real Amazon Redshift table with a declared DISTKEY and SORTKEY; see DORY_TEST_REDSHIFT_TABLE"]
fn redshift_live_table_details_surface_distkey_and_sortkey() -> Result<(), DbError> {
    let env = LiveRedshiftEnv::from_env();
    let schema =
        std::env::var("DORY_TEST_REDSHIFT_SCHEMA").unwrap_or_else(|_| "public".to_string());
    let table = std::env::var("DORY_TEST_REDSHIFT_TABLE")
        .expect("DORY_TEST_REDSHIFT_TABLE must name a table with a DISTKEY and SORTKEY");
...
#[test]
#[ignore = "requires a real Amazon Redshift table with a declared (informational) primary key; see DORY_TEST_REDSHIFT_PK_TABLE"]
fn redshift_live_declared_primary_key_is_advisory_with_no_fabricated_index() -> Result<(), DbError>
{
    let env = LiveRedshiftEnv::from_env();
    let schema =
        std::env::var("DORY_TEST_REDSHIFT_SCHEMA").unwrap_or_else(|_| "public".to_string());
    let table = std::env::var("DORY_TEST_REDSHIFT_PK_TABLE")
        .expect("DORY_TEST_REDSHIFT_PK_TABLE must name a table with a declared PRIMARY KEY");
...
#[test]
#[ignore = "requires a real Amazon Redshift table with a SUPER or VARBYTE column; see DORY_TEST_REDSHIFT_SUPER_TABLE/DORY_TEST_REDSHIFT_SUPER_COLUMN"]
fn redshift_live_super_or_varbyte_column_renders_as_text_without_panic() -> Result<(), DbError> {
    let env = LiveRedshiftEnv::from_env();
    let schema =
        std::env::var("DORY_TEST_REDSHIFT_SCHEMA").unwrap_or_else(|_| "public".to_string());
    let table = std::env::var("DORY_TEST_REDSHIFT_SUPER_TABLE")
        .expect("DORY_TEST_REDSHIFT_SUPER_TABLE must name a table with a SUPER/VARBYTE column");
    let column = std::env::var("DORY_TEST_REDSHIFT_SUPER_COLUMN")
        .expect("DORY_TEST_REDSHIFT_SUPER_COLUMN must name that SUPER/VARBYTE column");
    ...
    assert_eq!(result.columns[0].kind, dory_core::ColumnKind::Text);

    if let Some(row) = result.rows.first() {
        match &row[0] {
            dory_core::Value::Text(_) | dory_core::Value::Null => {}
            other => {
...
fn redshift_live_select_into_is_rejected_at_the_wire_not_executed() {
    ...
    let table_name = format!("dory_select_into_probe_{}", uuid::Uuid::new_v4().simple());
...
///
/// ```text
/// DORY_TEST_REDSHIFT_HOST=... \
/// DORY_TEST_REDSHIFT_PASSWORD=... \
/// DORY_TEST_REDSHIFT_SSL_ROOT_CERT=/path/to/private-ca.pem \
/// DORY_TEST_REDSHIFT_SSL_CLIENT_CERT=/path/to/client.pem \
/// DORY_TEST_REDSHIFT_SSL_CLIENT_KEY=/path/to/client-key.pem \
/// cargo nextest run -p dory_driver_redshift --run-ignored all \
...
fn redshift_live_verify_full_with_private_ca_and_client_cert() {
    ...
    let ssl_root_cert_path = std::env::var("DORY_TEST_REDSHIFT_SSL_ROOT_CERT").ok();
    let ssl_client_cert_path = std::env::var("DORY_TEST_REDSHIFT_SSL_CLIENT_CERT").ok();
    let ssl_client_key_path = std::env::var("DORY_TEST_REDSHIFT_SSL_CLIENT_KEY").ok();
