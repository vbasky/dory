...
fn ensure_read_only(sql: &str) -> Result<(), DbError> {
    ...
    if is_select_into(sql) {
        return Err(DbError::NotSupported(
            "Amazon Redshift connections are read-only in Dory; SELECT ... INTO creates a table and is not supported".to_string(),
        ));
    }

    if let Some(keyword) = nested_write_keyword(sql) {
        return Err(DbError::NotSupported(format!(
            "Amazon Redshift connections are read-only in Dory; {keyword} is not supported"
        )));
    }
    ...
    match classify_query_for_language(&QueryLanguage::Sql, sql) {
        ...
        _ => Err(DbError::NotSupported(
            "Amazon Redshift connections are read-only in Dory; only SELECT/EXPLAIN/SHOW statements are supported".to_string(),
        )),
    }
}
...
mod tests {
    ...
    fn write_temp_file(label: &str, contents: &[u8]) -> std::path::PathBuf {
        ...
        let path = std::env::temp_dir().join(format!(
            "dory_redshift_tls_{label}_{}_{unique}",
            std::process::id()
        ));
    ...
    fn load_root_certificate_errors_on_missing_path() {
        let missing = std::env::temp_dir().join("dory_redshift_tls_missing_ca_does_not_exist");
    ...
    // `native_tls::Identity::from_pkcs8` is backed by Security.framework on
    // macOS, which rejects this PKCS#8 PEM pair with "Unknown format in import".
    // The same fixture is valid on the OpenSSL backend used on Linux/Windows.
    #[test]
    ...
    fn load_client_identity_errors_on_unreadable_cert_path() {
        ...
        let certs = RedshiftTlsCerts {
            ...
            client_cert_path: Some(
                std::env::temp_dir()
                    .join("dory_redshift_tls_missing_client_cert")
                    .to_string_lossy()
                    .to_string(),
            ),
            client_key_path: Some(key_path.to_string_lossy().to_string()),
