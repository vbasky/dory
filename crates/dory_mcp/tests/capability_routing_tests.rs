...
use dory_mcp::handlers::discovery_schema::{
    ConnectionInfo, ConnectionMetadata, DiscoverySchemaCatalog,
};
use dory_policy::{ExecutionClassification, PolicyEngine, PolicyEvaluationRequest};
...
fn query_handler_classifies_queries_by_language() {
    ...
    let sql_classification =
        dory_core::classify_query_for_governance(&QueryLanguage::Sql, "SELECT * FROM users");
    assert!(matches!(sql_classification, ExecutionClassification::Read));

    let mongo_classification =
        dory_core::classify_query_for_governance(&QueryLanguage::MongoQuery, "db.users.find({})");
    assert!(matches!(
...
fn syntax_info_enables_driver_agnostic_query_rendering() {
    ...
    assert_eq!(
        ...
        dory_core::PlaceholderStyle::DollarNumber
    );
