...
use dory_core::observability::EventCategory;
use dory_core::observability::actions::MCP_AUTHORIZE;
use dory_mcp::handlers::query::{QueryExecutionRequest, QueryHandlerError, handle_query_tool};
use dory_mcp::handlers::scripts::{ScriptHandler, ScriptHandlerError, ScriptLifecycleState};
use dory_mcp::server::authorization::{AuthorizationRequest, authorize_request};
use dory_mcp::server::bootstrap::{
    BootstrapConfig, BootstrapError, TransportKind, validate_v1_transport_profile,
};
use dory_mcp::server::request_context::RequestIdentity;
use dory_mcp::server::router::{RouteTarget, route_tool};
use dory_policy::{
...
fn authorization_denies_untrusted_and_audits_reason() {
    let audit_service = fresh_audit_service("dory-mcp-authz-test.sqlite");
