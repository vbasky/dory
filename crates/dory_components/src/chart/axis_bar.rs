...
use dory_core::{ColumnKind, ColumnMeta};
...
fn agg_label(kind: AggKind) -> String {
    match kind {
        AggKind::None => dory_i18n::t!("chart.axis_bar.aggregation.none"),
        AggKind::Sum => dory_i18n::t!("chart.axis_bar.aggregation.sum"),
        AggKind::Avg => dory_i18n::t!("chart.axis_bar.aggregation.avg"),
        AggKind::Min => dory_i18n::t!("chart.axis_bar.aggregation.min"),
        AggKind::Max => dory_i18n::t!("chart.axis_bar.aggregation.max"),
    }
}
    ...
    FAgg: Fn(AggKind, &mut Window, &mut App) + Clone + Send + Sync + 'static,
{
    ...
    // Group pill
    let group_role = dory_i18n::t!("chart.axis_bar.group");
    let group_pill = {
        let handler = on_pill_click.clone();
        pill_element(
            "axis-pill-group",
...
mod tests {
    ...
    fn agg_label_sum_diverges_between_locales() {
        let en = dory_i18n::t!("chart.axis_bar.aggregation.sum", locale = "en");
        let es = dory_i18n::t!("chart.axis_bar.aggregation.sum", locale = "es");
        assert_eq!(en, "sum");
        assert_eq!(es, "suma");
        assert_ne!(en, es);
    }
    ...
    fn axis_bar_group_key_resolves() {
        let en = dory_i18n::t!("chart.axis_bar.group", locale = "en");
        let es = dory_i18n::t!("chart.axis_bar.group", locale = "es");
        assert_eq!(en, "Group");
        assert_eq!(es, "Grupo");
        assert_ne!(en, es);
    }
