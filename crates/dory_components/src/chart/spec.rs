...
impl ChartSpec {
    ...
    pub fn from_bindings(
        ...
        let axis_kind = if x_col_meta.kind == dory_core::ColumnKind::Timestamp {
            AxisKind::Time
        } else {
            AxisKind::Numeric
        };
...
mod tests {
    ...
    fn make_col_meta(name: &str, kind: dory_core::ColumnKind) -> dory_core::ColumnMeta {
        dory_core::ColumnMeta {
            name: name.to_owned(),
    ...
    fn three_col_meta() -> Vec<dory_core::ColumnMeta> {
        vec![
            make_col_meta("ts", dory_core::ColumnKind::Timestamp),
            make_col_meta("cpu", dory_core::ColumnKind::Float),
            make_col_meta("mem", dory_core::ColumnKind::Float),
        ]
    }
    ...
    fn from_bindings_numeric_x_uses_numeric_axis_kind() {
        let cols = vec![
            make_col_meta("seq", dory_core::ColumnKind::Integer),
            make_col_meta("val", dory_core::ColumnKind::Float),
        ];
        let bindings = BindingSpec {
            x: 0,
            y: vec![1],
