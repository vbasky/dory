...
pub fn count_columns_for_why(columns: &[dory_core::ColumnMeta]) -> (usize, usize) {
    let numeric = columns
        .iter()
        .filter(|c| {
            matches!(
                c.kind,
                dory_core::ColumnKind::Float | dory_core::ColumnKind::Integer
            )
        })
        .count();
    let timestamp = columns
        .iter()
        .filter(|c| matches!(c.kind, dory_core::ColumnKind::Timestamp))
        .count();
    (numeric, timestamp)
}
