...
//! This keeps `dory_components` free of GPUI / executor dependencies and makes
...
//! in the host crate (`dory_ui`), not here.
...
///
/// Distinct from [`dory_core::ResolvedWindow`], which is a **driver-output**
...
/// A local-audit chart source that aggregates the Dory audit SQLite store.
///
/// Returns `ChartDataPlan::LocalAudit(spec)` — no IO, no connection, no store
/// handle. The host executor in `dory_ui` performs the actual aggregation via
/// `dory_audit::AuditService::aggregate`.
///
...
/// 1. Add a `SavedChartSource` variant in `dory_components::saved_chart`.
...
///
/// No changes to `dory_ui` are required. `ChartDocument::request_reexecute`
...
mod tests {
    ...
    /// This is a compilation-level check: if dory_storage/dory_audit/gpui types
    /// leaked into data_source.rs the crate would fail to build (those crates are not
    /// in dory_components' dependencies). This test exists as documentation of the
    ...
    fn purity_guard_no_io_or_gpui_types_at_compile_time() {
        ...
        // dory_components does not depend on dory_storage, dory_audit,
