...
pub enum TextVariant {
    ...
    Code,
    /// Chrome label that uses mono metrics (size/weight) but follows the user's
    /// UI font setting. Used by status bar, tab bars, and sidebar chrome where
    /// a compact fixed-width *look* is desired but the family should match the
    /// rest of the UI.
    MonoChrome,
...
impl Text {
    ...
    pub fn mono_chrome(content: impl Into<SharedString>) -> Self {
        Self::from_variant(TextVariant::MonoChrome, content)
    }
...
impl TextVariant {
    ...
    pub(crate) fn role_contract(self) -> TextRoleContract {
        match self {
            ...
            Self::MonoChrome => TextRoleContract {
...
impl RenderOnce for Text {
    fn render(self, _window: &mut Window, cx: &mut App) -> impl IntoElement {
        ...
        // Only inline code stays on the bundled mono family. Every other role
        // — including variants whose contract leaves `family` unset so they
        // inherit GPUI's default — follows the user's FontSetting. Leaving
        // `family: None` on the theme default used to pick up a Nerd Font
        // written into `theme.font_family` and break hit-testing.
        let is_code_role = matches!(self.variant, TextVariant::Code);
        let resolved_family = if is_code_role {
            contract.family.unwrap_or(AppFonts::MONO)
        } else {
            crate::typography::AppFonts::current_ui_family(cx)
        };

        if is_code_role && !contract.fallbacks.is_empty() {
            let mut text_font = font(resolved_family);
            ...
            el.font_family(resolved_family)
        }
    }
}
...
mod tests {
    ...
    fn shared_typography_roles_expose_expected_font_contracts() {
        ...
        assert_eq!(sidebar.family, Some(AppFonts::BODY));
    ...
    fn shared_body_and_mono_roles_keep_expected_defaults() {
        ...
        assert_eq!(key_hint.family, Some(AppFonts::BODY));
        assert_eq!(key_hint.fallbacks, NO_FALLBACKS);
