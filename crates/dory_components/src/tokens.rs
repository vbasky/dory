...
impl SyntaxColors {
    /// Dark-palette syntax colors (VS Code Dark+ inspired). These are the
    /// historical values and remain the default for dark themes.
    pub fn table() -> Hsla {
        rgb(0x4EC9B0).into()
    }
    ...
    /// Light-palette syntax colors. The dark-palette pastels wash out on
    /// light backgrounds, so light themes use deeper, higher-contrast
    /// equivalents that stay legible on near-white surfaces.
    pub fn table_light() -> Hsla {
        rgb(0x00897B).into()
    }

    pub fn view_light() -> Hsla {
        rgb(0xB45309).into()
    }

    pub fn column_light() -> Hsla {
        rgb(0x1D4ED8).into()
    }

    pub fn type_item_light() -> Hsla {
        rgb(0x7C3AED).into()
    }

    pub fn folder_dim_light() -> Hsla {
        rgb(0x57606A).into()
    }

    pub fn database_light() -> Hsla {
        rgb(0xB35900).into()
    }

    pub fn schema_light() -> Hsla {
        rgb(0x0550AE).into()
    }

    /// Resolve the syntax color for the active theme setting.
    pub fn resolve(setting: dory_core::ThemeSetting, kind: SyntaxColorKind) -> Hsla {
        let is_light = !setting.is_dark();
        match kind {
            SyntaxColorKind::Table if is_light => Self::table_light(),
            SyntaxColorKind::View if is_light => Self::view_light(),
            SyntaxColorKind::Column if is_light => Self::column_light(),
            SyntaxColorKind::TypeItem if is_light => Self::type_item_light(),
            SyntaxColorKind::FolderDim if is_light => Self::folder_dim_light(),
            SyntaxColorKind::Database if is_light => Self::database_light(),
            SyntaxColorKind::Schema if is_light => Self::schema_light(),
            SyntaxColorKind::Table => Self::table(),
            SyntaxColorKind::View => Self::view(),
            SyntaxColorKind::Column => Self::column(),
            SyntaxColorKind::TypeItem => Self::type_item(),
            SyntaxColorKind::FolderDim => Self::folder_dim(),
            SyntaxColorKind::Database => Self::database(),
            SyntaxColorKind::Schema => Self::schema(),
        }
    }
}

/// Kind of sidebar syntax color to resolve.
#[derive(Clone, Copy)]
pub enum SyntaxColorKind {
    Table,
    View,
    Column,
    TypeItem,
