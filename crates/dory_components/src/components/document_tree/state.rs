...
use dory_core::{QueryResult, Value};
use gpui::{
...
impl DocumentTreeState {
    ...
    fn extract_document_value(row: &[Value], columns: &[dory_core::ColumnMeta]) -> Value {
...
fn document_label(index: usize) -> String {
    dory_i18n::t!("document_tree.document_label", index = index)
}
...
mod tests {
    ...
    use dory_core::{ColumnKind, ColumnMeta, Value};
    ...
    fn document_label_interpolates_index() {
        ...
        assert_eq!(
            label,
            dory_i18n::t!("document_tree.document_label", index = 3)
        );
