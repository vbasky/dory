...
pub fn validate_json(s: &str, allow_empty: bool) -> Result<(), String> {
    if s.is_empty() {
        return if allow_empty {
            ...
            Err(dory_i18n::t!("components.json_editor.empty_error"))
        };
    }
            ...
            if let Some(on_format) = self.on_format {
                left_buttons = left_buttons.child(
                    Button::new(SharedString::from(format!("{}-format", prefix)))
                        .label(dory_i18n::t!("components.json_editor.format"))
                        .small()
            ...
            if let Some(on_compact) = self.on_compact {
                left_buttons = left_buttons.child(
                    Button::new(SharedString::from(format!("{}-compact", prefix)))
                        .label(dory_i18n::t!("components.json_editor.compact"))
                        .small()
                        .with_variant(ButtonVariant::Ghost)
                        .on_click(on_compact),
                );
        ...
        let right_buttons = div()
            ...
            .child(
                Button::new(SharedString::from(format!("{}-cancel", prefix)))
                    .label(dory_i18n::t!("components.json_editor.cancel"))
                    .small()
                    .with_variant(ButtonVariant::Ghost)
            ...
            .child(
                Button::new(SharedString::from(format!("{}-save", prefix)))
                    .label(dory_i18n::t!("components.json_editor.save"))
                    .small()
...
mod tests {
    ...
    fn validate_json_empty_error_matches_translated_catalog() {
        ...
        assert_eq!(
            error,
            dory_i18n::t!("components.json_editor.empty_error", locale = "en")
        );
    }

    #[test]
    fn validate_json_empty_error_diverges_between_locales() {
        let en = dory_i18n::t!("components.json_editor.empty_error", locale = "en");
        let es = dory_i18n::t!("components.json_editor.empty_error", locale = "es");
        assert_ne!(en, es);
    }
