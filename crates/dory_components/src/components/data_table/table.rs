        ...
        .map(|visual_ix| {
            ...
            let (row_data, pending_insert_data, row_state, data_row_ix) = match source {
                ...
                Some(VisualRowSource::Insert(insert_idx)) => {
                    ...
                    (None, data, dory_core::RowState::PendingInsert, visual_ix)
                }
                None => {
                    // Should not happen, but handle gracefully
                    (None, None, dory_core::RowState::Clean, visual_ix)
                }
            };
            ...
            let row_bg = match row_state {
                dory_core::RowState::Dirty => None, // Cell-level only — see dirty cell highlight below
                dory_core::RowState::Saving => Some(RowColors::saving(theme)),
                dory_core::RowState::Error(_) => Some(RowColors::error(theme)),
                dory_core::RowState::Clean => None,
                dory_core::RowState::PendingInsert => Some(RowColors::insert(theme)),
                dory_core::RowState::PendingDelete => Some(RowColors::delete(theme)),
            };
