...
//! `dory_ui_base::SavedChartManager` backed by SQLite.
//!
//! # Crate placement
//!
//! `SavedChart` lives in `dory_components` rather than `dory_core` because
...
use dory_core::{CollectionRef, Identifiable, ResolvedWindow};
...
///
/// Mirrors the variants in `dory_ui::ui::common::time_range::TimeRange` but
