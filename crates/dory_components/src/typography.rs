...
impl AppFonts {
    /// System UI font family name. GPUI resolves this to the platform default
    /// (SF Pro on macOS, Segoe UI on Windows, the desktop font on Linux).
    pub const SYSTEM: &'static str = ".SystemUIFont";

    pub const HEADLINE: &'static str = Self::SYSTEM;
    pub const BODY: &'static str = Self::SYSTEM;
    /// Monospace family used for SQL, code, and numeric chrome. Resolves to the
    /// platform's default monospace font (Menlo on macOS, Consolas on Windows,
    /// the desktop mono font on Linux).
    pub const MONO: &'static str = "monospace";
    pub const MONO_FALLBACK: &'static str =
        "ui-monospace, SFMono-Regular, Menlo, Consolas, monospace";
    pub const CODE: &'static str = Self::MONO;
    pub const SHORTCUT: &'static str = Self::MONO;

    /// The font family to use for non-mono UI text (body, labels, headlines)
    /// given the active `FontSetting`. An empty family means the platform
    /// system font.
    pub fn ui_family(setting: &dory_core::FontSetting) -> &'static str {
        let setting = setting.clone().sanitize_for_ui();
        if setting.is_system() {
            Self::SYSTEM
        } else {
            // Font families are not 'static by nature, but GPUI's font APIs
            // want a family string. We leak the family name once so it lives
            // for the process lifetime — the set of fonts a user picks is tiny
            // and stable, so this is bounded.
            Box::leak(setting.family.into_boxed_str())
        }
    }

    /// Read the active UI font family from the GPUI context. Falls back to the
    /// platform system font when `FontSettingGlobal` has not been registered.
    pub fn current_ui_family(cx: &App) -> &'static str {
        Self::ui_family(&crate::semantic::FontSettingGlobal::get(cx))
    }
}
...
impl MonoLabel {
    ...
    fn build_text(
        ...
        let text = Text::mono_chrome(text).font_size(size);
...
mod typography_role_tests {
    ...
    #[test]
    fn mono_label_uses_shared_ui_family_with_label_metrics() {
        let text = MonoLabel::new("driver-key").text();

        let contract = text.role_contract();
        assert_eq!(contract.family, Some(AppFonts::BODY));
        assert_eq!(contract.fallbacks, &[] as &[&str]);
    ...
    #[test]
    fn mono_caption_uses_shared_ui_family_with_caption_metrics() {
    ...
    #[test]
    fn mono_meta_uses_shared_ui_family_with_small_metadata_metrics() {
        let text = MonoMeta::new("dory-postgres").text();
        ...
        let inspection = MonoMeta::new("dory-postgres").inspect();
        assert_eq!(
            inspection.color_selection,
            MonoColorSelection::MutedForeground
        );
    ...
    fn mono_helpers_allow_explicit_color_overrides_without_losing_mono_contract() {
        ...
        assert_eq!(label.role_contract().family, Some(AppFonts::BODY));
        assert_eq!(caption.role_contract().family, Some(AppFonts::BODY));
        assert_eq!(meta.role_contract().family, Some(AppFonts::BODY));
    ...
    fn mono_helpers_allow_explicit_size_and_weight_overrides() {
        ...
        let meta = MonoMeta::new("dory-postgres")
...
#[cfg(test)]
mod tests {
    use super::{AppFonts, Body, Caption, FieldLabel, Headline, RequiredMarker};
    ...
    #[test]
    fn ui_roles_use_the_system_font_and_code_stays_on_the_system_mono() {
