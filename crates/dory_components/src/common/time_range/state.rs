...
use dory_core::chrono::{DateTime, Local, NaiveDate, NaiveDateTime, NaiveTime, TimeZone, Utc};
...
pub fn timestamp_from_date_time(
    ...
    let time = NaiveTime::from_hms_opt(hour, minute, 0)
        .ok_or_else(|| dory_i18n::t!("common.time_range.error.invalid_selection"))?;
    ...
    let timestamp = match mode {
        ...
        TimestampDisplayMode::Local => Local
            ...
            .single()
            .ok_or_else(|| dory_i18n::t!("common.time_range.error.ambiguous_local"))?
            .timestamp_millis(),
    };
...
pub fn validate_custom_range_parts(
    ...
    let start_ms = timestamp_from_date_time(start_date, start_hour, start_minute, mode)
        .map_err(|error| dory_i18n::t!("common.time_range.error.invalid_start", error = error))?;
    let end_ms = timestamp_from_date_time(end_date, end_hour, end_minute, mode)
        .map_err(|error| dory_i18n::t!("common.time_range.error.invalid_end", error = error))?;

    if start_ms > end_ms {
        return Err(dory_i18n::t!("common.time_range.error.start_after_end"));
    }
...
mod tests {
    ...
    fn custom_range_parts_require_ordered_date_time_values() {
        ...
        assert_eq!(
            result,
            Err(dory_i18n::t!("common.time_range.error.start_after_end"))
        );
    }
    ...
    fn time_range_start_after_end_diverges_between_locales() {
        let en = dory_i18n::t!("common.time_range.error.start_after_end", locale = "en");
        let es = dory_i18n::t!("common.time_range.error.start_after_end", locale = "es");
        assert_ne!(en, es);
    }
}
