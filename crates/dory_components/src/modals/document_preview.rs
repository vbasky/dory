...
impl Render for DocumentPreviewModal {
    fn render(&mut self, _window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        ModalFrame::new("document-preview-modal", &self.focus_handle, close)
            ...
            .title(dory_i18n::t!("modals.document_preview.title"))
            .width(px(1000.0))
            .height(px(700.0))
            .top_offset(px(60.0))
            .block_scroll()
...
mod tests {
    ...
    fn document_preview_title_diverges_between_locales() {
        let en = dory_i18n::t!("modals.document_preview.title", locale = "en");
        let es = dory_i18n::t!("modals.document_preview.title", locale = "es");
        assert_ne!(en, es);
    }
}
