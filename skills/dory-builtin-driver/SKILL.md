---
name: dory-builtin-driver
description: >
  Add native built-in Dory database drivers using the repository's driver traits and registration flow.
  Trigger: When adding a built-in Dory driver, native database adapter, or `crates/dory_driver_*` crate.
license: MIT
---

## When to Use

- The user wants database support compiled into Dory.
- The change creates or extends `crates/dory_driver_<name>/`.
- The driver should appear without configuring an RPC service.

## Source of Truth

- `ARCHITECTURE.md`
- `crates/dory_core/src/core/traits.rs`
- `crates/dory_core/src/driver/capabilities.rs`
- `crates/dory_core/src/driver/form.rs`
- `crates/dory_app/src/app_state.rs`
- Existing `crates/dory_driver_*/` crates and their `README.md` files
...
- Implement `QueryGenerator` when Dory must generate native read/mutation templates or MCP previews.
- Keep schema loading shallow by default; fetch expensive details on demand.

## Checklist

1. Create or update `crates/dory_driver_<name>/`.
2. Add workspace and crate manifest entries following existing driver crates.
3. Add the feature flag in the `dory` crate manifest.
4. Register the driver in `AppState::build_builtin_drivers()` behind that feature flag.
5. Add `crates/dory_driver_<name>/README.md` with only **Features** and **Limitations**.
6. Add focused tests for metadata, config/form parsing, generated queries, and non-live behavior.

## Boundaries

- No concrete-driver checks in `dory_ui` or app workflow code.
...
```bash
cargo fmt --all -- --check
cargo check -p dory --features <driver-feature>
cargo test -p dory_driver_<name>
