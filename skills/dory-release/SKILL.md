...
name: dory-release
description: >
  Prepare and create Dory releases under the trunk + short-lived release-branch model
  ...
  or prepare release notes for Dory.
license: MIT
---
...
Dory uses **trunk + short-lived release branches**.
...
The flake exposes a prebuilt-binary package (`dory-bin`, default) backed by `nix/release-info.nix`. Refresh it after **any published tag** (rc or stable):
