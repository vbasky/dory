---
name: dory-rpc-driver
description: >
  Create external Dory database drivers that speak the Driver RPC protocol.
...
- The driver should live outside the Dory binary.
- The user mentions RPC services, external drivers, plugins, sockets, or `dory-driver-host`.
...
- Dory stores launch config in `cfg_services`, `cfg_service_args`, and `cfg_service_env` and probes services on startup.
- Internal registry key is `rpc:<socket_id>`.
- The service is the source of truth for driver kind, metadata, and form definition.
- Restart Dory after changing persisted RPC service settings.
...
- Empty `command` and empty `args`: Dory expects the service to already be running.
- Empty `command` with non-empty `args`: Dory launches `dory-driver-host` for driver services.
- Explicit `command`: Dory starts and owns that process, then kills it on shutdown.
