//! Example custom driver for Dory
//!
//! This is a minimal implementation of the Dory driver protocol.
//! It simulates a simple key-value database for testing purposes.
//!
//! Usage:
//!   cargo run --bin custom-driver -- --socket my-driver.sock
//!
//! Then register the binary from Settings → RPC Services in Dory.
//! If Dory manages the service, keep the launch command configured.
//! If you want a manual/already-running service, leave both `command` and `args` empty in Dory.
                        ...
                        if endpoint.trim().is_empty() {
                            ...
                            DriverResponseEnvelope::ok(
                                ...
                                DriverResponseBody::SessionOpened {
                                    ...
                                    schema_features: dory_core::SchemaFeatures::empty(),
                                    code_gen_capabilities: dory_core::CodeGenCapabilities::empty(
                                    ),
                                },
                            )
                        }
    ...
    if sql_lower.starts_with("select") {
        ...
        let columns = vec![
            ColumnMeta {
                ...
                kind: dory_core::ColumnKind::Unknown,
                nullable: false,
...
fn create_metadata() -> DriverMetadata {
    DriverMetadata {
        ...
        description: "Example custom driver for testing Dory integration".into(),
...
fn parse_args() -> Args {
    ...
    while let Some(arg) = args.next() {
        match arg.as_str() {
            ...
            "--help" | "-h" => {
                println!("Custom Driver Example for Dory");
                println!();
