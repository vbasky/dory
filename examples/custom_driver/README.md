...
This example is a standalone driver host that speaks the Dory Driver RPC protocol.
...
5. Save the service. Dory persists it in the internal SQLite-backed settings store.
...
- If Dory starts this service from `Settings → RPC Services`, Dory tracks it and stops it on Dory shutdown.
- If you start the service manually and leave both `command` and `args` empty, Dory uses the running socket but does not own or stop that process.

## Notes

- The service key used internally by Dory is `rpc:<socket_id>`.
- If `command` and `args` are both omitted, Dory expects this service to already be running.
- If `command` is omitted but `args` is present, Dory launches `dory-driver-host`, and your `args` must include both `--driver` and `--socket` with the same socket ID.
...
- **Driver does not appear in UI**: check the Services settings panel and Dory logs for launch/probe diagnostics.
...
- **Version mismatch**: ensure example and Dory are built from compatible code.
