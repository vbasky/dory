//! Example custom auth provider for Dory.
//!
//! This is a minimal implementation of the Dory auth-provider RPC protocol.
...
//!
//! Register the binary from Settings → RPC Services in Dory as an
...
use dory_core::chrono::{self, Duration};
use dory_core::{
    AuthFormDef, DbError, FormFieldDef, FormFieldKind, FormSection, FormTab, SelectOption,
};
use dory_core::secrecy::SecretString;
use dory_ipc::auth::AUTH_PROVIDER_RPC_AUTH_TOKEN_ENV;
use dory_ipc::auth_provider_protocol::{
...
fn parse_args() -> Args {
    ...
    while let Some(arg) = args.next() {
        match arg.as_str() {
            ...
            "--help" | "-h" => {
                println!("Custom Auth Provider Example for Dory");
                println!();
