...
This example is a standalone auth-provider host that speaks the Dory auth-provider RPC protocol.
...
RPC services are created from the Dory UI.

1. Build the example binary.
2. Start Dory normally.
...
5. Save the service.
6. Restart Dory so the new RPC service is probed on startup.
...
10. Save the auth profile and run a login/credential resolution flow through Dory.

## Process ownership

- If Dory starts this service from `Settings → RPC Services`, Dory tracks it and stops it on shutdown.
- If you start the service manually and leave both `command` and `args` empty in the UI, Dory uses the running socket but does not own or stop that process.

## Notes

- Auth-provider services never appear as database drivers.
- For managed auth-provider services, Dory requires an explicit `command`; it does not assume `dory-driver-host`.
- Dory injects its auth-provider handshake token automatically when it launches the service.
- Use absolute paths for `command` while testing to avoid PATH issues.

## Troubleshooting

- **Provider does not appear in Auth Profiles**: confirm the service is enabled in `Settings → RPC Services`, then restart Dory and check the logs for probe failures.
- **Connection refused**: ensure `socket_id` matches between the saved service and the `--socket` argument.
- **Version mismatch**: ensure the example and Dory were built from compatible code.
