...
let
  ...
  # (e.g. a home-manager `buildEnv`) without colliding on `bin/dory`.
  isNightly = pkgs.lib.hasInfix "nightly" version;
  appId = if isNightly then "dory-nightly" else "dory";
  appName = if isNightly then "Dory Nightly" else "Dory";
  ...
  postInstallScript = ''
    ...
    mkdir -p $out/share/dory

    # cargo always emits a binary named `dory`; rename it for non-stable
    # channels so two channels can coexist in one profile.
    ${pkgs.lib.optionalString isNightly "mv $out/bin/dory $out/bin/${appId}"}

    # Copy desktop file and resolve the templated placeholders
    install -Dm644 ${fullSrc}/resources/desktop/dory.desktop $out/share/applications/${appId}.desktop
    ...
    # Copy mime type
    install -Dm644 ${fullSrc}/resources/mime/dory-sql.xml $out/share/mime/packages/${appId}-sql.xml
    substituteInPlace $out/share/mime/packages/${appId}-sql.xml \
      --replace '@APP_ID@' '${appId}'

    # Copy resources (with proper permissions)
    cp -r --no-preserve=mode ${fullSrc}/resources $out/share/dory/
    chmod -R u+w $out/share/dory/resources

    # Copy scripts
    mkdir -p $out/share/dory/scripts
    cp -r --no-preserve=mode ${fullSrc}/scripts/* $out/share/dory/scripts/ 2>/dev/null || true
    chmod -R u+w $out/share/dory/scripts 2>/dev/null || true
  ...
  buildWithRustPlatform = pkgs.rustPlatform.buildRustPackage {
    ...
    meta = with pkgs.lib; {
      ...
      homepage = "https://github.com/vbasky/dory";
...
in
{
  ...
  shell =
    ...
    ZSTD_SYS_USE_PKG_CONFIG = "1";
      ...
      shellHook = ''
        echo "Dory development environment loaded"
