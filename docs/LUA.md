...
The `dory_lua` crate is Dory's sandboxed Lua 5.4 runtime for connection hooks. This document describes the crate's architecture, the Lua API exposed to hook scripts, the sandbox and timeout model, and how the runtime is wired into the application.

---

## What This Crate Does

`dory_lua` lets users write Lua scripts that run during connection lifecycle events (pre-connect, post-connect, pre-disconnect, post-disconnect). The hooks are general-purpose: they can drive SSO login flows, environment setup, audit logging, or triggering external tools before/after a connection opens.
...
| `dory_core` | workspace | Traits (`HookExecutor`), types (`ConnectionHook`, `HookContext`, etc.)                                 |
...
| `os`        | System calls: `os.execute()` would be a full shell escape, `os.remove()` can delete files. Even `os.getenv()` is replaced with the gated `dory.env.get()`. |
...
### `dory.env.*` — Environment Variables
...
### `dory.process.*` — Controlled Process Execution
...
\* For a non-detached `run`, `timeout_ms` is effectively required when there is no hook-level timeout: a call with neither a `timeout_ms` nor a hook-level timeout fails with the runtime error `"dory.process.run requires a timeout_ms when no hook-level timeout is set"`. A detached `run` is exempt from this constraint.
...
**Limitation**: This hook only fires for Lua bytecode instructions. If the script calls a blocking Rust function (like `dory.process.run`), the instruction hook won't fire until that function returns. That's why...

### Layer 2: Shared Process Executor

Inside `dory.process.run`, process execution is delegated to the shared `dory_core::execute_streaming_process()` helper. That helper:
...
Defined in `dory_core::connection::hook`:
...
The capability checks happen at VM creation time, not at call time. If `logging` is false, the `dory.log` table simply doesn't exist in the VM. There's no runtime check; the sandbox is structural.
...
The `output` sender is optional. When present, Lua log calls and `dory.process.run({ stream = true })` forward live output to the UI while still preserving the final buffered output in `HookResult`.
...
### The `dory` Table Lazy Init Pattern
...
Each `register_*_api` function calls this to get-or-create the `dory` global. This allows capabilities to be registered independently without knowing about each other — each just adds its sub-table to the shared parent.
...
- **Log liberally** — `dory.log.info()` output appears in the result panel. It's the only way to communicate progress and debug issues.
...
Everything is synchronous and blocking. The Lua VM runs on a background thread, and `dory.process.run` blocks that thread until the shared process executor finishes. For most hook use cases (CLI tool calls, environment checks), this is fine. But you can't do async HTTP requests or parallel operations.

### No Network Access

There's no HTTP client, socket library, or network API. The only way to interact with external services is through `dory.process.run` with an allowlisted CLI tool. This is intentional — a sandboxed HTTP client would need careful URL filtering and would expand the attack surface significantly.
...
The supported way to communicate progress and diagnostics is `dory.log.*`. That output is buffered into the final `HookResult`, and it can also be streamed live when the caller requests it.
...
The optional `dory_lua` dependency and its `lua` feature live on the app crate, `crates/dory_app/Cargo.toml`:
...
The binary crate, `crates/dory/Cargo.toml`, has no direct `dory_lua` dependency. Its `lua` feature simply forwards to the app and UI crates, and is part of the default set:
...
`crates/dory_app/src/hook_executor.rs` defines the router (re-exported from `crates/dory_app/src/lib.rs`):
...
- `api/dory.rs`: process option validation, expired hook timeout handling before spawn, live log event formatting, and streamed partial stdout/stderr during cancellation
...
The three-layer timeout (instruction hook, shared process executor, per-process timeout) can be confusing. The key insight: **process-level timeout is recoverable** (the script continues), **hook-level timeout is not** (the hook fails). So you should always set `timeout_ms` on `dory.process.run` calls to something lower than the hook's timeout, allowing the script to handle the failure gracefully.

### Why Not Just Allow `os.execute()`?

It might seem simpler to load the `os` library and let users run whatever they want. The problem is that `os.execute()` provides no output capture, no timeout, no cancellation, and no program filtering. The `dory.process.run` API gives us all of these. The allowlist is the price of constraining which programs a hook can launch in a GUI app that runs user scripts. It is an ergonomics/footgun guard rather than a security isolation boundary — PATH substitution can still swap the binary behind an allowed name.
