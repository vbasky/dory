...
scripts, or Lua snippets Dory runs around a connection's lifecycle.
...
**Theme mode** System follows the OS appearance and swaps between the chosen dark and light palettes. Dark and Light lock the workspace to that appearance regardless of the OS.

The language list is derived from Dory's shipped translation catalogs: English
...
effect after you restart Dory, so the control shows a permanent note to that
...
These three settings govern how Dory treats risky queries across **all**
...
| Setting | Default | What it does |
|---------|---------|--------------|
| **Use the stable database** | Off | Make a Nightly build share the stable `dory.db` instead of `dory-nightly.db`. Applies on next launch. |
...
sets how much of Dory's internal logging is folded into the audit trail. Saving
...
External drivers and auth providers run as separate processes that Dory talks
...
| **Lua** | An in-process Lua script | A file path and a set of capabilities (see below). Lua runs inside Dory — no external interpreter. |

Scripts are edited in Dory's editor and stored under a `hooks/` folder by
default.
...
| **Controlled process run** | Off | Call `dory.process.run(...)` to launch external processes. |

> Enabling **Controlled process run** lets the hook execute arbitrary external
> commands. Dory shows a security warning when it's on, both in the hook
...
| **Inherit parent environment** | On by default; pass Dory's env to the hook. |
...
| **Ready signal** (Detached) | Text Dory waits for in the hook's output before continuing. |
| **On Failure** | The failure policy — see below. |

Dory always injects context env vars into process hooks: `DORY_PROFILE_ID`,
`DORY_PROFILE_NAME`, `DORY_DB_KIND`, and, when known, `DORY_HOST`,
`DORY_PORT`, `DORY_DATABASE`.

> **Secrets never leak into hooks by accident.** On top of your Env Denylist,
> Dory always strips inherited variables whose name contains `SECRET`, `TOKEN`,
