# Dory AI + MCP Integration Guide

This guide explains how to integrate AI agents with Dory via the standalone MCP server binary.
...
Dory exposes MCP server functionality via the `dory mcp` subcommand that speaks the Model Context Protocol over stdio. AI clients (Claude Desktop, Cursor, etc.) launch this binary as a subprocess and communicate via JSON-RPC 2.0, newline-delimited.
...
The MCP server and the Dory GUI app are independent processes. They share the same unified SQLite database at `~/.local/share/dory/dory.db` (profiles, governance, audit, history, sessions). Governance configured in the GUI (trusted clients, roles, policies, per-connection settings) is read by the server from that database on startup. The `--config-dir` flag is accepted for CLI compatibility but does not relocate the unified database; governance and audit always read from `~/.local/share/dory/dory.db`.
...
The MCP server is integrated into the main `dory` binary.
...
| `--config-dir <path>` | Accepted for CLI compatibility. The governance/audit database is always resolved to the unified `~/.local/share/dory/dory.db`; this flag does not relocate it. For isolated test environments, override `HOME`/`XDG_DATA_HOME` instead. |
...
The `client-id` value must match a trusted client entry you created in the Dory GUI under **Settings → MCP → Clients**.

**Note**: If you built Dory without the `mcp` feature (`--no-default-features`), the MCP server will not be available.
...
Built-ins are injected at startup in both the GUI app (`AppState`) and the MCP server (via the `builtin_policies()` / `builtin_roles()` loops in `dory_mcp_server::governance`). They are never written to disk. Any attempt to delete a built-in returns an error.
...
## 7. Operator Setup in Dory GUI

Configure governance in the Dory GUI before starting the MCP server.
...
Dory persists all state in a single unified SQLite database and a few supporting directories. Paths are resolved by `dirs` (`XDG_*` on Linux, `~/Library` on macOS).

Typical Linux defaults:

| Path | Contents |
|------|----------|
| `~/.local/share/dory/dory.db` | Unified database: profiles, auth, SSH tunnels, governance, audit events, history, sessions, UI state |
| `~/.local/share/dory/sessions/` | Scratch and shadow files for auto-save session restore |
| `~/.local/share/dory/scripts/` | User-authored scripts directory |

The `dory.db` database contains all domain tables under prefixed schemas:
...
Important for tests: do not use real user directories. Pass `--config-dir` to the binary or set `HOME`/`XDG_CONFIG_HOME`/`XDG_DATA_HOME` to temp paths for isolated runs. The `dory_audit::temp_sqlite_path(name)` helper generates isolated paths for audit tests.
...
- [ ] `dory` built with MCP support (enabled by default, or with `--features mcp`)
- [ ] Trusted client registered and active in Dory GUI
...
- Do not read/write `~/.config/dory` or `~/.local/share/dory` in test code.
- Built-in policies and roles are available without any setup — do not insert them manually in test fixtures.
- The `dory_audit::temp_sqlite_path(name)` helper generates an isolated path for each test.
...
- Check the pending queue in the Dory workspace and approve/reject explicitly.
