...
This file documents the storage and management of RPC services in Dory.

Dory now persists a first-class RPC services foundation through `RpcServiceKind`:
...
RPC services are stored in SQLite at `~/.local/share/dory/dory.db`, not in a JSON file.
...
- Dory preserves compatibility for driver registration IDs as `rpc:<socket_id>`.
- If API metadata is missing on an existing driver row, Dory defaults it to the current `driver_rpc` contract at version `1.1`.
- If API metadata is missing on an auth-provider row, Dory defaults it to the current `auth_provider_rpc` contract at version `1.2`.
- `api_family` / `api_major` are used as startup preflight for auth providers before Dory probes the socket.

## Semantics

- `socket_id` is used literally as the socket filename
- Dory internally identifies each service as `rpc:<socket_id>`
- Dory classifies each service by `service_kind` before runtime adaptation
...
- `socket_id` (required): local socket name used by Dory and the service.
...
- `command` (optional): executable to run when Dory needs to start the service.
  - If omitted and `args` is also empty, Dory treats the service as already running and does not spawn anything.
  - For `driver`, if omitted and `args` is non-empty, Dory launches `dory-driver-host`.
  - For `auth_provider`, if Dory must launch the service, `command` must be set explicitly.
...
- Relative `command` path that does not resolve under the Dory process environment
...
- Omitting `command` while providing partial `args`; if you want Dory to launch the default host, `args` must include both `--driver` and `--socket`.
- Configuring an auth-provider service with `args` but no `command`; Dory will reject that launch config instead of assuming the driver host
