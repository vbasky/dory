...
Dory can turn a query result into a chart. The charting engine is fully
...
The chart engine lives in the `dory_components` crate under
`crates/dory_components/src/chart/`. Its `mod.rs` describes the full
pipeline:
...
in `crates/dory_components/src/result_panel/`.
...
`detect_chart_columns` in `crates/dory_components/src/chart/detect.rs` applies
...
`crates/dory_components/src/saved_chart.rs`. Saved charts are stored in the
...
(`crates/dory_ui_base/src/saved_chart_manager.rs`). Writes go to the
...
`crates/dory_ui/src/ui/views/workspace/actions.rs`) routes by source type:
...
`crates/dory_ui_document/src/dedup.rs`. Before opening a saved chart,
...
`crates/dory_ui_document/src/data_grid_panel/context_menu.rs`, which
requires both:
