...
option only appears when the result has a usable original query and Dory can
...
When a driver supports it (CloudWatch is the reference implementation), Dory can
...
Remote dashboards are read-only browsing/import; Dory never modifies the source.

---

## Audit viewer

The audit viewer is the single place to review everything Dory logged: queries,
...
When something you did fails, Dory shows a toast with a **View in Audit** action.
...
lives in the same `dory.db` as everything else
