...
## The Access tab: how Dory reaches the database
...
Dory runs a fixed pre-connect pipeline before the driver ever opens a socket:
...
3. **Opening access** — establish the SSH tunnel, proxy, or SSM session (or
   nothing, for Direct).
4. **Driver connect + schema fetch** — the driver connects and Dory loads the
   shallow schema.
...
Auth Profiles hold provider-driven authentication that Dory resolves at
...
> AWS profiles that Dory reflects live from your `~/.aws/config` appear as
...
When a connection (or the Account/Role dropdowns) needs an SSO session, Dory
opens a login modal:
...
- Dory continues automatically once you finish authenticating in the browser.
...
The **local** tunnel port is assigned automatically by Dory and the OS — only
...
- When you connect through an SSH/proxy/SSM tunnel, Dory always uses the
...
- **Secrets live in your OS keyring**, never in the Dory database. If the
