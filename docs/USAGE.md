...
A practical, end-user introduction to working with Dory: connecting to a
database, browsing its schema, running queries, working with results, charting,
and the keyboard model.

Dory is keyboard-first. Almost every action has both a mouse affordance and a
...
On startup Dory restores your previous session (open tabs). On a fresh
...
The **Access** tab controls how Dory reaches the database:
...
- **Managed** — provider-managed access (for example `aws-ssm`), where Dory
  opens access through an external provider before connecting.

When you connect, Dory runs a pre-connect pipeline: authentication and session
...
Schema is loaded lazily. On connect, Dory fetches shallow metadata (names).
...
statements, and the active driver advertises batch support, Dory shows a
...
Dory detects dangerous operations across languages — SQL `DELETE`/`DROP`/
...
onto a different connection; on import Dory verifies that the referenced tables
...
- **Relational (ORM-style) paths** — type a dotted path that walks foreign keys,
  ...
  `created_by.organization.name = 'Acme'`. Dory resolves the path against the
...
Dory keeps a history of completed queries and lets you save named queries.
...
Dory uses a layered, context-aware keymap. The active layer depends on which
