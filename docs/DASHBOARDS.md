...
Dory persists chart configurations as **Saved Charts** and groups them into
...
All dashboard and saved-chart data lives in `~/.local/share/dory/dory.db`
...
Repositories live in `crates/dory_storage/src/repositories/viz_*.rs` and
...
(`crates/dory_ui_document/src/chart_document/`), keyed by
...
(`crates/dory_ui_base/src/dashboard_manager.rs`) carries no chart reference —
...
Execution lives in
`crates/dory_ui_document/src/instance_inspector/mod.rs`.
...
`crates/dory_driver_cloudwatch/src/` provides:
...
This is **read-only browsing and import**, not a sync feature. Dory never
