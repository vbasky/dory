# Prebuilt-binary derivation for Dory.
#
...

{
  ...
  # that callers that pass no argument (i.e. dory-bin) are byte-for-byte
...
let
  ...
  artifact = releaseInfo.artifacts.${system} or (throw
    "dory: no prebuilt binary published for system '${system}'. "
    + "Use the source build (packages.dory-source) instead.");
  ...
  # home-manager `buildEnv`) without colliding on `bin/dory`.
...
stdenv.mkDerivation {
  ...
  installPhase = ''
    ...
    install -Dm755 dory $out/bin/${appId}
    ...
    install -Dm644 resources/desktop/dory.desktop \
    ...
    # and tarballs from before the branding split shipped resources/icons/dory.svg.
    ...
    elif [ -f resources/icons/dory.svg ]; then
      install -Dm644 resources/icons/dory.svg "$icon_dst"
    else
      echo "dory: no brand mark found in the release tarball" >&2
      exit 1
    fi

    install -Dm644 resources/mime/dory-sql.xml \
